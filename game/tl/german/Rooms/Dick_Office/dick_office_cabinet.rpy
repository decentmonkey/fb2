# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:42
translate german dick_office_cabinet_environment_a295a245:

    # "Я не верю в то что происходит!!!"
    "Ich kann nicht fassen, dass das passiert!!!"

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:46
translate german dick_office_cabinet_environment_93e4e7c0:

    # mt "Вот он! Жирный ублюдок, за которым я столько бегала!"
    mt "Da ist er ja. Der fette Bastard, den ich verfolge!"

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:52
translate german dick_office_cabinet_environment_84ed6bc3:

    # mt "Умные юридические книжки."
    # "Дик в них зарылся."
    # "Надеюсь с них будет толк в моем деле, иначе от них нет никакой пользы!"
    mt "Anwaltsbücher."
    "Dick ist in sich vertieft."
    "Ich hoffe, das hilft mir, sonst ist es nutzlos!"

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:58
translate german dick_office_cabinet_environment_01eea9a8:

    # mt "Что это такое? Это ведь не Дик?"
    # "Может это тот на кого он пытается быть похож?"
    # "Ему до этого далеко!"
    mt "Was ist das? Ist das Dick?"
    "Vielleicht ist das, wie er gerne sein will."
    "Davon ist er weit entfernt."

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:62
translate german dick_office_cabinet_environment_399e1442:

    # mt "Здесь столько стульев!"
    # "Дик мог бы и предложить мне один!"
    mt "Viele Stühle hier!"
    "Dick hätte mir einen anbieten können!"

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:65
translate german dick_office_cabinet_environment_5041bc79:

    # mt "Вазы, цвет..."
    # "Откуда все это здесь?"
    # "Явно чья-то женская рука..."
    mt "Bunte Vasen..."
    "Wo kommen die her?"
    "Eindeutig von einer Frau..."

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:69
translate german dick_office_cabinet_environment_bd7c4edd:

    # mt "Что это?"
    mt "Ist das..?"

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:75
translate german dick_office_cabinet_environment_3be0e16d:

    # mt "ЧТО????"
    # "ОНА?? ЗДЕСЬ?!?!"
    mt "WAS???"
    "SIE??? HIER???"

translate german strings:

    # Rooms/Dick_Office/dick_office_cabinet.rpy:28
    old "СЕКРЕТАРЬ"
    new "SEKRETÄRIN"

# TODO: Translation updated at 2018-12-08 17:35

# game/Rooms/Dick_Office/dick_office_cabinet.rpy:47
translate german dick_office_cabinet_environment_f85fb8b3:

    # mt "Мне надо как-то вырвать Дика из когтей этой сучки!"
    mt "Irgendwie muss ich Dick aus ihren Krallen befreien!"

