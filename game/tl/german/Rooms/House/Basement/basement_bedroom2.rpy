# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/House/Basement/basement_bedroom2.rpy:45
translate german basement_bedroom2_environment_4c19961a:

    # mt "Здесь я теперь сплю..."
    # "(хмык)"
    # "Но это временно!!!"
    # "Это какое-то недоразумение, которое скоро разрешится!"
    mt "Ich schlafe hier..."
    "(schnief)"
    "Aber das ist nur übergangsweise!!!"
    "Das ist ales ein Missverständnis, das bald aufgeklärt sein wird!"

# game/Rooms/House/Basement/basement_bedroom2.rpy:57
translate german basement_bedroom2_environment_f1f099e4:

    # mt "Моя кровать..."
    # "Но это временно!!!"
    # "Это какое-то недоразумение, которое скоро разрешится!"
    mt "Mein Bett..."
    "Aber das ist nur übergangsweise!!!"
    "Das ist ales ein Missverständnis, das bald aufgeklärt sein wird!"

# game/Rooms/House/Basement/basement_bedroom2.rpy:62
translate german basement_bedroom2_environment_dbc51c20:

    # mt "Мне надо раздеться сначала"
    mt "Ich muss mich erst ausziehen."

# game/Rooms/House/Basement/basement_bedroom2.rpy:74
translate german basement_bedroom2_environment_544975d9:

    # mt "Какая-то книга Юлии..."
    mt "Ein Buch von Julia..."

# game/Rooms/House/Basement/basement_bedroom2.rpy:80
translate german basement_bedroom2_environment_0beb8f0a:

    # mt "Старый шкаф..."
    mt "Alter Kleiderschrank..."

# game/Rooms/House/Basement/basement_bedroom2.rpy:87
translate german basement_bedroom2_environment_a9f53f6a:

    # mt "Эта тусклая лампа - это почти весь свет, который есть в этой каморке..."
    mt "Eine schwache Lampe. Wohl das einzige Licht in diesem Schrank..."

# game/Rooms/House/Basement/basement_bedroom2.rpy:89
translate german basement_bedroom2_environment_dbe585da:

    # mt "Живопись..."
    # "Я не знала что Юлия увлекается этим."
    # "Она пыталась быть похожей на меня?"
    # "..."
    mt "Gemälde..."
    "Ich wusste nicht, dass das Julia gefällt."
    "Hat sie versucht, wie ich zu sein?"
    "..."

# game/Rooms/House/Basement/basement_bedroom2.rpy:95
translate german basement_bedroom2_environment_cab17d2a:

    # mt "Этот старый яркий пестрый стол пытается скрасить уныние этой каморки..."
    # "Тщетно..."
    mt "Dieser alte, bunte Tisch versucht diesen tristen Raum aufzuhellen."
    "Vergebens..."

translate german strings:

    # Rooms/House/Basement/basement_bedroom2.rpy:33
    old "ШКАФ"
    new "UMKLEIDE"

# TODO: Translation updated at 2018-12-12 15:08

# game/Rooms/House/Basement/basement_bedroom2.rpy:48
translate german basement_bedroom2_environment_55fca1d7:

    # mt "Здесь я теперь сплю..."
    # "(хмык)"
    # "Но это временно!!!"
    # "Это какое-то недоразумение, которое скоро разрешится!"
    mt "Ich schlafe hier..."
    "(schnief)"
    "Aber das ist nur übergangsweise!!!"
    "Das ist ales ein Missverständnis, das bald aufgeklärt sein wird!"

# game/Rooms/House/Basement/basement_bedroom2.rpy:64
translate german basement_bedroom2_environment_fb695496:

    # mt "Моя кровать..."
    # "Но это временно!!!"
    # "Это какое-то недоразумение, которое скоро разрешится!"
    mt "Mein Bett..."
    "Aber das ist nur übergangsweise!!!"
    "Das ist ales ein Missverständnis, das bald aufgeklärt sein wird!"

