# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/House/bedroom_bardie.rpy:41
translate german bedroom_bardie_environment_5f0c394b:

    # mt "Раньше здесь была моя гардеробная."
    # "Они переделали мою гардеробную под комнату этого оболтуса!!!"
    mt "Das war mal mein begehbarer Kleiderschrank."
    "Mein Schrank sieht jetzt aus wie das Zimmer von dem Trottel."

# game/Rooms/House/bedroom_bardie.rpy:46
translate german bedroom_bardie_environment_14c17054:

    # mt "Этот оболтус разбрасывает вещи где попало!"
    mt "Diese Idiotin schmeisst ihre Sachen überall rum!"

# game/Rooms/House/bedroom_bardie.rpy:48
translate german bedroom_bardie_environment_0cb9b3ed:

    # mt "Я сейчас мечтаю даже о такой дурацкой кровати!"
    mt "Jetzt träume ich sogar von so einem dämlichen Bett!"

# game/Rooms/House/bedroom_bardie.rpy:50
translate german bedroom_bardie_environment_215a7c63:

    # mt "Стул Барди..."
    # "Хорошо бы он свалился с него!!!"
    mt "Bardies Stuhl..."
    "Ich wünschte, er wäre damit umgekippt!!!"

# game/Rooms/House/bedroom_bardie.rpy:53
translate german bedroom_bardie_environment_e083f928:

    # mt "Я выкину эти его музыкальные штуки, когда верну себе мой дом!"
    mt "Sein Zeug fliegt auch raus, wenn ich mein Haus zurück habe!"

# game/Rooms/House/bedroom_bardie.rpy:55
translate german bedroom_bardie_environment_0c08bf3e:

    # mt "Дурацкие детские постеры!"
    mt "Dumme Babyposter!"

# game/Rooms/House/bedroom_bardie.rpy:57
translate german bedroom_bardie_environment_81c6d78f:

    # mt "В этой комнате куча хлама!"
    # "Этот оболтус не убирает за собой!"
    mt "Soviel Müll in diesem Zimmer!"
    "Dieser Spast räumt nicht auf."

# game/Rooms/House/bedroom_bardie.rpy:60
translate german bedroom_bardie_environment_1c61f77e:

    # mt "Компьютер этого оболтуса."
    # "Интересно, что он там смотрит?"
    mt "Das ist der Idiotencomputer."
    "Ich frage mich, was er anschaut."

# game/Rooms/House/bedroom_bardie.rpy:79
translate german bedroom_bardie_environment_09a5cf46:

    # mt "Фу! Какая мерзость!"
    mt "EH! Was für eine Scheuslichkeit!"

translate german strings:

    # Rooms/House/bedroom_bardie.rpy:62
    old "Посмотреть на компьютер."
    new "Schau auf den PC."

    # Rooms/House/bedroom_bardie.rpy:62
    old "Уйти..."
    new "Verschwinde..."

