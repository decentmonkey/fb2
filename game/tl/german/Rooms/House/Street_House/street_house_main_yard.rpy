# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/House/Street_House/street_house_main_yard.rpy:46
translate german street_house_main_yard_environment_9fb6cf0f:

    # mt "Моя бывшая машина..."
    # "Но я верну все назад, клянусь!"
    # "Я устроим им всем АД!!!"
    mt "Mein altes Auto..."
    "Ich bekomme alles zurück, ich schwöre!"
    "Und ich mache ihnen Allen alles zur HÖLLE!!!"

# game/Rooms/House/Street_House/street_house_main_yard.rpy:52
translate german street_house_main_yard_environment_1f97ba8c:

    # mt "Фред... Мой бывший водитель..."
    # "Редкий ублюдок..."
    # "Он не избежит своей участи!"
    mt "Fred... Mein ehemaliger Fahrer..."
    "Selten dummer Bastard..."
    "Er wird seinem Schicksal nicht entkommen!"

# game/Rooms/House/Street_House/street_house_main_yard.rpy:57
translate german street_house_main_yard_environment_60cdfbfc:

    # mt "Мне не о чем разговаривать с ним!!!"
    mt "Ich habe nichts mit ihm zu besprechen!!!"

# game/Rooms/House/Street_House/street_house_main_yard.rpy:60
translate german street_house_main_yard_environment_060a1c80:

    # mt "(хмык)"
    mt "(schnief)"

translate german strings:

    # Rooms/House/Street_House/street_house_main_yard.rpy:18
    old "В ДОМ"
    new "IN DAS HAUS"

    # Rooms/House/Street_House/street_house_main_yard.rpy:22
    old "К ЗАБОРУ"
    new "ZUM ZAUN"

    # Rooms/House/Street_House/street_house_main_yard.rpy:24
    old "К ВОРОТАМ"
    new "ZUM TOR"

