# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:44
translate german rich_hotel_event_scene_environment_3f064834:

    # beef "Ну, давай кукла!"
    # "Притворись Моникой Бакфетт!"
    beef "Ну, давай кукла!"
    "Притворись Моникой Бакфетт!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:47
translate german rich_hotel_event_scene_environment_71473458:

    # "Или тебя надо шлепнуть по попке, чтобы ты заговорила?!"
    "Или тебя надо шлепнуть по попке, чтобы ты заговорила?!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:51
translate german rich_hotel_event_scene_environment_3d99d345:

    # mt "Мелани смотрит на меня."
    # "Интересно что она думает обо всем этом..."
    mt "Мелани смотрит на меня."
    "Интересно что она думает обо всем этом..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:56
translate german rich_hotel_event_scene_environment_e24d0151:

    # mt "Эти люди..."
    # "Здесь столько людей... Они все смотрят на меня..."
    # "Ждут..."
    # "Что же мне делать???"
    # "Сказать им правду?"
    mt "Эти люди..."
    "Здесь столько людей... Они все смотрят на меня..."
    "Ждут..."
    "Что же мне делать???"
    "Сказать им правду?"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:63
translate german rich_hotel_event_scene_environment_a127c17c:

    # mt "Эта одежда закрывает собой мой постер!"
    mt "Эта одежда закрывает собой мой постер!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:66
translate german rich_hotel_event_scene_environment_5b5b3f94:

    # mt "Эта дверь... Может быть мне просто убежать?"
    # "Но нет, это не выход из положения..."
    mt "Эта дверь... Может быть мне просто убежать?"
    "Но нет, это не выход из положения..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:70
translate german rich_hotel_event_scene_environment_c7064c5c:

    # mt "Это место так украшено цветами..."
    mt "Это место так украшено цветами..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:73
translate german rich_hotel_event_scene_environment_65b86e51:

    # mt "Милый светильник."
    # "Приятный свет."
    mt "Милый светильник."
    "Приятный свет."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_scene.rpy:78
translate german rich_hotel_event_scene_environment_c8ebb783:

    # mt "Мелани?! Здесь?!"
    # "А почему не Я?!"
    mt "Мелани?! Здесь?!"
    "А почему не Я?!"

