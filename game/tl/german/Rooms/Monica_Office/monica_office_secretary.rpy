# TODO: Translation updated at 2018-12-05 17:53

# game/Rooms/Monica_Office/monica_office_secretary.rpy:59
translate german monica_office_secretary_environment_44e0e614:

    # mt "Это этаж моего главного офиса."
    # "У меня здесь все под рукой."
    # "Моя секретарь, фото-студия и мой кабинет."
    # "Вернее сейчас кабинет не мой, но скоро будет МОЙ снова!!!"
    mt "Das ist das Stockwerk mit meinem Hauptbüro."
    "Ich habe hier alles zur Hand."
    "Meine Sekretärin, mein Fotostudio und mein Büro."
    "Der Schrank gehört derzeit nicht mir, aber bald wird er wieder meiner sein!!!"

# game/Rooms/Monica_Office/monica_office_secretary.rpy:65
translate german monica_office_secretary_environment_08ba9afd:

    # mt "Моя секретарь слишком много увлекается чтением."
    mt "Meine Sekretärin ist viel mit Lesen beschäftigt."

# game/Rooms/Monica_Office/monica_office_secretary.rpy:68
translate german monica_office_secretary_environment_e672e7bf:

    # mt "Моя секретарь играет на музыкальных инструментах."
    mt "Meine Sekretärin spielt ein Instrument."

# game/Rooms/Monica_Office/monica_office_secretary.rpy:71
translate german monica_office_secretary_environment_ee057710:

    # mt "За этим столом моя секретарь проводит почти все рабочее время."
    mt "Meine Sekretärin verbringt die meiste Zeit hier an diesem Tisch."

# game/Rooms/Monica_Office/monica_office_secretary.rpy:74
translate german monica_office_secretary_environment_9b84380b:

    # mt "Из этих окон ужасный вид на город!"
    mt "Von hier aus hat man eine schreckliche Aussicht!"

# game/Rooms/Monica_Office/monica_office_secretary.rpy:77
translate german monica_office_secretary_environment_15bd50a9:

    # mt "Это чайный столик."
    # "Моя секретарь обычно угощает чаем, либо кофе тех, кто ожидает моего приема."
    # "Неудивительно что выпечки и кофе здесь больше нет..."
    # "А жаль, я бы с удовольствием съела что-нибудь!"
    mt "Das ist der Teetisch."
    "Meine Sekretärin serviert hier normalerweise Tee oder Kaffee für die, die auch mich warten."
    "Kein Wunder dass wir hier kein gebäck und Kaffee mehr haben..."
    "Echt schade. Ich würde so gerne etwas essen!"

# game/Rooms/Monica_Office/monica_office_secretary.rpy:89
translate german monica_office_secretary_environment_f6f37a04:

    # mt "Это моя секретарша."
    # mt "Одна из немногих нормальных женщин, которые знают как держать себя с окружающими."
    # "Она очень воспитана и интеллигентна."
    # "Я знаю, она сохранит верность мне и поможет вернуть все назад..."
    mt "Das ist meine Sekretärin."
    mt "Eine der wenigen Personen, die weiß, wie man sich um Umgang mit Anderen verhält."
    "Sie ist wohl erzogen und intelligent."
    "Ich weiß sie steht loyal zu mir und würde mir auf jede Weise helfen, um mir mein Leben zurück zu holen..."

translate german strings:

    # Rooms/Monica_Office/monica_office_secretary.rpy:33
    old "ФОТОСТУДИЯ"
    new "FOTOSTUDIO"

    # Rooms/Monica_Office/monica_office_secretary.rpy:35
    old "КАБИНЕТ МОНИКИ"
    new "BÜRO MONICA"

