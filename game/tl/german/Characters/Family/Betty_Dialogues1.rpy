# TODO: Translation updated at 2018-12-06 21:43

# game/Characters/Family/Betty_Dialogues1.rpy:17
translate german bettyDialogue2_f5e3591c:

    # mt "Бетти отдыхает..."
    # "Мне лучше уйти отсюда пока она на меня не наорала."
    mt "Betty ruht sich aus..."
    "Ich sollte hier verschwinden, bevor sie mich anschreit."

# game/Characters/Family/Betty_Dialogues1.rpy:20
translate german bettyDialogue2_58d95acf:

    # "Сучка..."
    "Schlampe!"

# game/Characters/Family/Betty_Dialogues1.rpy:24
translate german bettyDialogue3_7562f25e:

    # mt "Мне лучше не подходить к Бетти в таком виде..."
    # "Она и так все время называет меня шлюхой..."
    mt "In diesem Aufzug gehe ich besser nicht zu Betty..."
    "Dauernd nennt sie mich eine Nutte..."

# game/Characters/Family/Betty_Dialogues1.rpy:27
translate german bettyDialogue3_58d95acf:

    # "Сучка..."
    "Schlampe!"

# TODO: Translation updated at 2019-08-29 19:51

# game/Characters/Family/Betty_Dialogues1.rpy:9
translate german bettyDialogue1_269ef339:

    # betty "Я буду следить за тем как ты работаешь."
    betty "Ich schaue dir auf die Finger."

