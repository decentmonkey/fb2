# TODO: Translation updated at 2018-12-05 17:53

# game/Characters/Monica/Wardrobe_Basement.rpy:8
translate german wardrobeBasement_b658c23d:

    # mt "Что мне одеть?"
    mt "Was soll ich anziehen?"

# game/Characters/Monica/Wardrobe_Basement.rpy:59
translate german wardrobeBasement_dialogue1_whore_be4692e0:

    # mt "Никогда не представляла себя в этом..."
    # "Это просто ужас..."
    mt "Ich hätte mich nie in dieser Lage vorstellen können..."
    "Es ist furchtbar..."

# game/Characters/Monica/Wardrobe_Basement.rpy:62
translate german wardrobeBasement_dialogue1_whore_f4ed01ee:

    # mt "Я не могла подумать, когда увидела эти тряпки в хостеле, что мне придется носить их."
    # "Вместо моего красивого платья... Рррррр!!!"
    mt "Ich hätte nie gedacht, das ich sowas tragen würde, als ich diese Lumpen im Hostel gesehen habe."
    "Anstatt meinem wunderschönen Kleid... Grrrr!!!"

# game/Characters/Monica/Wardrobe_Basement.rpy:67
translate german wardrobeBasement_dialogue2_pants_d6d3051b:

    # mt "Это не мои трусики..."
    # "И мне не комфортно их носить..."
    # "Но это лучше чем вообще без них..."
    mt "Das sind nicht meine Höschen..."
    "Und ich fühle mich nicht wohl, die zu tragen..."
    "Aber besser so als ganz ohne..."

# game/Characters/Monica/Wardrobe_Basement.rpy:72
translate german wardrobeBasement_dialogue2_governess_5f3c77f6:

    # mt "Кошмарная униформа."
    # "Я никогда себя не представляла в этом..."
    mt "Ein furchtbares Kleid."
    "Darin hätte ich mich nie vorstellen können."

# game/Characters/Monica/Wardrobe_Basement.rpy:77
translate german wardrobeBasement_dialogue2_nude_5ce627fb:

    # mt "Какой ужас..."
    mt "Was für ein Horror..."

# game/Characters/Monica/Wardrobe_Basement.rpy:88
translate german hook_basement_bedroom_check_exit_cloth_b6d3bca0:

    # mt "Я не могу идти в дом в таком виде!"
    # "Этот сопляк Барди только этого и ждёт!"
    # "И, не представляю что со мной сделает Бетти за это!"
    # "Она и так все время называет меня шлюхой..."
    # "Сучка..."
    mt "So kann ich nicht rein gehen!"
    "Bardie der Pisser wartet doch nur auf sowas!"
    "Und ich habe keine Ahnung, was Betty mir antun würde dafür!"
    "Dauernd nennt sie mich eine Nutte..."
    "Miststück..."

# game/Characters/Monica/Wardrobe_Basement.rpy:101
translate german hook_basement_bedroom_check_exit_cloth_map_c28d4e8a:

    # mt "Я не могу идти на улицу в таком виде."
    # "Я еще не сошла с ума!!!"
    # "Мне надо одеться."
    mt "So kann ich nicht raus gehen."
    "Noch bin ich nicht verrückt!!!"
    "Ich brauche Kleider."

translate german strings:

    # Characters/Monica/Wardrobe_Basement.rpy:9
    old "Одежда шлюхи."
    new "Hurenkleidung."

    # Characters/Monica/Wardrobe_Basement.rpy:9
    old "Униформа гувернантки."
    new "Dienstmädchenuniform."

    # Characters/Monica/Wardrobe_Basement.rpy:9
    old "Оставить трусики Юлии."
    new "Zieh Julias Höschen aus."

    # Characters/Monica/Wardrobe_Basement.rpy:9
    old "Одеть трусики Юлии."
    new "Zieh Julias Höschen an."

    # Characters/Monica/Wardrobe_Basement.rpy:9
    old "Снять все."
    new "Zieh alles aus."

# TODO: Translation updated at 2018-12-06 21:43

# game/Characters/Monica/Wardrobe_Basement.rpy:88
translate german hook_basement_bedroom_check_exit_cloth_14d7d0c2:

    # mt "Я не могу идти в дом в таком виде!"
    # "Этот сопляк Барди только этого и ждёт!"
    # "И, не представляю что со мной сделает Бетти за это!"
    # "Она и так все время называет меня шлюхой..."
    mt "So kann ich nicht rein gehen!"
    "Bardie der Pisser wartet doch nur auf sowas!"
    "Und ich habe keine Ahnung, was Betty mir antun würde dafür!"
    "Dauernd nennt sie mich eine Nutte..."

# game/Characters/Monica/Wardrobe_Basement.rpy:93
translate german hook_basement_bedroom_check_exit_cloth_58d95acf:

    # "Сучка..."
    "Miststück..."

# TODO: Translation updated at 2019-01-14 00:27

# game/Characters/Monica/Wardrobe_Basement.rpy:76
translate german wardrobeBasement_dialogue2_pants_betty_f66a75c7:

    # mt "Это трусики Бетти..."
    # "Какой ужас..."
    mt "Das sind Bettys Höschen..."
    "Was für ein Horror..."

translate german strings:

    # Characters/Monica/Wardrobe_Basement.rpy:10
    old "Только трусики."
    new "Nur höschen."

# TODO: Translation updated at 2019-03-02 22:56

translate german strings:

    # Characters/Monica/Wardrobe_Basement.rpy:124
    old "Переодеться в одежду шлюхи (другой нет!)."
    new "Zieh die Hurenkleider an (keine Anderen!)."

    # Characters/Monica/Wardrobe_Basement.rpy:124
    old "Не переодеваться."
    new "Zieh dich nicht um."

