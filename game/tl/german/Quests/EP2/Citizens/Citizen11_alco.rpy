# TODO: Translation updated at 2019-08-26 19:44

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:8
translate german take_trash_bottle_68e0eb1b:

    # m "Похоже кто-то выкинул бутылку водки... Может ли она мне пригодиться?"
    m "Anscheinend hat jemand eine Flasche Wodka weggeworfen. Könnte die mir nützlich sein?"

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:10
translate german take_trash_bottle_68e0eb1b_1:

    # m "Похоже кто-то выкинул бутылку водки... Может ли она мне пригодиться?"
    m "Anscheinend hat jemand eine Flasche Wodka weggeworfen. Könnte die mir nützlich sein?"

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:16
translate german take_trash_bottle_bb99ad64:

    # mt "Не знаю зачем, но думаю она может пригодиться."
    mt "Ich weiß nicht warum, aber ich glaube, die ist nützlich."

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:21
translate german take_trash_bottle_6a633068:

    # m "Черт! Из нее уже кто-то пил!"
    m "Verdammisch! Aus der hat bereits jemand getrunken!"

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:25
translate german take_trash_bottle_71207c20:

    # m "Ну и ладно, оставлю ее у себя, вдруг пригодится."
    m "Na gut, ich behalte sie, falls ich sie mal brauchen sollte. Man weiß ja nie."

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:35
translate german take_trash_bottle_3fc4b018:

    # mt "Оставлю бутылку здесь. Мне ни к чему это пойло!"
    mt "Ich lasse die Flasche da, das brauche ich nicht!"

# game/Quests/EP2/Citizens/Citizen11_alco.rpy:39
translate german take_trash_bottle_3195d48e:

    # m "Ну уж нет, я не собираюсь лазать по помойкам!"
    m "Ehm, nein, ich klettere sicher nicht auf den Müll!"

translate german strings:

    # game/Quests/EP2/Citizens/Citizen11_alco.rpy:11
    old "Взять бутылку."
    new "Nimm die Flasche."

    # game/Quests/EP2/Citizens/Citizen11_alco.rpy:11
    old "Не брать."
    new "Nimm sie nicht."

    # game/Quests/EP2/Citizens/Citizen11_alco.rpy:23
    old "Оставить бутылку себе."
    new "Behalte die Flasche."

    # game/Quests/EP2/Citizens/Citizen11_alco.rpy:23
    old "Поставить бутылку на место."
    new "Stell die Flasche zurück."

    # game/Quests/EP2/Citizens/Citizen11_alco.rpy:29
    old "Водка"
    new "Wodka"

