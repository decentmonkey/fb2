# TODO: Translation updated at 2019-09-04 20:20

# game/Quests/Dick_Car_Parting.rpy:7
translate german dick_meeting1_car_parting1_a44838a4:

    # dick "Дорогая! Пожалуйста, садись в машину!"
    dick "Darling! Bitte steig ein!"

# game/Quests/Dick_Car_Parting.rpy:23
translate german dick_meeting1_car_parting2_9b936715:

    # m "Сначала дешевый магазин в какой-то дыре..."
    # "Затем пустой ресторан..."
    # m "Ну что, дорогой?"
    # "Наш вечер на сегодня окончен?"
    # dick "Моника."
    # "Ты знаешь, по пути к тебе домой есть один бар."
    # "Может быть заглянем туда на минутку?"
    # "Там весело!"
    m "Erst ein billiger Laden in einem billigen Loch..."
    "Dann ein leeres Restaurant..."
    m "Und, Schätzchen?"
    "Ist unser Abend schon vorbei für heute?"
    dick "Monica."
    "Weißt Du, auf dem Rückweg liegt noch eine Bar..."
    "Warum schauen wir nicht noch für eine Minute rein?"
    "Es ist lustig da."

# game/Quests/Dick_Car_Parting.rpy:35
translate german dick_meeting1_car_parting2_ee672605:

    # m "Дик."
    # "Я даже не знаю."
    # "Может в другой раз?"
    m "Dick."
    "Ich weiß nicht."
    "Vielleicht ein ander Mal."

# game/Quests/Dick_Car_Parting.rpy:40
translate german dick_meeting1_car_parting2_41006220:

    # dick "Ну дорогая."
    # "Я с тобой даже не успел побыть."
    dick "Na gut."
    "Ich hatte nicht mal Zeit mit dir."

# game/Quests/Dick_Car_Parting.rpy:45
translate german dick_meeting1_car_parting2_28c16d2b:

    # m "Нет, Дик!"
    # "Ты уже дважды разочаровал меня!"
    # "Боюсь, что третий раз уже слишком сильно скажется на наших отношениях!"
    # "А ты для меня пока еще полезен."
    m "Nein, Dick!"
    "Du hast mich schon zweimal enttäuscht!"
    "Ich fürchte, dass ein drittes Mal zu viel negativen Einfluss auf unsere Beziehung haben wird."
    "Aber bisher bist du noch nützlich für mich."

# game/Quests/Dick_Car_Parting.rpy:50
translate german dick_meeting1_car_parting2_e3afa1ad:

    # "Пока еще..."
    "Bisher..."

# game/Quests/Dick_Car_Parting.rpy:52
translate german dick_meeting1_car_parting2_5f93b3cb:

    # dick "Хорошо, Моника."
    # "Отвезешь меня к дому?"
    dick "Na gut, Monica."
    "Fährst du mich nach Hause?"

# game/Quests/Dick_Car_Parting.rpy:61
translate german dick_meeting1_car_parting2_361bc315:

    # m "Дик, ты правда думаешь что я буду тратить время чтобы отвозить тебя домой?"
    m "Dick, denkst du wirklich, ich veschwende meine Zeit damit, dich nach Hause zu fahren?"

# game/Quests/Dick_Car_Parting.rpy:63
translate german dick_meeting1_car_parting2_6dc338c8:

    # dick "Дорогая, я бы тебе показал где я живу и..."
    dick "Schätzchen, ich zeige dir, wo ich wohne..."

# game/Quests/Dick_Car_Parting.rpy:65
translate german dick_meeting1_car_parting2_0407bbc7:

    # m "Нет, Дик!"
    # "Я не куплюсь на такую уловку!"
    # "Мне неинтересно где ты живешь! Я не хочу этого знать!"
    # m "..."
    m "Nein, Dick!"
    "Ich falle darauf sicher nicht rein!"
    "Es interessiert mich nicht, wo du wohnst. Ist mir egal!"
    m "..."

# game/Quests/Dick_Car_Parting.rpy:72
translate german dick_meeting1_car_parting2_3ecbc530:

    # m "И вообще, Дик!"
    # "Это случилось!"
    # dick "Что, Моника?"
    # m "Ты умудрился мне надоесть, в конце концов!"
    m "Dick, ehrlich gesagt..."
    "Ist es jetzt passiert!"
    dick "Was denn, Monica?"
    m "Du hast es geschafft mich endgültig zu langweilen!"

# game/Quests/Dick_Car_Parting.rpy:78
translate german dick_meeting1_car_parting2_40c829b5:

    # m "Фред!"
    m "Fred!"

# game/Quests/Dick_Car_Parting.rpy:80
translate german dick_meeting1_car_parting2_dd053b7b:

    # fred "Да, Мэм?"
    fred "Ja?"

# game/Quests/Dick_Car_Parting.rpy:82
translate german dick_meeting1_car_parting2_2ab87f62:

    # m "Фред, быстро останови машину!"
    # "Мистеру Дику надо срочно выйти!"
    m "Fred, halte sofort an!"
    "Dick möchte gerne jetzt aussteigen!"

# game/Quests/Dick_Car_Parting.rpy:85
translate german dick_meeting1_car_parting2_a95e18d2:

    # fred "Конечно, Мэм!"
    fred "Natürlich!"

# game/Quests/Dick_Car_Parting.rpy:93
translate german dick_meeting1_car_parting2_361bc315_1:

    # m "Дик, ты правда думаешь что я буду тратить время чтобы отвозить тебя домой?"
    m "Dick, denkst du wirklich, ich veschwende meine Zeit damit, dich nach Hause zu fahren?"

# game/Quests/Dick_Car_Parting.rpy:95
translate german dick_meeting1_car_parting2_6dc338c8_1:

    # dick "Дорогая, я бы тебе показал где я живу и..."
    dick "Schätzchen, ich zeige dir, wo ich wohne..."

# game/Quests/Dick_Car_Parting.rpy:97
translate german dick_meeting1_car_parting2_0407bbc7_1:

    # m "Нет, Дик!"
    # "Я не куплюсь на такую уловку!"
    # "Мне неинтересно где ты живешь! Я не хочу этого знать!"
    # m "..."
    m "Nein, Dick!"
    "Ich falle darauf sicher nicht rein!"
    "Es interessiert mich nicht, wo du wohnst. Ist mir egal!"
    m "..."

# game/Quests/Dick_Car_Parting.rpy:103
translate german dick_meeting1_car_parting2_6c70dc0e:

    # m "Хорошо. Я отвезу тебя к твоему офису."
    # "Тебя устроит это?"
    m "Gerne, ich fahre dich zum Büro."
    "Ist das ok für dich?"

# game/Quests/Dick_Car_Parting.rpy:107
translate german dick_meeting1_car_parting2_b7ced523:

    # m "Хорошо, дорогая! Меня это устроит!"
    m "Klar, Hübsche! Ist ok."

# game/Quests/Dick_Car_Parting.rpy:120
translate german dick_meeting1_car_parting3_ea6eb378:

    # dick "Мне очень жаль что все так вышло в этот вечер..."
    dick "Es tut mir sehr leid, dass an diesem Abend alles so abgeluafen ist, wie es ist."

# game/Quests/Dick_Car_Parting.rpy:123
translate german dick_meeting1_car_parting3_f4f7cde9:

    # m "Дик, ты испортил все с самого начала."
    m "Dick, du hast von Anfang an alles vermasselt."

# game/Quests/Dick_Car_Parting.rpy:125
translate german dick_meeting1_car_parting3_7ca7e903:

    # m "Нам повезло что мы как раз проезжали мимо той дыры, куда ты привез меня!"
    # "Вот и оставайся здесь! А я не хочу тебя видеть!"
    m "Wir haben Glück, wir sind gerade an dem Loch vorbei gefahren, zu dem du mich gebracht hast!"
    "Bleib hier! Ich möchte dich nicht mehr sehen!"

# game/Quests/Dick_Car_Parting.rpy:129
translate german dick_meeting1_car_parting3_476d51ba:

    # dick "Дорогая, но ты должна меня понять."
    # "Ведь я стараюсь."
    # "Если у тебя есть какие-то желания, я все сделаю для тебя."
    dick "Schätzchen, du musst mich verstehen."
    "Ich versuche es zumindest."
    "Wenn du irgendwelche Wünsche hast, werde ich sie dir von den Lippen ablesen."

# game/Quests/Dick_Car_Parting.rpy:134
translate german dick_meeting1_car_parting3_46fde947:

    # m "Ладно, Дик."
    # "Я подумаю о том что ты можешь для меня сделать еще."
    # "Но сейчас я поеду домой."
    # "А ты добирайся сам!"
    # "ПОНЯЛ??"
    # dick "Да, Моника."
    # "Я понял."
    # m "Тогда пока!"
    # dick "До встречи, Моника."
    # "Буду ждать."
    m "So, Dick."
    "Ich überlege mir was, das du für mich tun kannst."
    "Aber jetzt gehe ich nach Hause."
    "Und du gehst in dein eigenes zu Hause!"
    "VERSTANDEN??"
    dick "Ja, Monica,"
    "Verstanden."
    m "Dann gute Nacht!"
    dick "Man sieht sich, Monica."
    "Ich werde auf dich warten."

# game/Quests/Dick_Car_Parting.rpy:151
translate german dick_meeting1_car_parting4_fred_dialogue_2bd7df44:

    # m "Дик..."
    m "Dick..."

# game/Quests/Dick_Car_Parting.rpy:158
translate german dick_meeting1_car_parting4_fred_dialogue_6ffb30d5:

    # m "Вот ведь жирный урод!"
    # "Испортил мне весь вечер своим присутствием."
    m "Fetter Freak!"
    "Er hat mit seiner Anwesenheit den ganzen Abend verdorben."

# game/Quests/Dick_Car_Parting.rpy:162
translate german dick_meeting1_car_parting4_fred_dialogue_b7f423cb:

    # "Ты тоже так считаешь, Фред?"
    "Siehst du das auch so, Fred?"

# game/Quests/Dick_Car_Parting.rpy:165
translate german dick_meeting1_car_parting4_fred_dialogue_50693721:

    # fred "Но Мэм."
    # "На мой взгляд Вы ему нравитесь."
    # "Он старался Вам угодить."
    fred "Naja."
    "Ich denke, er mag Sie."
    "Und er hat versucht, Sie glücklich zu machen."

# game/Quests/Dick_Car_Parting.rpy:172
translate german dick_meeting1_car_parting4_fred_dialogue_3d2df59e:

    # m "Не спорь со мной, Фред!"
    # "А лучше заткнись!"
    m "Diskutiere nicht mit mir, Fred!"
    "Und jetzt halt die Klappe!"

# game/Quests/Dick_Car_Parting.rpy:177
translate german dick_meeting1_car_parting4_fred_dialogue_40bfe269:

    # m "Хорошо, Мэм."
    # "Конечно, Мэм."
    m "Ok."
    "Natürlich."

# game/Quests/Dick_Car_Parting.rpy:183
translate german dick_meeting1_car_parting4_fred_dialogue_880f6f6c:

    # m "Дурацкий вечер!"
    # "Этот Дик..."
    # "Ландо, хоть он и облажался, но он, похоже, старался угодить мне."
    # "Посмотрим, может с него еще будет какая-то польза..."
    # "Но то что ему он может никогда не рассчитывать на секс со мной - это сто процентов!"
    m "Dämliche Nacht!"
    "Dieser Dick..."
    "Hm, obwohl er alles versaut hat, hat er sich zumindest Mühe gegeben, mich zufrieden zu stellen."
    "Mal schauen, ob er immer noch nützlich ist..."
    "Allerdings steht es zu 100 Prozent fest, dass er niemals mit mir Sex haben wird!"

# game/Quests/Dick_Car_Parting.rpy:189
translate german dick_meeting1_car_parting4_fred_dialogue_40c829b5:

    # m "Фред!"
    m "Fred!"

# game/Quests/Dick_Car_Parting.rpy:191
translate german dick_meeting1_car_parting4_fred_dialogue_dd053b7b:

    # fred "Да, Мэм?"
    fred "Ja?"

# game/Quests/Dick_Car_Parting.rpy:193
translate german dick_meeting1_car_parting4_fred_dialogue_fe017777:

    # m "Долго ты будешь меня везти!?"
    m "Wie lange willst du mich noch rum fahren?"

# game/Quests/Dick_Car_Parting.rpy:195
translate german dick_meeting1_car_parting4_fred_dialogue_43802f23:

    # fred "Мы уж почти приехали, Мэм!"
    fred "Wir sind fast da!"

translate german strings:

    # game/Quests/Dick_Car_Parting.rpy:55
    old "Выбросить его прямо здесь..."
    new "Wirf es gleich hier hin..."

    # game/Quests/Dick_Car_Parting.rpy:55
    old "Отвезти к офису..."
    new "Geh in's Büro..."

    # game/Quests/Dick_Car_Parting.rpy:152
    old "Дик жирный урод!"
    new "Dick, dieser fette Bastard!"

    # game/Quests/Dick_Car_Parting.rpy:152
    old "Дик облажался, но он старался..."
    new "Dick hat es vermasselt, aber er hat sich wenigstens Mühe gegeben."

