# TODO: Translation updated at 2020-01-07 23:40

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:6
translate german ep210_dialogues7_escort_hotel_1_1115b3b3:

    # mt "Надеюсь, я не увижу этого извращенца Филиппа сегодня..."
    # mt "Это было ужасно!"
    # mt "Как он посмел так со мной обращаться?!"
    # mt "Сволочь!"
    # mt "Почему меня, такую добрую и красивую женщину, окружают одни подлецы?!"
    mt "Надеюсь, я не увижу этого извращенца Филиппа сегодня..."
    mt "Das war widerlich!"
    mt "Как он посмел так со мной обращаться?!"
    mt "Сволочь!"
    mt "Почему меня, такую добрую и красивую женщину, окружают одни подлецы?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:13
translate german ep210_dialogues7_escort_hotel_1_a7bd557d:

    # hotel_staff "Мэм... Добрый вечер, Мэм..."
    # m "Что?"
    # mt "Что могло понадобиться этому неудачнику от меня?!"
    hotel_staff "Мэм... Добрый вечер, Мэм..."
    m "Was?"
    mt "Что могло понадобиться этому неудачнику от меня?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:21
translate german ep210_dialogues7_escort_hotel_1_0a63eb3e:

    # mt "Я помню этого неудачника!"
    # mt "Это он тогда был в туалете, когда я..."
    # mt "Когда мерзавец Филипп заставил меня..."
    # mt "Делать всякие его извращенские гадости!"
    mt "Я помню этого неудачника!"
    mt "Это он тогда был в туалете, когда я..."
    mt "Когда мерзавец Филипп заставил меня..."
    mt "Делать всякие его извращенские гадости!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:27
translate german ep210_dialogues7_escort_hotel_1_c78569db:

    # hotel_staff "Один наш очень уважаемый клиент сказал мне, что..."
    # hotel_staff "..."
    # m "Что?"
    hotel_staff "Один наш очень уважаемый клиент сказал мне, что..."
    hotel_staff "..."
    m "Was?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:31
translate german ep210_dialogues7_escort_hotel_1_ec0340a9:

    # hotel_staff "Кхм... Что я могу договориться и..."
    # hotel_staff "Вам не нужно будет оплачивать Ваш ужин, Мэм."
    hotel_staff "Кхм... Что я могу договориться и..."
    hotel_staff "Вам не нужно будет оплачивать Ваш ужин, Мэм."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:34
translate german ep210_dialogues7_escort_hotel_1_47c904cd:

    # mt "???" 
    # m "Ужин за счет ресторана?"
    mt "???" 
    m "Ужин за счет ресторана?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:37
translate german ep210_dialogues7_escort_hotel_1_83e5051d:

    # mt "Может быть, кто-то из моих поклонников решил таким образом сделать мне приятное?"
    # mt "Тогда почему этот никчемный представитель обслуживающего персонала мне об этом сообщает?"
    # mt "А не администратор или, хотя бы, официант?"
    # mt "Странно это все..."
    # m "..."
    mt "Может быть, кто-то из моих поклонников решил таким образом сделать мне приятное?"
    mt "Тогда почему этот никчемный представитель обслуживающего персонала мне об этом сообщает?"
    mt "А не администратор или, хотя бы, официант?"
    mt "Странно это все..."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:43
translate german ep210_dialogues7_escort_hotel_1_1161f0d9:

    # hotel_staff "Так Вы позволите, Мэм?"
    # mt "Что позволю?"
    hotel_staff "Так Вы позволите, Мэм?"
    mt "Что позволю?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:47
translate german ep210_dialogues7_escort_hotel_1_950e9351:

    # hotel_staff "..."
    # hotel_staff "Мистер Филипп сказал мне, что если я... Кхм..."
    hotel_staff "..."
    hotel_staff "Мистер Филипп сказал мне, что если я... Кхм..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:50
translate german ep210_dialogues7_escort_hotel_1_0a5616b8:

    # mt "!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:52
translate german ep210_dialogues7_escort_hotel_1_0bcfcd23:

    # hotel_staff "Если я договорюсь, что Ваш ужин будет бесплатным для Вас, то..."
    # hotel_staff "Тогда вы пойдете со мной. И за 50 долларов проведете со мной время..."
    # hotel_staff "В каком-нибудь уединенном месте."
    # hotel_staff "Я знаю одно такое место, Мэм."
    hotel_staff "Если я договорюсь, что Ваш ужин будет бесплатным для Вас, то..."
    hotel_staff "Тогда вы пойдете со мной. И за 50 долларов проведете со мной время..."
    hotel_staff "В каком-нибудь уединенном месте."
    hotel_staff "Я знаю одно такое место, Мэм."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:58
translate german ep210_dialogues7_escort_hotel_1_f179d8e2:

    # m "ЧТО???"
    # m "Филипп?!"
    m "WAS?!?!"
    m "Филипп?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:61
translate german ep210_dialogues7_escort_hotel_1_527c91c9:

    # hotel_staff "Да, Мэм."
    # mt "!!!"
    hotel_staff "Ja."
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:67
translate german ep210_dialogues7_escort_hotel_1_99fb8dcb:

    # m "Я поверить не могу!"
    # m "Халдей, как ты смеешь предлагать мне подобное?!"
    # m "Как ты вообще посмел подойти к МОЕМУ столику?!"
    # m "И заговорить со МНОЙ?!"
    # m "!!!"
    m "Я поверить не могу!"
    m "Халдей, как ты смеешь предлагать мне подобное?!"
    m "Как ты вообще посмел подойти к МОЕМУ столику?!"
    m "И заговорить со МНОЙ?!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:73
translate german ep210_dialogues7_escort_hotel_1_89f9e800:

    # m "Никчемный неудачник!"
    # m "Еще слово - и я сделаю так. Что тебя сегодня же вышвырнут с этой работы!!!"
    # m "Пошел вон отсюда!!!"
    # m "!!!"
    m "Никчемный неудачник!"
    m "Еще слово - и я сделаю так. Что тебя сегодня же вышвырнут с этой работы!!!"
    m "Raus hier!!!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:83
translate german ep210_dialogues7_escort_hotel_1_c9d52248:

    # m "50 долларов?!"
    m "50 долларов?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:85
translate german ep210_dialogues7_escort_hotel_1_6807851d:

    # mt "Мало того, что этот мерзавец сказал этому неудачнику..."
    # mt "Что можно купить меня..."
    # mt "Так еще и за 50 долларов!!!"
    mt "Мало того, что этот мерзавец сказал этому неудачнику..."
    mt "Что можно купить меня..."
    mt "Так еще и за 50 долларов!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:89
translate german ep210_dialogues7_escort_hotel_1_56bb04d1:

    # m "!!!"
    # m "За 50 долларов ты можешь пойти и сам с собой провести время!"
    m "!!!"
    m "За 50 долларов ты можешь пойти и сам с собой провести время!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:92
translate german ep210_dialogues7_escort_hotel_1_bfcf2e42:

    # hotel_staff "Мэм, но ведь... Помимо этих 50 долларов..."
    # hotel_staff "Вам не нужно будет оплачивать Ваш чек."
    hotel_staff "Мэм, но ведь... Помимо этих 50 долларов..."
    hotel_staff "Вам не нужно будет оплачивать Ваш чек."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:95
translate german ep210_dialogues7_escort_hotel_1_8f36ea57:

    # m "И что?!"
    # m "Ужин я и сама могу оплатить!"
    m "И что?!"
    m "Ужин я и сама могу оплатить!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:98
translate german ep210_dialogues7_escort_hotel_1_a0b15db9:

    # mt "Хотя ужинать здесь чертовски дорого для меня сейчас..."
    mt "Хотя ужинать здесь чертовски дорого для меня сейчас..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:100
translate german ep210_dialogues7_escort_hotel_1_7938a00f:

    # hotel_staff "Я готов дать вам 55 долларов, Мэм."
    # hotel_staff "Извините, но у меня просто больше нет денег."
    # hotel_staff "55 долларов и бесплатный ужин."
    hotel_staff "Я готов дать вам 55 долларов, Мэм."
    hotel_staff "Извините, но у меня просто больше нет денег."
    hotel_staff "55 долларов и бесплатный ужин."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:105
translate german ep210_dialogues7_escort_hotel_1_24cfda66:

    # mt "Это все отвратительно и мерзко!"
    # mt "С другой стороны... 55 долларов... Они не будут лишними."
    # mt "А если кто-нибудь узнает?!"
    mt "Это все отвратительно и мерзко!"
    mt "С другой стороны... 55 долларов... Они не будут лишними."
    mt "А если кто-нибудь узнает?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:109
translate german ep210_dialogues7_escort_hotel_1_be49c468:

    # m "..."
    # m "Я не могу согласиться на такое..."
    m "..."
    m "Я не могу согласиться на такое..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:112
translate german ep210_dialogues7_escort_hotel_1_bff2ac65:

    # hotel_staff "Мэм, Вы можете не переживать. Об этом никто не узнает."
    hotel_staff "Мэм, Вы можете не переживать. Об этом никто не узнает."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:115
translate german ep210_dialogues7_escort_hotel_1_4bb7ded8:

    # mt "Черт! Мне действительно нужны эти деньги..."
    # mt "Я могу пригрозить этому неудачнику увольнением, если он кому-то расскажет..."
    # mt "Тогда он испугается и об этом никто никогда не узнает."
    # mt "Зато у меня прибавится 55 долларов. Плюс бесплатный ужин."
    # mt "И я не собираюсь делать ничего из такого, что требовал Филипп от меня..."
    # mt "..."
    mt "Черт! Мне действительно нужны эти деньги..."
    mt "Я могу пригрозить этому неудачнику увольнением, если он кому-то расскажет..."
    mt "Тогда он испугается и об этом никто никогда не узнает."
    mt "Зато у меня прибавится 55 долларов. Плюс бесплатный ужин."
    mt "И я не собираюсь делать ничего из такого, что требовал Филипп от меня..."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:124
translate german ep210_dialogues7_escort_hotel_1_2eb937fa:

    # m "Ты пожалеешь, если расскажешь об этом хоть одной живой душе!"
    # m "Я сделаю так, что тебя выкинут с работы!"
    # m "И ты после этого не сможешь устроиться на работу даже..."
    m "Ты пожалеешь, если расскажешь об этом хоть одной живой душе!"
    m "Я сделаю так, что тебя выкинут с работы!"
    m "И ты после этого не сможешь устроиться на работу даже..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:130
translate german ep210_dialogues7_escort_hotel_1_aee8074b:

    # m "Даже посудомойщиком в какой-нибудь грязной дыре в трущобах!"
    m "Даже посудомойщиком в какой-нибудь грязной дыре в трущобах!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:133
translate german ep210_dialogues7_escort_hotel_1_0040eca6:

    # m "Думаю, ты догадываешься о том..."
    # m "Что у меня много знакомых, которые легко смогут это устроить..."
    # m "Я большой и влиятельный человек в этом городе!"
    m "Думаю, ты догадываешься о том..."
    m "Что у меня много знакомых, которые легко смогут это устроить..."
    m "Я большой и влиятельный человек в этом городе!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:137
translate german ep210_dialogues7_escort_hotel_1_a44e7980:

    # hotel_staff "Конечно, Мэм! Я никому не скажу, Мэм!"
    # hotel_staff "Сотрудникам отеля запрещено пользоваться услугами проституток. За это могут уволить..."
    hotel_staff "Конечно, Мэм! Я никому не скажу, Мэм!"
    hotel_staff "Сотрудникам отеля запрещено пользоваться услугами проституток. За это могут уволить..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:141
translate german ep210_dialogues7_escort_hotel_1_007d0ff0:

    # m "Проституток?!"
    # m "!!!"
    # m "По-твоему, я проститутка?!"
    m "Проституток?!"
    m "!!!"
    m "По-твоему, я проститутка?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:146
translate german ep210_dialogues7_escort_hotel_1_1c6fd7a3:

    # hotel_staff "Нет, Мэм! Конечно, нет!!!"
    # hotel_staff "Просто..."
    # hotel_staff "Никто из руководства разбираться не будет."
    # hotel_staff "Меня просто уволят и все."
    hotel_staff "Нет, Мэм! Конечно, нет!!!"
    hotel_staff "Просто..."
    hotel_staff "Никто из руководства разбираться не будет."
    hotel_staff "Меня просто уволят и все."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:152
translate german ep210_dialogues7_escort_hotel_1_e583c2ae:

    # m "Что за уединенное место?"
    # m "..."
    # m "Если это какой-то туалет, то даже не думай!"
    # hotel_staff "Мэм, я взял ключи от пустого роскошного номера..."
    # hotel_staff "Мы можем пройти туда незаметно. Нас никто не увидит..."
    m "Что за уединенное место?"
    m "..."
    m "Если это какой-то туалет, то даже не думай!"
    hotel_staff "Мэм, я взял ключи от пустого роскошного номера..."
    hotel_staff "Мы можем пройти туда незаметно. Нас никто не увидит..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:160
translate german ep210_dialogues7_escort_hotel_1_6fe45f1a:

    # mt "Хм... В таком случае..."
    # mt "Минимален риск того, что меня кто-нибудь увидит и..."
    # mt "Догадается, что я оказала услугу за деньги этому неудачнику."
    # m "..."
    mt "Хм... В таком случае..."
    mt "Минимален риск того, что меня кто-нибудь увидит и..."
    mt "Догадается, что я оказала услугу за деньги этому неудачнику."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:165
translate german ep210_dialogues7_escort_hotel_1_be22e9b7:

    # hotel_staff "..."
    # hotel_staff "Мэм? Могу я Вас пригласить в этот номер?"
    hotel_staff "..."
    hotel_staff "Мэм? Могу я Вас пригласить в этот номер?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:169
translate german ep210_dialogues7_escort_hotel_1_9d6d6e44:

    # m "Я тебя предупредила, если кто-нибудь узнает..."
    # m "!!!"
    m "Я тебя предупредила, если кто-нибудь узнает..."
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:172
translate german ep210_dialogues7_escort_hotel_1_84c1130f:

    # hotel_staff "Не переживайте, Мэм. Об этом никто никогда не узнает."
    # hotel_staff "Пойдемте, Мэм?"
    hotel_staff "Не переживайте, Мэм. Об этом никто никогда не узнает."
    hotel_staff "Пойдемте, Мэм?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:181
translate german ep210_dialogues7_escort_hotel_2_67f2fac4:

    # m "Это и есть твое уединенное место?!"
    # m "Это что вообще?! Служебный коридор?!"
    m "Это и есть твое уединенное место?!"
    m "Это что вообще?! Служебный коридор?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:184
translate german ep210_dialogues7_escort_hotel_2_e6618c21:

    # m "!!!"
    # m "Ты же говорил, что у тебя есть ключи от номера..."
    m "!!!"
    m "Ты же говорил, что у тебя есть ключи от номера..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:188
translate german ep210_dialogues7_escort_hotel_2_d9d8bc80:

    # hotel_staff "Это у мистера Филиппа есть деньги на номер в отеле."
    # hotel_staff "Он водит в специальные номера всех девушек, которые стоят больше $ 100."
    # hotel_staff "А я всего лишь служащий здесь и у меня нет столько денег."
    hotel_staff "Это у мистера Филиппа есть деньги на номер в отеле."
    hotel_staff "Он водит в специальные номера всех девушек, которые стоят больше $ 100."
    hotel_staff "А я всего лишь служащий здесь и у меня нет столько денег."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:192
translate german ep210_dialogues7_escort_hotel_2_440c3053:

    # hotel_staff "Если бы я сразу сказал, что это обычный корридор..."
    # hotel_staff "То Вы не согласились бы, Мэм."
    # hotel_staff "Однако, прошу Вас, поверьте! Здесь никто никогда не ходит!"
    hotel_staff "Если бы я сразу сказал, что это обычный корридор..."
    hotel_staff "То Вы не согласились бы, Мэм."
    hotel_staff "Однако, прошу Вас, поверьте! Здесь никто никогда не ходит!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:197
translate german ep210_dialogues7_escort_hotel_2_4dfdaca1:

    # mt "У этого мерзавца Филиппа совести и денег хватает только на грязный туалет!"
    # mt "Сволочь! Наговорил этому неудачнику непонятно что про меня!!!"
    # mt "..."
    mt "У этого мерзавца Филиппа совести и денег хватает только на грязный туалет!"
    mt "Сволочь! Наговорил этому неудачнику непонятно что про меня!!!"
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:201
translate german ep210_dialogues7_escort_hotel_2_78bf1641:

    # mt "Стоп... Он водит в номера только тех, кто стоит дороже $ 100???"
    # mt "Но Я... Вот сволочь!!!"
    # mt "Ненавижу его!"
    mt "Стоп... Он водит в номера только тех, кто стоит дороже $ 100???"
    mt "Но Я... Вот сволочь!!!"
    mt "Ich hasse ihn!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:205
translate german ep210_dialogues7_escort_hotel_2_5daa885b:

    # hotel_staff "Кхм..." 
    # hotel_staff "Мэм, мне очень хотелось бы, чтобы Вы..."
    # hotel_staff "Поработали своим ротиком..." 
    hotel_staff "Кхм..." 
    hotel_staff "Мэм, мне очень хотелось бы, чтобы Вы..."
    hotel_staff "Поработали своим ротиком..." 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:211
translate german ep210_dialogues7_escort_hotel_2_89d36db4:

    # hotel_staff "У Вас очень удобный для члена ротик..."
    hotel_staff "У Вас очень удобный для члена ротик..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:215
translate german ep210_dialogues7_escort_hotel_2_2b7ddd6e:

    # hotel_staff "Мне в прошлый раз очень понравилось..."
    # hotel_staff "И мне хочется повторить..."
    hotel_staff "Мне в прошлый раз очень понравилось..."
    hotel_staff "И мне хочется повторить..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:220
translate german ep210_dialogues7_escort_hotel_2_89d36db4_1:

    # hotel_staff "У Вас очень удобный для члена ротик..."
    hotel_staff "У Вас очень удобный для члена ротик..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:226
translate german ep210_dialogues7_escort_hotel_2_674db6ec:

    # m "Что?!"
    # m "Да как ты смеешь так со мной говорить?!"
    # m "!!!"
    m "Was?!"
    m "Да как ты смеешь так со мной говорить?!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:230
translate german ep210_dialogues7_escort_hotel_2_97de3cbd:

    # hotel_staff "Но, Мэм..."
    # hotel_staff "Так сказал мистер Филипп. А еще он сказал, что Вы согласитесь..."
    # hotel_staff "Я отказался от Вашего предложения в прошлый раз..."
    hotel_staff "Но, Мэм..."
    hotel_staff "Так сказал мистер Филипп. А еще он сказал, что Вы согласитесь..."
    hotel_staff "Я отказался от Вашего предложения в прошлый раз..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:234
translate german ep210_dialogues7_escort_hotel_2_9cfd9e2f:

    # hotel_staff "Но потом жалел об этом. Я... Я передумал."
    # hotel_staff "И хочу, чтобы Вы сделали это, Мэм..."
    # mt "Этот неудачник Филипп еще пожалеет об этом!"
    hotel_staff "Но потом жалел об этом. Я... Я передумал."
    hotel_staff "И хочу, чтобы Вы сделали это, Мэм..."
    mt "Этот неудачник Филипп еще пожалеет об этом!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:239
translate german ep210_dialogues7_escort_hotel_2_0a5616b8:

    # mt "!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:244
translate german ep210_dialogues7_escort_hotel_2_2e3c6062:

    # m "Как ты смеешь предлагать мне подобное?!"
    # m "!!!"
    # m "Никчемный неудачник!"
    m "Как ты смеешь предлагать мне подобное?!"
    m "!!!"
    m "Никчемный неудачник!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:248
translate german ep210_dialogues7_escort_hotel_2_8a148dd1:

    # m "Оставь себе свои 55 долларов и развлекай себя сам!"
    # m "Я ни за что не стану делать этого!!!"
    # m "!!!"
    m "Оставь себе свои 55 долларов и развлекай себя сам!"
    m "Я ни за что не стану делать этого!!!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:257
translate german ep210_dialogues7_escort_hotel_2_515ce2b6:

    # hotel_staff "Мэм, простите, что я говорю что-то не так..."
    # hotel_staff "Просто я очень... Стесняюсь... И никак не хотел Вас обидеть."
    # hotel_staff "..."
    hotel_staff "Мэм, простите, что я говорю что-то не так..."
    hotel_staff "Просто я очень... Стесняюсь... И никак не хотел Вас обидеть."
    hotel_staff "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:261
translate german ep210_dialogues7_escort_hotel_2_4a1c5312:

    # m "Ну и отлично! Тогда я пошла."
    m "Ну и отлично! Тогда я пошла."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:263
translate german ep210_dialogues7_escort_hotel_2_9d38bf90:

    # mt "Достаточно и того, что мне не нужно оплачивать чек."
    mt "Достаточно и того, что мне не нужно оплачивать чек."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:265
translate german ep210_dialogues7_escort_hotel_2_989b24dd:

    # hotel_staff "Мэм, в таком случае. Я вынужден буду сообщить охране, что Вы не оплатили свой чек."
    hotel_staff "Мэм, в таком случае. Я вынужден буду сообщить охране, что Вы не оплатили свой чек."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:268
translate german ep210_dialogues7_escort_hotel_2_a0164c57:

    # m "Никчемный сотрудник никчемного отеля!"
    m "Никчемный сотрудник никчемного отеля!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:270
translate german ep210_dialogues7_escort_hotel_2_457d2baf:

    # hotel_staff "Мэм, простите, но я жду, когда Вы начнете..."
    hotel_staff "Мэм, простите, но я жду, когда Вы начнете..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:272
translate german ep210_dialogues7_escort_hotel_2_ecdbdbe2:

    # mt "Черт! И зачем я только согласилась?!"
    mt "Черт! И зачем я только согласилась?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:275
translate german ep210_dialogues7_escort_hotel_2_657e00cf:

    # mt "Моника, неужели ты сейчас сделаешь это?!"
    # mt "Но ведь 55 долларов не будут лишними..."
    mt "Моника, неужели ты сейчас сделаешь это?!"
    mt "Но ведь 55 долларов не будут лишними..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:278
translate german ep210_dialogues7_escort_hotel_2_a2abfdb2:

    # mt "Да и я уже свыклась с мыслью, что мне не надо платить за счет в ресторане..."
    # mt "..."
    # mt "К черту! Все-равно об этом никто не узнает..."
    mt "Да и я уже свыклась с мыслью, что мне не надо платить за счет в ресторане..."
    mt "..."
    mt "К черту! Все-равно об этом никто не узнает..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:282
translate german ep210_dialogues7_escort_hotel_2_40b06156:

    # mt "А я в такой ситуации, когда приходится идти на жертвы..."
    # mt "Хотя я никогда не предполагала, что жертвой будет необходимость трогать член какого-то халдея..."
    # mt "Моими восхитительными губами... О БОЖЕ!!!"
    mt "А я в такой ситуации, когда приходится идти на жертвы..."
    mt "Хотя я никогда не предполагала, что жертвой будет необходимость трогать член какого-то халдея..."
    mt "Моими восхитительными губами... О БОЖЕ!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:286
translate german ep210_dialogues7_escort_hotel_2_e106f39d:

    # hotel_staff "Мэм... Простите... Я жду..."
    # hotel_staff "Мне надо скорее закончить, ведь меня ждут важные дела..."
    # hotel_staff "В отеле много гостей и мне надо предложить каждому отести его одежду в гардероб..."
    hotel_staff "Мэм... Простите... Я жду..."
    hotel_staff "Мне надо скорее закончить, ведь меня ждут важные дела..."
    hotel_staff "В отеле много гостей и мне надо предложить каждому отести его одежду в гардероб..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:290
translate german ep210_dialogues7_escort_hotel_2_7eba29db:

    # mt "Это... Важные дела?!"
    # mt "Он считает это важнее того, что перед ним стою Я?!"
    mt "Это... Важные дела?!"
    mt "Он считает это важнее того, что перед ним стою Я?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:293
translate german ep210_dialogues7_escort_hotel_2_82249b71:

    # hotel_staff "Мэм. Если Вы не решитесь, то я вынужден буду уйти."
    # hotel_staff "А Вы - платить за счет в ресторане..."
    # mt "!!!"
    hotel_staff "Мэм. Если Вы не решитесь, то я вынужден буду уйти."
    hotel_staff "А Вы - платить за счет в ресторане..."
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:298
translate german ep210_dialogues7_escort_hotel_2_be37b652:

    # hotel_staff "ОООООО!!!"
    hotel_staff "OOOOOH!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:300
translate german ep210_dialogues7_escort_hotel_2_87537511:

    # hotel_staff "Какой каааайф!"
    hotel_staff "Какой каааайф!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:302
translate german ep210_dialogues7_escort_hotel_2_b76f5bfe:

    # hotel_staff "ААААААХ! О, Мэм, сделайте так еще раз!"
    hotel_staff "ААААААХ! О, Мэм, сделайте так еще раз!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:304
translate german ep210_dialogues7_escort_hotel_2_f7b76f1a:

    # hotel_staff "Да, еще-еще!"
    hotel_staff "Да, еще-еще!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:306
translate german ep210_dialogues7_escort_hotel_2_18dba444:

    # hotel_staff "МММММММ!"
    hotel_staff "МММММММ!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:309
translate german ep210_dialogues7_escort_hotel_2_d16c8ee7:

    # reception "Ага!!! Я так и знала!!!"
    # reception "Тебе нельзя пользоваться услугами нелегальных шлюх!"
    reception "Ага!!! Я так и знала!!!"
    reception "Тебе нельзя пользоваться услугами нелегальных шлюх!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:312
translate german ep210_dialogues7_escort_hotel_2_daf05117:

    # hotel_staff "А... Что... Нет-нет!"
    # hotel_staff "Я не..."
    hotel_staff "А... Что... Нет-нет!"
    hotel_staff "Я не..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:318
translate german ep210_dialogues7_escort_hotel_2_9ed146f3:

    # reception "И кто посмел оказывать этому халдею такие услуги?!"
    # reception "Ага! Я так и знала, что это ты, шлюха!"
    reception "И кто посмел оказывать этому халдею такие услуги?!"
    reception "Ага! Я так и знала, что это ты, шлюха!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:321
translate german ep210_dialogues7_escort_hotel_2_c7ea3606:

    # reception "Еще раз тут появишься - сдам тебя полиции!!!"
    # reception "А сейчас быстро пошла отсюда, пока я не позвала охрану!"
    reception "Еще раз тут появишься - сдам тебя полиции!!!"
    reception "А сейчас быстро пошла отсюда, пока я не позвала охрану!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:325
translate german ep210_dialogues7_escort_hotel_2_0a5616b8_1:

    # mt "!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:331
translate german ep210_dialogues7_escort_hotel_3_56a9a62e:

    # mt "Какой кошмар!!!"
    # mt "Моника, ты делала минет какому-то служащему отеля!"
    # mt "Неуверенному в себе неудачнику!"
    # mt "И этот никчемный администратор все увидела!!!"
    # mt "!!!"
    # mt "Что же мне теперь делать?!"
    # mt "Она подумала, что я проститутка! Но ведь это не так! (хмык)"
    # mt "..."
    # mt "С другой стороны..."
    # mt "Какая мне разница, что думает какая-то там никчемная администраторша?"
    # mt "Бесполезная!"
    # mt "Ее мнение для меня ничего не значит!"
    mt "Was für ein Alptraum!!!"
    mt "Моника, ты делала минет какому-то служащему отеля!"
    mt "Неуверенному в себе неудачнику!"
    mt "И этот никчемный администратор все увидела!!!"
    mt "!!!"
    mt "Was mache ich jetzt?"
    mt "Она подумала, что я проститутка! Но ведь это не так! (хмык)"
    mt "..."
    mt "Andererseits..."
    mt "Какая мне разница, что думает какая-то там никчемная администраторша?"
    mt "Бесполезная!"
    mt "Ее мнение для меня ничего не значит!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:348
translate german ep210_dialogues7_escort_hotel_4_efcc141d:

    # mt "Никчемный сотрудник никчемного отеля!"
    # mt "Как он мог предлагать мне такую мерзость?!"
    # mt "Неужели он думал, что я, Моника Бакфетт, соглашусь на подобное?! Фи!"
    mt "Никчемный сотрудник никчемного отеля!"
    mt "Как он мог предлагать мне такую мерзость?!"
    mt "Неужели он думал, что я, Моника Бакфетт, соглашусь на подобное?! Фи!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:356
translate german ep210_dialogues7_escort_hotel_5_a025b5b3:

    # mt "Я не хочу больше возвращаться в этот никчемный ресторан!"
    # mt "По крайней мере, не сегодня..."
    mt "Я не хочу больше возвращаться в этот никчемный ресторан!"
    mt "Zumindest für heute!..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:366
translate german ep210_dialogues7_escort_hotel_5_1_6b5fe713:

    # m "Да мне плевать, что ты там думаешь!"
    # m "Твое мнение для меня ничего не значит!"
    # m "Стоит тут какая-то ничегонезначащая низкоквалифицированная служащая!"
    m "Да мне плевать, что ты там думаешь!"
    m "Твое мнение для меня ничего не значит!"
    m "Стоит тут какая-то ничегонезначащая низкоквалифицированная служащая!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:370
translate german ep210_dialogues7_escort_hotel_5_1_3d919782:

    # m "Какого-то второсортного отеля!"
    # m "Никчемная! Бесполезная!"
    # m "И пытается строить из себя тут главную! Это смешно!"
    m "Какого-то второсортного отеля!"
    m "Никчемная! Бесполезная!"
    m "И пытается строить из себя тут главную! Это смешно!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:375
translate german ep210_dialogues7_escort_hotel_5_1_dbfacf71:

    # reception "Всегда рады видеть Вас в нашем отеле!" 
    # m "Сучка!!!"
    reception "Всегда рады видеть Вас в нашем отеле!" 
    m "Schlampe!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:388
translate german ep210_dialogues7_escort_hotel_6_c0d6bdbf:

    # mt "Мне нужно просто быстро пройти. Может, эта сучка меня не заметит."
    mt "Мне нужно просто быстро пройти. Может, эта сучка меня не заметит."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:391
translate german ep210_dialogues7_escort_hotel_6_0b252955:

    # reception "Это снова ты?! Иди отсюда!"
    # reception "Я знаю, что ты сюда пришла заниматься проституцией."
    # reception "Нелегально этого делать нельзя!"
    reception "Это снова ты?! Иди отсюда!"
    reception "Я знаю, что ты сюда пришла заниматься проституцией."
    reception "Нелегально этого делать нельзя!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:396
translate german ep210_dialogues7_escort_hotel_6_d844fa61:

    # m "Я пришла в ресторан, просто поужинать."
    # m "Что мне сделать, чтоб ты меня пропустила?"
    m "Я пришла в ресторан, просто поужинать."
    m "Что мне сделать, чтоб ты меня пропустила?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:399
translate german ep210_dialogues7_escort_hotel_6_7300c5af:

    # reception "Сначала нужно прекратить притворяться какой-то там леди."
    # reception "И сказать мне, кто ты на самом деле!"
    reception "Сначала нужно прекратить притворяться какой-то там леди."
    reception "И сказать мне, кто ты на самом деле!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:402
translate german ep210_dialogues7_escort_hotel_6_ca816f27:

    # m "Я на самом деле леди!"
    # m "Я одна из самых богатых и влиятельных женщин этого города!"
    m "Я на самом деле леди!"
    m "Я одна из самых богатых и влиятельных женщин этого города!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:406
translate german ep210_dialogues7_escort_hotel_6_0bad1949:

    # reception "Влиятельная женщина? Которая за деньги сосет член в коридоре у служащего отеля?"
    reception "Влиятельная женщина? Которая за деньги сосет член в коридоре у служащего отеля?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:408
translate german ep210_dialogues7_escort_hotel_6_0113b610:

    # mt "Черт!!!"
    # mt "!!!"
    # mt "Что же мне придумать?!"
    mt "Verdammt!!!"
    mt "!!!"
    mt "Что же мне придумать?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:413
translate german ep210_dialogues7_escort_hotel_6_dc0e42e7:

    # m "!!!"
    # m "Я делал это не за деньги! Это мои личные отношения! И это тебя не должно касаться!"
    m "!!!"
    m "Я делал это не за деньги! Это мои личные отношения! И это тебя не должно касаться!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:417
translate german ep210_dialogues7_escort_hotel_6_5d61a707:

    # reception "Да? А служащий признался, что ты делала это за деньги."
    # reception "Так что, про личные отношения расскажи кому-нибудь другому..."
    # mt "!!!"
    reception "Да? А служащий признался, что ты делала это за деньги."
    reception "Так что, про личные отношения расскажи кому-нибудь другому..."
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:421
translate german ep210_dialogues7_escort_hotel_6_2db8e934:

    # reception "Неужели ты думала, что я поверю в такую чушь?"
    # reception "Если хочешь заниматься этим здесь, то можешь это делать..."
    # reception "Но только легально."
    reception "Неужели ты думала, что я поверю в такую чушь?"
    reception "Если хочешь заниматься этим здесь, то можешь это делать..."
    reception "Но только легально."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:425
translate german ep210_dialogues7_escort_hotel_6_0a5616b8:

    # mt "!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:430
translate german ep210_dialogues7_escort_hotel_6_158bef3e:

    # mt "Мне надо притвориться, что мне это интересно."
    # mt "Иначе эта сучка не пустит меня в ресторан."
    mt "Мне надо притвориться, что мне это интересно."
    mt "Иначе эта сучка не пустит меня в ресторан."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:433
translate german ep210_dialogues7_escort_hotel_6_b0506657:

    # m "..."
    # m "Я поняла. Что ты хочешь от меня услышать?"
    m "..."
    m "Я поняла. Что ты хочешь от меня услышать?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:440
translate german ep210_dialogues7_escort_hotel_6_fbf0bcbb:

    # reception "Ты должна признаться, что ты шлюха."
    # reception "Которая ходит сюда, чтобы заниматься сексом за деньги."
    # reception "С уважаемыми посетителями нашего дорогого отеля."
    reception "Ты должна признаться, что ты шлюха."
    reception "Которая ходит сюда, чтобы заниматься сексом за деньги."
    reception "С уважаемыми посетителями нашего дорогого отеля."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:444
translate german ep210_dialogues7_escort_hotel_6_0a5616b8_1:

    # mt "!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:446
translate german ep210_dialogues7_escort_hotel_6_003329ff:

    # reception "Я могу понять тебя..."
    # mt "???"
    reception "Я могу понять тебя..."
    mt "???"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:449
translate german ep210_dialogues7_escort_hotel_6_b8565c81:

    # reception "Ты ходишь пешком, а значит обслуживаешь клиентов на улице."
    # reception "И естественно, что ты хочешь обслуживать более респектабельных и богатых клиентов."
    # reception "Которых можно найти только здесь, а не на улице."
    # reception "В принципе, по внешнему виду ты подходишь нам..."
    reception "Ты ходишь пешком, а значит обслуживаешь клиентов на улице."
    reception "И естественно, что ты хочешь обслуживать более респектабельных и богатых клиентов."
    reception "Которых можно найти только здесь, а не на улице."
    reception "В принципе, по внешнему виду ты подходишь нам..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:454
translate german ep210_dialogues7_escort_hotel_6_b37da0e3:

    # reception "Но для того, чтобы работать в нашем престижном месте, тебе нужно соблюдать правила."
    # reception "И делиться заработанными деньгами с администрацией."
    # reception "И не строить из себя какую-то леди!"
    reception "Но для того, чтобы работать в нашем престижном месте, тебе нужно соблюдать правила."
    reception "И делиться заработанными деньгами с администрацией."
    reception "И не строить из себя какую-то леди!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:458
translate german ep210_dialogues7_escort_hotel_6_b71ae042:

    # m "Но я леди..."
    # reception "Смотрю, ты снова за свое..."
    # reception "Можешь идти отсюда!"
    m "Но я леди..."
    reception "Смотрю, ты снова за свое..."
    reception "Можешь идти отсюда!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:464
translate german ep210_dialogues7_escort_hotel_6_8b0c5a25:

    # mt "Черт! Придется сказать этой сучке, что она требует!"
    mt "Черт! Придется сказать этой сучке, что она требует!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:466
translate german ep210_dialogues7_escort_hotel_6_ec3b14d3:

    # m "..."
    # m "Да... Я пришла, чтобы заработать денег..."
    # reception "И? Ты леди?"
    m "..."
    m "Да... Я пришла, чтобы заработать денег..."
    reception "И? Ты леди?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:470
translate german ep210_dialogues7_escort_hotel_6_88e90a24:

    # m "Нет..."
    m "Nein..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:472
translate german ep210_dialogues7_escort_hotel_6_89a6cf3e:

    # reception "А кто ты?"
    # mt "!!!"
    reception "А кто ты?"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:477
translate german ep210_dialogues7_escort_hotel_6_132b1416:

    # m "Да... Я... Я... Шлюха..."
    m "Да... Я... Я... Шлюха..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:479
translate german ep210_dialogues7_escort_hotel_6_1f23a40e:

    # reception "Ты признаешь, что врала, что просто ужинаешь в ресторане?"
    # reception "А сама пыталась заработать деньги..."
    # reception "Осуществляя незаконные услуги сексуального характера. Нелегально."
    # m "..."
    reception "Ты признаешь, что врала, что просто ужинаешь в ресторане?"
    reception "А сама пыталась заработать деньги..."
    reception "Осуществляя незаконные услуги сексуального характера. Нелегально."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:486
translate german ep210_dialogues7_escort_hotel_6_722e8148:

    # m "Да, признаю."
    m "Да, признаю."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:488
translate german ep210_dialogues7_escort_hotel_6_e1888b44:

    # reception "В тот раз... Когда ты пришла сюда с целью снять номер..."
    # reception "И врала что-то про потеряный багаж, про забытую сумочку в такси..."
    # reception "Твоя истинная цель была - продать свое тело кому-то из постояльцев отеля?"
    reception "В тот раз... Когда ты пришла сюда с целью снять номер..."
    reception "И врала что-то про потеряный багаж, про забытую сумочку в такси..."
    reception "Твоя истинная цель была - продать свое тело кому-то из постояльцев отеля?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:492
translate german ep210_dialogues7_escort_hotel_6_011f5b04:

    # m "Да..."
    m "Ja."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:494
translate german ep210_dialogues7_escort_hotel_6_c408c925:

    # reception "Что 'да'? Отвечай более развернуто!"
    reception "Что 'да'? Отвечай более развернуто!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:496
translate german ep210_dialogues7_escort_hotel_6_a6ab4abd:

    # mt "Ненавижу эту сучку!"
    mt "Ненавижу эту сучку!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:500
translate german ep210_dialogues7_escort_hotel_6_a508de27:

    # m "Да... Я пришла тогда, чтобы... Продать свое тело."
    m "Да... Я пришла тогда, чтобы... Продать свое тело."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:503
translate german ep210_dialogues7_escort_hotel_6_843d974a:

    # reception "Я наконец-то вывела тебя на чистую воду!"
    # reception "Отныне, если ты хочешь посещать этот отель..."
    # reception "Ты можешь это делать только в качестве девушки по вызову."
    reception "Я наконец-то вывела тебя на чистую воду!"
    reception "Отныне, если ты хочешь посещать этот отель..."
    reception "Ты можешь это делать только в качестве девушки по вызову."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:507
translate german ep210_dialogues7_escort_hotel_6_492d2137:

    # reception "Которая официально работает на администрацию отеля."
    # reception "Ты же об этом меня хочешь попросить?"
    reception "Которая официально работает на администрацию отеля."
    reception "Ты же об этом меня хочешь попросить?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:510
translate german ep210_dialogues7_escort_hotel_6_e612037c:

    # m "..."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:513
translate german ep210_dialogues7_escort_hotel_6_b75bd41c:

    # m "Да, я хочу устроиться сюда на работу."
    m "Да, я хочу устроиться сюда на работу."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:516
translate german ep210_dialogues7_escort_hotel_6_83fc57b3:

    # reception "Наши клиенты очень респектабельны... Поэтому наши девочки должны быть высшего качества."
    # reception "Я должна быть уверена, что ты подходишь."
    # reception "Надеюсь, у тебя нет проблем с тем, чтобы доказать свою пригодность?"
    reception "Наши клиенты очень респектабельны... Поэтому наши девочки должны быть высшего качества."
    reception "Я должна быть уверена, что ты подходишь."
    reception "Надеюсь, у тебя нет проблем с тем, чтобы доказать свою пригодность?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:520
translate german ep210_dialogues7_escort_hotel_6_dbfede7e:

    # m "..."
    # m "Я готова доказать свою пригодность к этой работе..."
    m "..."
    m "Я готова доказать свою пригодность к этой работе..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:523
translate german ep210_dialogues7_escort_hotel_6_4262ea96:

    # reception "Пошли за мной."
    reception "Пошли за мной."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:526
translate german ep210_dialogues7_escort_hotel_6_8f697fb4:

    # mt "Какой кошмар!!!"
    # mt "Мне предложили работать шлюхой в отеле!"
    # mt "Моника, до чего ты докатилась?!"
    # mt "..."
    mt "Was für ein Alptraum!!!"
    mt "Мне предложили работать шлюхой в отеле!"
    mt "Моника, до чего ты докатилась?!"
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:531
translate german ep210_dialogues7_escort_hotel_6_467dbf51:

    # mt "С другой стороны..."
    # mt "Это хорошая возможность быстро заработать неплохие деньги."
    # mt "Но черт! Неужели я смогу работать здесь?!"
    mt "Andererseits..."
    mt "Это хорошая возможность быстро заработать неплохие деньги."
    mt "Но черт! Неужели я смогу работать здесь?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:543
translate german ep210_dialogues7_escort_hotel_7_7f1c141a:

    # reception "У нас тут дружный коллектив."
    # reception "Поэтому все девочки должны одобрить твою кандидатуру."
    reception "У нас тут дружный коллектив."
    reception "Поэтому все девочки должны одобрить твою кандидатуру."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:546
translate german ep210_dialogues7_escort_hotel_7_ae808af9:

    # girl_1 "А, это она... Я тут раньше ее видела..." 
    girl_1 "А, это она... Я тут раньше ее видела..." 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:548
translate german ep210_dialogues7_escort_hotel_7_00542798:

    # girl_2 "Я так и знала, что она не просто посетитель, а работает здесь..." 
    girl_2 "Я так и знала, что она не просто посетитель, а работает здесь..." 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:550
translate german ep210_dialogues7_escort_hotel_7_28276371:

    # girl_3 "Она и правда тут работала?!" 
    girl_3 "Она и правда тут работала?!" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:553
translate german ep210_dialogues7_escort_hotel_7_aa7b7b06:

    # reception "Да, девочки. Я вывела ее на чистую воду."
    # reception "Она пыталась отобрать у вас клиентов."
    reception "Да, девочки. Я вывела ее на чистую воду."
    reception "Она пыталась отобрать у вас клиентов."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:556
translate german ep210_dialogues7_escort_hotel_7_f8bc3a1f:

    # reception "Теперь она хочет стать одной из вас."
    # reception "Подходит она или нет? Как вы считаете?"
    reception "Теперь она хочет стать одной из вас."
    reception "Подходит она или нет? Как вы считаете?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:560
translate german ep210_dialogues7_escort_hotel_7_5770dff7:

    # m "Эти девушки что, все работают здесь?"
    m "Эти девушки что, все работают здесь?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:562
translate german ep210_dialogues7_escort_hotel_7_1e4adf3c:

    # reception "Да. И работают они легально."
    # reception "В отличие от тебя!"
    reception "Да. И работают они легально."
    reception "В отличие от тебя!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:565
translate german ep210_dialogues7_escort_hotel_7_768ddf16:

    # mt "..."
    # mt "Черт! По-моему, они не очень дружелюбно настроены..."
    mt "..."
    mt "Черт! По-моему, они не очень дружелюбно настроены..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:569
translate german ep210_dialogues7_escort_hotel_7_e3818cc4:

    # girl_4 "Нас и так много..."
    # girl_4 "Зачем нам еще одна? Еще и такая?" 
    girl_4 "Нас и так много..."
    girl_4 "Зачем нам еще одна? Еще и такая?" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:572
translate german ep210_dialogues7_escort_hotel_7_dec36140:

    # girl_1 "Согласна. Она пыталась отнять у нас наш заработок!" 
    # girl_3 "Да ладно. Пусть разденется. Давайте посмотрим сначала на нее." 
    # girl_1 "Да. Вдруг у нее жирная задница?"
    girl_1 "Согласна. Она пыталась отнять у нас наш заработок!" 
    girl_3 "Да ладно. Пусть разденется. Давайте посмотрим сначала на нее." 
    girl_1 "Да. Вдруг у нее жирная задница?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:576
translate german ep210_dialogues7_escort_hotel_7_0cec6bc8:

    # mt "!!!"
    # mt "На свою задницу посмотрела бы!"
    # mt "Сучка!"
    mt "!!!"
    mt "На свою задницу посмотрела бы!"
    mt "Schlampe!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:580
translate german ep210_dialogues7_escort_hotel_7_d9ac91f4:

    # reception "Да. Давай, раздевайся!"
    # mt "???"
    reception "Да. Давай, раздевайся!"
    mt "???"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:586
translate german ep210_dialogues7_escort_hotel_7_9d1a784d:

    # m "Ты сейчас серьезно?!"
    # m "Я должна раздеться, чтобы эти..."
    # m "Эти девушки..."
    # m "Решали, подойду я для работы или нет?!"
    # m "!!!"
    m "Ты сейчас серьезно?!"
    m "Я должна раздеться, чтобы эти..."
    m "Эти девушки..."
    m "Решали, подойду я для работы или нет?!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:592
translate german ep210_dialogues7_escort_hotel_7_c72f05af:

    # m "Я!"
    # m "Не собираюсь!"
    # m "Раздеваться перед вами!"
    # m "К черту ваш второсортный отель и ваши правила!"
    m "Я!"
    m "Das werde ich nicht tun!"
    m "Раздеваться перед вами!"
    m "К черту ваш второсортный отель и ваши правила!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:597
translate german ep210_dialogues7_escort_hotel_7_99566fff:

    # m "В отличие от вас всех я умная..."
    # m "И легко найду другой способ заработать деньги!"
    m "В отличие от вас всех я умная..."
    m "И легко найду другой способ заработать деньги!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:601
translate german ep210_dialogues7_escort_hotel_7_5a5e1e47:

    # reception "Посмотрим, как много ты заработаешь..." 
    # reception "На нищих с улицы!"
    reception "Посмотрим, как много ты заработаешь..." 
    reception "На нищих с улицы!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:604
translate german ep210_dialogues7_escort_hotel_7_f16f7bf6:

    # reception "Когда в кармане не останется ни цента..."
    # reception "Ты знаешь, к кому обращаться." 
    # m "Сучка!!!"
    reception "Когда в кармане не останется ни цента..."
    reception "Ты знаешь, к кому обращаться." 
    m "Schlampe!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:616
translate german ep210_dialogues7_escort_hotel_7_172736e4:

    # mt "!!!"
    # mt "Эти сучки осматривают меня, как товар."
    # mt "Как это мерзко!"
    mt "!!!"
    mt "Эти сучки осматривают меня, как товар."
    mt "Как это мерзко!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:621
translate german ep210_dialogues7_escort_hotel_7_1a5ac263:

    # girl_2 "А она красивая..." 
    # girl_1 "Да ну! А вдруг у нее задница жирная?" 
    # girl_3 "А ну-ка повернись!" 
    girl_2 "А она красивая..." 
    girl_1 "Да ну! А вдруг у нее задница жирная?" 
    girl_3 "А ну-ка повернись!" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:626
translate german ep210_dialogues7_escort_hotel_7_fe57e446:

    # reception "Чего стоишь?! Поворачивайся!"
    # mt "!!!"
    reception "Чего стоишь?! Поворачивайся!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:632
translate german ep210_dialogues7_escort_hotel_7_1837972d:

    # girl_3 "Ну смотри, у нее отличная задница!"
    # girl_1 "Ну и что? Зачем нам еще одна нужна?"
    # girl_1 "Да, девочки? Зачем она нам?"
    # girl_2 "Ага..." 
    girl_3 "Ну смотри, у нее отличная задница!"
    girl_1 "Ну и что? Зачем нам еще одна нужна?"
    girl_1 "Да, девочки? Зачем она нам?"
    girl_2 "Ага..." 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:637
translate german ep210_dialogues7_escort_hotel_7_36ce00b3:

    # reception "Послушайте, вы всегда отказываетесь обслуживать проблемных клиентов..."
    # girl_3 "Пффф... С какой стати мы должны обслуживать всяких козлов?!"
    reception "Послушайте, вы всегда отказываетесь обслуживать проблемных клиентов..."
    girl_3 "Пффф... С какой стати мы должны обслуживать всяких козлов?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:640
translate german ep210_dialogues7_escort_hotel_7_7fdbf30b:

    # reception "А у нас приличное заведение с отличной репутацией."
    # reception "Ни один клиент не должен уйти неудовлетворенным, даже проблемный!"
    reception "А у нас приличное заведение с отличной репутацией."
    reception "Ни один клиент не должен уйти неудовлетворенным, даже проблемный!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:643
translate german ep210_dialogues7_escort_hotel_7_0688bb56:

    # girl_4 "Ты хочешь, чтобы она работала с проблемными? Не отбирала наших?" 
    girl_4 "Ты хочешь, чтобы она работала с проблемными? Не отбирала наших?" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:645
translate german ep210_dialogues7_escort_hotel_7_fb982f1b:

    # reception "Да, все верно."
    # mt "!!!"
    # reception "Она провинилась и ей нужно сначала заслужить наше доверие."
    # reception "Поэтому она будет работать с проблемными клиентами!"
    reception "Да, все верно."
    mt "!!!"
    reception "Она провинилась и ей нужно сначала заслужить наше доверие."
    reception "Поэтому она будет работать с проблемными клиентами!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:650
translate german ep210_dialogues7_escort_hotel_7_ae6a0778:

    # mt "О боже! Они хотят меня отправить ко всяким извращенцам!!!"
    # mt "С которыми сами не хотят работать!!!"
    # mt "!!!"
    mt "О боже! Они хотят меня отправить ко всяким извращенцам!!!"
    mt "С которыми сами не хотят работать!!!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:654
translate german ep210_dialogues7_escort_hotel_7_121af78d:

    # girl_4 "Думаю, это хорошая идея..."
    girl_4 "Думаю, это хорошая идея..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:656
translate german ep210_dialogues7_escort_hotel_7_2975d464:

    # girl_1 "Давно надо было так сделать!"
    # girl_3 "Да, я согласна с девочками! А ты что скажешь?" 
    girl_1 "Давно надо было так сделать!"
    girl_3 "Да, я согласна с девочками! А ты что скажешь?" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:659
translate german ep210_dialogues7_escort_hotel_7_6f43fe75:

    # girl_2 "Ага. Я тоже так считаю..."
    girl_2 "Ага. Я тоже так считаю..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:662
translate german ep210_dialogues7_escort_hotel_7_e0cab437:

    # reception "Можешь одеться."
    # m "..."
    reception "Можешь одеться."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:666
translate german ep210_dialogues7_escort_hotel_7_1fbea979:

    # reception "Ну что? Ты согласна работать с теми, кого мы тебе дадим?"
    # reception "У нас здесь строгие правила."
    # reception "Будешь зарабатывать 50 процентов от суммы, оплаченной клиентом."
    reception "Ну что? Ты согласна работать с теми, кого мы тебе дадим?"
    reception "У нас здесь строгие правила."
    reception "Будешь зарабатывать 50 процентов от суммы, оплаченной клиентом."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:670
translate german ep210_dialogues7_escort_hotel_7_ec6767d5:

    # mt "50 процентов? Всего?"
    # m "..."
    mt "50 процентов? Всего?"
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:673
translate german ep210_dialogues7_escort_hotel_7_29b0893b:

    # reception "Решение по вопросу чаевых принимается отдельно."
    # reception "Нельзя говорить 'нет' клиентам."
    # reception "Любая запрошенная услуга, если она есть в прайсе, должна быть оказана!"
    reception "Решение по вопросу чаевых принимается отдельно."
    reception "Нельзя говорить 'нет' клиентам."
    reception "Любая запрошенная услуга, если она есть в прайсе, должна быть оказана!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:677
translate german ep210_dialogues7_escort_hotel_7_ed44597c:

    # reception "А в нашем прайсе есть практически все!"
    # reception "За отказ или грубость к клиенту - предупреждение или увольнение."
    # reception "И еще у тебя нет права выбирать клиентов, как у других девушек."
    # mt "!!!"
    reception "А в нашем прайсе есть практически все!"
    reception "За отказ или грубость к клиенту - предупреждение или увольнение."
    reception "И еще у тебя нет права выбирать клиентов, как у других девушек."
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:682
translate german ep210_dialogues7_escort_hotel_7_00de58af:

    # reception "Тебе нужно просто сидеть за столиком. Клиент сам подойдет к тебе."
    # reception "А если поступит вызов, то тебя позовут."
    reception "Тебе нужно просто сидеть за столиком. Клиент сам подойдет к тебе."
    reception "А если поступит вызов, то тебя позовут."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:685
translate german ep210_dialogues7_escort_hotel_7_b42cbdf9:

    # reception "Если согласна на такие условия..."
    # reception "То завтра можешь приступать к работе."
    reception "Если согласна на такие условия..."
    reception "То завтра можешь приступать к работе."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:688
translate german ep210_dialogues7_escort_hotel_7_72c4389d:

    # mt "..."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:693
translate german ep210_dialogues7_escort_hotel_7_0e530d9a:

    # mt "Она сказала, что у меня будут проблемные клиенты..."
    # mt "Значит, будут одни извращенцы."
    mt "Она сказала, что у меня будут проблемные клиенты..."
    mt "Значит, будут одни извращенцы."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:696
translate german ep210_dialogues7_escort_hotel_7_84f19c0a:

    # mt "Неизвестно, чего мне от них ожидать..."
    # mt "Нет! Я не могу согласиться на такое!"
    mt "Неизвестно, чего мне от них ожидать..."
    mt "Нет! Я не могу согласиться на такое!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:699
translate german ep210_dialogues7_escort_hotel_7_5711f77c:

    # m "Я..."
    # m "Я не смогу..."
    # m "Мне нужно время, чтобы подумать..."
    m "Ich..."
    m "Я не смогу..."
    m "Мне нужно время, чтобы подумать..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:703
translate german ep210_dialogues7_escort_hotel_7_6ea237b5:

    # reception "Здесь у тебя будет неплохой заработок..."
    # reception "Если, конечно, будешь хорошо себя вести с клиентами."
    reception "Здесь у тебя будет неплохой заработок..."
    reception "Если, конечно, будешь хорошо себя вести с клиентами."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:706
translate german ep210_dialogues7_escort_hotel_7_671b513d:

    # reception "Подумай..."
    # reception "Ты знаешь, где меня найти."
    reception "Подумай..."
    reception "Ты знаешь, где меня найти."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:714
translate german ep210_dialogues7_escort_hotel_7_ce32695b:

    # mt "Она сказала, что у меня будут проблемные клиенты..."
    # mt "Значит, будут одни извращенцы."
    # mt "Неизвестно, чего мне от них ожидать..."
    # mt "..."
    mt "Она сказала, что у меня будут проблемные клиенты..."
    mt "Значит, будут одни извращенцы."
    mt "Неизвестно, чего мне от них ожидать..."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:719
translate german ep210_dialogues7_escort_hotel_7_d4d5fcca:

    # mt "С другой стороны, мне нужны деньги."
    # mt "А 50 процентов от суммы, которую платит здесь клиент - это немало."
    # mt "Мне ничего не мешает сейчас согласиться..."
    # mt "А если что-то пойдет не так с клиентом, я просто уйду отсюда и все."
    # mt "Мне нужно попытаться заработать хоть какие-то деньги..."
    mt "С другой стороны, мне нужны деньги."
    mt "А 50 процентов от суммы, которую платит здесь клиент - это немало."
    mt "Мне ничего не мешает сейчас согласиться..."
    mt "А если что-то пойдет не так с клиентом, я просто уйду отсюда и все."
    mt "Мне нужно попытаться заработать хоть какие-то деньги..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:726
translate german ep210_dialogues7_escort_hotel_7_d386a0a9:

    # m "Я..."
    # m "Согласна на любую работу."
    # m "Обещаю, что не буду отбирать у девочек их клиентов."
    m "Ich..."
    m "Согласна на любую работу."
    m "Обещаю, что не буду отбирать у девочек их клиентов."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:731
translate german ep210_dialogues7_escort_hotel_7_3c823767:

    # girl_1 "Да зачем она нам нужна?!"
    # girl_1 "Давай, не будем ее брать!"
    girl_1 "Да зачем она нам нужна?!"
    girl_1 "Давай, не будем ее брать!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:734
translate german ep210_dialogues7_escort_hotel_7_c8ca7036:

    # reception "Все! Решение принято, вопрос закрыт!" 
    # reception "Все, девочки! Расходимся! Пора работать!"
    reception "Все! Решение принято, вопрос закрыт!" 
    reception "Все, девочки! Расходимся! Пора работать!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:740
translate german ep210_dialogues7_escort_hotel_7_60ec4ec6:

    # girl_4 "Добро пожаловать в наш дружный коллектив." 
    # mt "!!!" 
    girl_4 "Добро пожаловать в наш дружный коллектив." 
    mt "!!!" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:743
translate german ep210_dialogues7_escort_hotel_7_43e5d226:

    # mt "Они тут все сучки! А эта - особенно."
    # m "..."
    mt "Они тут все сучки! А эта - особенно."
    m "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:746
translate german ep210_dialogues7_escort_hotel_7_142b1871:

    # reception "Да, совсем забыла... Как тебя зовут то?"
    reception "Да, совсем забыла... Как тебя зовут то?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:748
translate german ep210_dialogues7_escort_hotel_7_27538e90:

    # m "Меня зовут [monica_hotel_name]."
    m "Меня зовут [monica_hotel_name]."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:751
translate german ep210_dialogues7_escort_hotel_7_dc3437fd:

    # reception "Завтра жду тебя на работе, [monica_hotel_name]!"
    reception "Завтра жду тебя на работе, [monica_hotel_name]!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:759
translate german ep210_dialogues7_escort_hotel_7a_dde7c0aa:

    # mt "Кошмар!!!"
    # mt "Неужели я решусь на такое?! Это же проституция!"
    # mt "Смогу ли я обслуживать мужчин за деньги?!"
    # mt "Возможно, эти эскорт-услуги не предполагают под собой интим?"
    # mt "Может, мне просто нужно будет сопровождать богатых мужчин на каких-то мероприятиях?"
    # mt "Если так, то я вполне справлюсь с этим."
    # mt "..."
    # mt "А если потом нужно будет заниматься с ними сексом?!"
    # mt "!!!"
    # mt "Мне нужно будет прийти сюда завтра."
    # mt "Я ведь в любой момент смогу отказаться и уйти."
    mt "Кошмар!!!"
    mt "Неужели я решусь на такое?! Это же проституция!"
    mt "Смогу ли я обслуживать мужчин за деньги?!"
    mt "Возможно, эти эскорт-услуги не предполагают под собой интим?"
    mt "Может, мне просто нужно будет сопровождать богатых мужчин на каких-то мероприятиях?"
    mt "Если так, то я вполне справлюсь с этим."
    mt "..."
    mt "А если потом нужно будет заниматься с ними сексом?!"
    mt "!!!"
    mt "Мне нужно будет прийти сюда завтра."
    mt "Я ведь в любой момент смогу отказаться и уйти."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:775
translate german ep210_dialogues7_escort_hotel_7b_7bf16a7f:

    # mt "Это омерзительно!"
    # mt "Я никогда не смогу решиться на такую работу!"
    # mt "Я Моника Бакфетт, а не какая-то проститутка! Я леди!"
    mt "Это омерзительно!"
    mt "Я никогда не смогу решиться на такую работу!"
    mt "Я Моника Бакфетт, а не какая-то проститутка! Я леди!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:783
translate german ep210_dialogues7_escort_hotel_7c_7b526d94:

    # mt "Я не хочу больше возвращаться туда! Не сегодня."
    # mt "Администратор сказала приходить завтра."
    mt "Я не хочу больше возвращаться туда! Не сегодня."
    mt "Администратор сказала приходить завтра."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:790
translate german ep210_dialogues7_escort_hotel_7e_9ab743bd:

    # mt "Неужели я решусь на такое?!"
    # mt "Смогу ли я обслуживать мужчин за деньги?!"
    # mt "!!!"
    # mt "Я ведь в любой момент смогу отказаться и уйти..."
    mt "Неужели я решусь на такое?!"
    mt "Смогу ли я обслуживать мужчин за деньги?!"
    mt "!!!"
    mt "Я ведь в любой момент смогу отказаться и уйти..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:802
translate german ep210_dialogues7_escort_hotel_8_7cc5ec6f:

    # reception "Пришла?"
    # m "Да..."
    reception "Пришла?"
    m "Ja."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:805
translate german ep210_dialogues7_escort_hotel_8_e650a62a:

    # reception "Хорошо."
    # reception "В этой одежде чтобы я тебя на работе не видела!"
    # reception "В служебном коридоре для тебя приготовлено платье."
    reception "Na gut."
    reception "В этой одежде чтобы я тебя на работе не видела!"
    reception "В служебном коридоре для тебя приготовлено платье."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:809
translate german ep210_dialogues7_escort_hotel_8_b979bdc1:

    # reception "Работать в ресторане всегда только в этом платье."
    # reception "Переодевайся и иди за столик."
    reception "Работать в ресторане всегда только в этом платье."
    reception "Переодевайся и иди за столик."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:813
translate german ep210_dialogues7_escort_hotel_8_abdd0dba:

    # m "А мне... Мне прямо сегодня нужно будет..."
    # m "Провести встречу с клиентом?"
    m "А мне... Мне прямо сегодня нужно будет..."
    m "Провести встречу с клиентом?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:816
translate german ep210_dialogues7_escort_hotel_8_5d814a50:

    # reception "Да. Если ты пришла заработать денег, то иди и работай."
    # reception "Здесь не место разводить сопли."
    reception "Да. Если ты пришла заработать денег, то иди и работай."
    reception "Здесь не место разводить сопли."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:819
translate german ep210_dialogues7_escort_hotel_8_72c4389d:

    # mt "..."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:824
translate german ep210_dialogues7_escort_hotel_8_6e3708fd:

    # mt "Нет! Я не могу это сделать!"
    mt "Нет! Я не могу это сделать!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:826
translate german ep210_dialogues7_escort_hotel_8_5711f77c:

    # m "Я..."
    # m "Я не смогу..."
    # m "Мне нужно время, чтобы подумать..."
    m "Ich..."
    m "Я не смогу..."
    m "Мне нужно время, чтобы подумать..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:830
translate german ep210_dialogues7_escort_hotel_8_6ea237b5:

    # reception "Здесь у тебя будет неплохой заработок..."
    # reception "Если, конечно, будешь хорошо себя вести с клиентами."
    reception "Здесь у тебя будет неплохой заработок..."
    reception "Если, конечно, будешь хорошо себя вести с клиентами."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:833
translate german ep210_dialogues7_escort_hotel_8_c455f783:

    # reception "Подумай..."
    # reception "Ты всегда меня можешь найти здесь."
    reception "Подумай..."
    reception "Ты всегда меня можешь найти здесь."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:840
translate german ep210_dialogues7_escort_hotel_8_645c1930:

    # reception "Запомни, никаких глупостей!" 
    # reception "Клиент всегда прав. Ты не должна спорить с клиентом."
    # reception "Если отказываешь от чего-либо, что требует клиент..."
    reception "Запомни, никаких глупостей!" 
    reception "Клиент всегда прав. Ты не должна спорить с клиентом."
    reception "Если отказываешь от чего-либо, что требует клиент..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:844
translate german ep210_dialogues7_escort_hotel_8_87bbe345:

    # reception "То он вправе не заплатить тебе."
    # reception "Если клиент не заплатил тебе, значит он мог заплатить другой девушке..."
    # reception "У которой нет глупых предрассудков."
    reception "То он вправе не заплатить тебе."
    reception "Если клиент не заплатил тебе, значит он мог заплатить другой девушке..."
    reception "У которой нет глупых предрассудков."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:848
translate german ep210_dialogues7_escort_hotel_8_1336c0e7:

    # reception "Следовательно, это потеря денег для организации."
    # reception "Это неприемлемо! Тогда ты вылетишь отсюда назад на улицу!"
    reception "Следовательно, это потеря денег для организации."
    reception "Это неприемлемо! Тогда ты вылетишь отсюда назад на улицу!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:851
translate german ep210_dialogues7_escort_hotel_8_45fc422e:

    # mt "Я просто попробую..."
    # mt "Если что-то пойдет не так - уйду отсюда."
    # mt "..."
    mt "Я просто попробую..."
    mt "Если что-то пойдет не так - уйду отсюда."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:863
translate german ep210_dialogues7_escort_hotel_8a_7cc5ec6f:

    # reception "Пришла?"
    # m "Да..."
    reception "Пришла?"
    m "Ja."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:866
translate german ep210_dialogues7_escort_hotel_8a_eafd48e9:

    # reception "Хорошо."
    # reception "Переодевайся и иди за столик."
    reception "Na gut."
    reception "Переодевайся и иди за столик."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:869
translate german ep210_dialogues7_escort_hotel_8a_cbfab5d3:

    # mt "..."
    # reception "Запомни, никаких глупостей!" 
    mt "..."
    reception "Запомни, никаких глупостей!" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:872
translate german ep210_dialogues7_escort_hotel_8a_9c6deac0:

    # m "Да-да, я помню..."
    m "Да-да, я помню..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:874
translate german ep210_dialogues7_escort_hotel_8a_9f749cfa:

    # mt "Что-то пойдет не так - я просто уйду и все."
    mt "Что-то пойдет не так - я просто уйду и все."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:884
translate german ep210_dialogues7_escort_hotel_8a2_3d1a6460:

    # girl_1 "О, кто к нам пришел!"
    # girl_1 "Сегодня у тебя не получится уводить наших клиентов..."
    # girl_1 "Будешь работать с тем, кого мы тебе дадим!"
    # m "..."
    # mt "Сучка. Не хочу с ней разговаривать."
    girl_1 "О, кто к нам пришел!"
    girl_1 "Сегодня у тебя не получится уводить наших клиентов..."
    girl_1 "Будешь работать с тем, кого мы тебе дадим!"
    m "..."
    mt "Сучка. Не хочу с ней разговаривать."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:893
translate german ep210_dialogues7_escort_hotel_8b_89c2aea9:

    # girl_2 "Привет."
    # girl_2 "Надеюсь, мне сегодня подвернется нормальный клиент..."
    # m "Да... И мне тоже."
    # girl_2 "Ну, не знаю... Тебе то, что останется..."
    # mt "!!!"
    girl_2 "Hey."
    girl_2 "Надеюсь, мне сегодня подвернется нормальный клиент..."
    m "Да... И мне тоже."
    girl_2 "Ну, не знаю... Тебе то, что останется..."
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:902
translate german ep210_dialogues7_escort_hotel_8c_c5c1c040:

    # girl_3 "Я тебя где-то видела раньше."
    # girl_3 "Не в этом ресторане..."
    # girl_3 "Хммм... Не могу вспомнить, где..."
    # m "Наверное, ты меня с кем-то путаешь."
    # mt "Черт!"
    # mt "Она могла видеть меня на обложке журнала!"
    # mt "Или на фото репортеров с конференции!"
    # mt "Я не хочу, чтобы они узнали, что я Моника Бакфетт!"
    girl_3 "Я тебя где-то видела раньше."
    girl_3 "Не в этом ресторане..."
    girl_3 "Хммм... Не могу вспомнить, где..."
    m "Наверное, ты меня с кем-то путаешь."
    mt "Huch!"
    mt "Она могла видеть меня на обложке журнала!"
    mt "Или на фото репортеров с конференции!"
    mt "Я не хочу, чтобы они узнали, что я Моника Бакфетт!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:914
translate german ep210_dialogues7_escort_hotel_8d_f6c89520:

    # girl_4 "Пришла? Это хорошо."
    # girl_4 "Это значит, что у меня сегодня не будет придурков-клиентов."
    # girl_4 "Чего не скажешь о тебе..."
    # mt "Как она со мной разговаривает?!"
    # mt "Я поставлю эту сучку на место!"
    # mt "При первой же возможности!"
    # mt "!!!"
    girl_4 "Пришла? Это хорошо."
    girl_4 "Это значит, что у меня сегодня не будет придурков-клиентов."
    girl_4 "Чего не скажешь о тебе..."
    mt "Как она со мной разговаривает?!"
    mt "Я поставлю эту сучку на место!"
    mt "При первой же возможности!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:939
translate german ep210_dialogues7_escort_hotel_9_892e4101:

    # mt "Так, Моника. Соберись!"
    # mt "Не надо так нервничать."
    # mt "Ты в любой момент может послать всех к черту и уйти."
    # client "Добрый вечер, Мисс."
    # client "Я Вас раньше здесь не видел. Как вас зовут?"
    # m "Добрый вечер, Мистер."
    # m "Меня зовут [monica_hotel_name]."
    # client "[monica_hotel_name]. Какое прекрасное имя. Как и его обладательница."
    # client "Я очень рад, что сегодня проведу вечер с Вами, [monica_hotel_name]."
    # mt "Хмм... Разговаривает вежливо, выглядит вполне прилично."
    # mt "Думаю, от него не стоит ожидать никаких извращений..."
    # client "Позволите мне сразу пригласить Вас в номер, [monica_hotel_name]?"
    # mt "..."
    mt "Так, Моника. Соберись!"
    mt "Не надо так нервничать."
    mt "Ты в любой момент может послать всех к черту и уйти."
    client "Добрый вечер, Мисс."
    client "Я Вас раньше здесь не видел. Как вас зовут?"
    m "Добрый вечер, Мистер."
    m "Меня зовут [monica_hotel_name]."
    client "[monica_hotel_name]. Какое прекрасное имя. Как и его обладательница."
    client "Я очень рад, что сегодня проведу вечер с Вами, [monica_hotel_name]."
    mt "Хмм... Разговаривает вежливо, выглядит вполне прилично."
    mt "Думаю, от него не стоит ожидать никаких извращений..."
    client "Позволите мне сразу пригласить Вас в номер, [monica_hotel_name]?"
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:957
translate german ep210_dialogues7_escort_hotel_9_3d014ec3:

    # mt "Нет! Я не могу это сделать!"
    # m "Я..."
    # m "Я не смогу с Вами пойти..."
    # m "Я... Я не работаю здесь!"
    # m "Я просто зашла поесть в ресторан..."
    # client "О, извините, Мисс."
    # client "Наверное, я перепутал столик."
    mt "Нет! Я не могу это сделать!"
    m "Ich..."
    m "Я не смогу с Вами пойти..."
    m "Я... Я не работаю здесь!"
    m "Я просто зашла поесть в ресторан..."
    client "О, извините, Мисс."
    client "Наверное, я перепутал столик."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:968
translate german ep210_dialogues7_escort_hotel_9_aa4bd397:

    # mt "Это же совсем нестрашно."
    # mt "Это просто обычный неудачник."
    # mt "Наверное, приходит сюда раз в месяц в день зарплаты. Фи!"
    # mt "Что-то пойдет не так - я просто уйду отсюда."
    # m "Да, мы можем пойти в номер."
    mt "Это же совсем нестрашно."
    mt "Это просто обычный неудачник."
    mt "Наверное, приходит сюда раз в месяц в день зарплаты. Фи!"
    mt "Что-то пойдет не так - я просто уйду отсюда."
    m "Да, мы можем пойти в номер."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:978
translate german ep210_dialogues7_escort_hotel_9_1_43fccd5d:

    # client "Пройдемте в номер, [monica_hotel_name]."
    # mt "..."
    # mt "Это же совсем нестрашно."
    # mt "Это просто обычный неудачник."
    # mt "Наверное, приходит сюда раз в месяц в день зарплаты. Фи!"
    # mt "Что-то пойдет не так - я просто уйду отсюда."
    client "Пройдемте в номер, [monica_hotel_name]."
    mt "..."
    mt "Это же совсем нестрашно."
    mt "Это просто обычный неудачник."
    mt "Наверное, приходит сюда раз в месяц в день зарплаты. Фи!"
    mt "Что-то пойдет не так - я просто уйду отсюда."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:989
translate german ep210_dialogues7_escort_hotel_10_68939711:

    # m "Что мне нужно сделать?"
    # mt "Дал бы денег просто так и отпустил бы..."
    # mt "У меня нет никакого желания делать для него что-то."
    # client "Для начала [monica_hotel_name] должна снять платье..."
    # mt "Моника, не переживай."
    # mt "Тебе нечего стесняться. У тебя великолепное тело."
    # mt "..."
    # mt "Он, наверняка, никогда не видел такой красивой женщины, как я."
    # mt "Сейчас он разденется и начнет лапать мое прекрасное тело. Фу!"
    # client "[monica_hotel_name], присаживайтесь рядом со мной."
    # client "Если [monica_hotel_name] позволит мне целовать ее ножку..."
    # client "Я буду щедр с Мисс."
    # mt "Он хочет целовать мне ноги?"
    # mt "Да еще и платить за это?!"
    # mt "Он это серьезно?!"
    # mt "???"
    # mt "Может, я ослышалась?"
    # m "Мистер, Вы хотите целовать мою ногу?"
    # client "Да, Мисс. Позвольте Вашу ножку."
    # m "..."
    # mt "О, как же это..."
    # m "Ммммм..."
    # mt "Это приятно..."
    # mt "Черт!"
    # mt "Я готова работать так каждый вечер."
    # client "А теперь [monica_hotel_name] возьмет мой палец в свой ротик..."
    # client "И пососет его..."
    # mt "???"
    m "Что мне нужно сделать?"
    mt "Дал бы денег просто так и отпустил бы..."
    mt "У меня нет никакого желания делать для него что-то."
    client "Для начала [monica_hotel_name] должна снять платье..."
    mt "Моника, не переживай."
    mt "Тебе нечего стесняться. У тебя великолепное тело."
    mt "..."
    mt "Он, наверняка, никогда не видел такой красивой женщины, как я."
    mt "Сейчас он разденется и начнет лапать мое прекрасное тело. Фу!"
    client "[monica_hotel_name], присаживайтесь рядом со мной."
    client "Если [monica_hotel_name] позволит мне целовать ее ножку..."
    client "Я буду щедр с Мисс."
    mt "Он хочет целовать мне ноги?"
    mt "Да еще и платить за это?!"
    mt "Он это серьезно?!"
    mt "???"
    mt "Может, я ослышалась?"
    m "Мистер, Вы хотите целовать мою ногу?"
    client "Да, Мисс. Позвольте Вашу ножку."
    m "..."
    mt "О, как же это..."
    m "Mmmm..."
    mt "Это приятно..."
    mt "Huch!"
    mt "Я готова работать так каждый вечер."
    client "А теперь [monica_hotel_name] возьмет мой палец в свой ротик..."
    client "И пососет его..."
    mt "???"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1029
translate german ep210_dialogues7_escort_hotel_10_12fd811f:

    # mt "Фу! Нет! Я не могу это сделать!"
    # m "Я..."
    # m "Я не смогу..."
    # m "Я... Я не предоставляю такую услугу!"
    # client "Но, Мисс... Мне сказали, что..."
    # m "Нет. Я не буду этого делать!"
    mt "Фу! Нет! Я не могу это сделать!"
    m "Ich..."
    m "Я не смогу..."
    m "Я... Я не предоставляю такую услугу!"
    client "Но, Мисс... Мне сказали, что..."
    m "Нет. Я не буду этого делать!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1040
translate german ep210_dialogues7_escort_hotel_10_706f823a:

    # mt "Это унизительно и..."
    # mt "Противно!"
    # mt "Но он сказал, что будет щедр..."
    # mt "Все же это не какое-то там гадкое извращенство..."
    # mt "Главное, чтобы меня не стошнило прямо на него."
    # client "Соси его!"
    # client "Оооооо!!!"
    # client "О, дааааа!"
    # client "Как же приятно! Ещееее!!!"
    # client "ААААААА!!!"
    # mt "Фу! Это было мерзко!"
    # client "[monica_hotel_name] старалась и мне очень понравилось."
    # m "..."
    # client "Здесь 700 долларов. Я обязательно приду к Вам еще раз, [monica_hotel_name]." 
    # mt "700 долларов?!"
    # mt "Я заработала 700 долларов, облизывая палец незнакомому мужчине!"
    # mt "..."
    # mt "Меньше всего хочется отдавать половину этой сучке на ресепшене..."
    # mt "Может, уйти и больше просто не приходить сюда?"
    # mt "Что она мне сделает?"
    # mt "С другой стороны, здесь я за вечер могу заработать такие большие деньги."
    # mt "А она меня больше не пустит в ресторан."
    # mt "Нет, мне не стоит пока терять эту возможность заработка."
    mt "Это унизительно и..."
    mt "Противно!"
    mt "Но он сказал, что будет щедр..."
    mt "Все же это не какое-то там гадкое извращенство..."
    mt "Главное, чтобы меня не стошнило прямо на него."
    client "Соси его!"
    client "Оооооо!!!"
    client "О, дааааа!"
    client "Как же приятно! Ещееее!!!"
    client "ААААААА!!!"
    mt "Фу! Это было мерзко!"
    client "[monica_hotel_name] старалась и мне очень понравилось."
    m "..."
    client "Здесь 700 долларов. Я обязательно приду к Вам еще раз, [monica_hotel_name]." 
    mt "700 долларов?!"
    mt "Я заработала 700 долларов, облизывая палец незнакомому мужчине!"
    mt "..."
    mt "Меньше всего хочется отдавать половину этой сучке на ресепшене..."
    mt "Может, уйти и больше просто не приходить сюда?"
    mt "Что она мне сделает?"
    mt "С другой стороны, здесь я за вечер могу заработать такие большие деньги."
    mt "А она меня больше не пустит в ресторан."
    mt "Нет, мне не стоит пока терять эту возможность заработка."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1077
translate german ep210_dialogues7_escort_hotel_11_a2a20df3:

    # m "Клиент заплатил 700 долларов."
    # reception "И остался доволен."
    # reception "Ты хорошо поработала ртом."
    # mt "!!!"
    # m "Откуда ты?.."
    # reception "Я все знаю..."
    # reception "Это моя работа!"
    # reception "Вот твои деньги."
    # reception "Завтра можешь приходить снова."
    # reception "На сегодня все. Клиенты не любят использованный товар."
    # mt "Вот сучка!"
    # mt "Откуда она знает, что я... Работала ртом?!"
    # mt "Она подглядывала?!"
    # mt "!!!"
    m "Клиент заплатил 700 долларов."
    reception "И остался доволен."
    reception "Ты хорошо поработала ртом."
    mt "!!!"
    m "Откуда ты?.."
    reception "Ich weiß schon Bescheid..."
    reception "Das ist mein Job!"
    reception "Hier ist dein Geld."
    reception "Завтра можешь приходить снова."
    reception "На сегодня все. Клиенты не любят использованный товар."
    mt "Вот сучка!"
    mt "Откуда она знает, что я... Работала ртом?!"
    mt "Она подглядывала?!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1097
translate german ep210_dialogues7_escort_hotel_11_1_2a417654:

    # mt "Я пока не могу так рисковать!"
    # mt "Эта сучка меня не пустит больше в ресторан, если я не отдам ей 50 процентов."
    # mt "А мне не выгодно терять такой заработок."
    # mt "..."
    mt "Я пока не могу так рисковать!"
    mt "Эта сучка меня не пустит больше в ресторан, если я не отдам ей 50 процентов."
    mt "А мне не выгодно терять такой заработок."
    mt "..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1109
translate german ep210_dialogues7_escort_hotel_11a_1c313cdf:

    # mt "Я сегодня за вечер заработала 350 долларов."
    # mt "Неплохой заработок, учитывая мою временную ситуацию."
    # mt "Возможно, мне стоит пересилить себя и поработать здесь еще какое-то время."
    # mt "Быть может, у меня получится зарабатывать еще больше."
    # mt "Мне нужно будет прийти сюда завтра."
    # mt "..."
    # mt "Я ведь в любой момент смогу отказаться и уйти."
    mt "Я сегодня за вечер заработала 350 долларов."
    mt "Неплохой заработок, учитывая мою временную ситуацию."
    mt "Возможно, мне стоит пересилить себя и поработать здесь еще какое-то время."
    mt "Быть может, у меня получится зарабатывать еще больше."
    mt "Мне нужно будет прийти сюда завтра."
    mt "..."
    mt "Я ведь в любой момент смогу отказаться и уйти."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1121
translate german ep210_dialogues7_escort_hotel_11b_4ad016fe:

    # mt "Я никогда не смогу решиться на такое!"
    # mt "Я Моника Бакфетт, а не какая-то проститутка!"
    mt "Я никогда не смогу решиться на такое!"
    mt "Я Моника Бакфетт, а не какая-то проститутка!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1127
translate german ep210_dialogues7_escort_hotel_12_1_ab8fa91a:

    # mt "Надеюсь, сегодня я смогу заработать хоть что-нибудь..."
    # mt "Мне нужны деньги."
    # hotel_staff "М-мэм?"
    # mt "Никчемный неудачник!"
    # mt "Это от него администратор узнала, что я..."
    # mt "Была с ним за деньги!"
    # mt "Я ему это еще припомню!"
    # hotel_staff "С ресепшена просили передать, что у Вас клиент."
    # mt "И где же он?"
    # hotel_staff "Он Вас ждет по этому адресу."
    # mt "..."
    # mt "Хмм... Это кафе... "
    # mt "Мне предстоит сопровождать клиента на мероприятии?"
    # mt "???"
    # mt "Ну что ж... Если мне что-то не понравится, я просто уйду из кафе."
    mt "Надеюсь, сегодня я смогу заработать хоть что-нибудь..."
    mt "Мне нужны деньги."
    hotel_staff "М-мэм?"
    mt "Никчемный неудачник!"
    mt "Это от него администратор узнала, что я..."
    mt "Была с ним за деньги!"
    mt "Я ему это еще припомню!"
    hotel_staff "С ресепшена просили передать, что у Вас клиент."
    mt "И где же он?"
    hotel_staff "Он Вас ждет по этому адресу."
    mt "..."
    mt "Хмм... Это кафе... "
    mt "Мне предстоит сопровождать клиента на мероприятии?"
    mt "???"
    mt "Ну что ж... Если мне что-то не понравится, я просто уйду из кафе."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1151
translate german ep210_dialogues7_escort_hotel_12_60279d1c:

    # mt "И где мне искать этого неудачника?"
    # mt "???"
    # mt "Долго мне тут стоять?!"
    # client "Вы [monica_hotel_name]?"
    # client "Рад, что Вы приехали."
    # mt "Ну наконец-то..."
    # client "Я вперве обращаюсь к услугам девушки с эскорта." 
    # client "И немного переживаю, как все пройдет."
    # client "Дело в том, что у меня тут мероприятие в компании знакомых и коллег."
    # client "И мне нужна девушка, которая будет просто меня сопровождать..."
    # client "Сделаете вид, что мы с Вами давно знакомы..."
    # client "Пообщаетесь с ними, поужинаете и, в принципе, все."
    # m "И все?" 
    # m "На этом моя работа будет закончена?"
    # client "Да-да. Мне другие услуги, которые оказывает ваша компания, не нужны."
    # client "Только эскорт-сопровождение на ужине. И все."
    # mt "Моника, тебе сегодня повезло с клиентом..."
    # mt "Вкусный ужин, светские разговоры в компании приличных людей..."
    # mt "И никаких извращенцев."
    # mt "Все, к чему ты так привыкла... И чего так не хватает теперь."
    # mt "Похоже, что эта работа не так уж и плоха..."
    # mt "Нет, конечно это отвратительно, но..."
    # mt "Учитывая другие варианты заработать деньги, эскорт не так уж плох..."
    # client "[monica_hotel_name], если Вы не против, то я могу пригласить Вас к столу..."
    # m "Да, пройдемте к столу."
    # client "Ребята, минуту внимания."
    # client "Хочу представить вам [monica_hotel_name]."
    # client "Это моя девушка и мы с ней встречаемся."
    # client_friend_1 "Привет, [monica_hotel_name]!"
    # client_friend_2 "Рад знакомству, [monica_hotel_name]!"
    # m "Спасибо." 
    # client_friend_1 "[monica_hotel_name], расскажите, чем Вы занимаетесь?" 
    # m "Я являюсь руководителем в одной крупной компании."
    # m "Помимо основной деятельности, связанной с индустрией моды..."
    # m "У меня еще бизнес в сфере недвижимости."
    # client_friend_2 "О, серьезно?!" 
    # m "Да, у меня в подчинении много сотрудников."
    # client_friend_1 "Ваша деятельность связана с модной индустрией?"
    # client_friend_1 "Расскажите нам. Это очень интересно." 
    # m "По работе мне часто приходится посещать публичные мероприятия..."
    # m "Выступать перед публикой, давать интервью."
    # m "И, конечно же, я всегда в курсе всех событий в мире моды."
    # client "Да, [monica_hotel_name] очень занятая девушка." 
    # client "Редко такое бывает, что мы можем провести вечер вместе..."
    # client "Она по вечерам обычно ведет какие-нибудь переговоры..."
    # client "И важные бизнес-встречи."
    # client "Ведь такие встречи очень важны для развития ее бизнеса."
    # client "Да, [monica_hotel_name]?"
    # m "Все верно."
    # m "У меня широкий круг знакомых, которые имеют немалый вес в этом городе..."
    # m "И даже в стране."
    # m "И не в моих интересах отказывать им, когда меня приглашают на встречу."
    # client_friend_2 "И как у Вас хватает времени на что-то, кроме работы?" 
    # mt "Никчемные людишки..."
    # mt "Млеют при виде меня и от мысли том, кто я такая!!!"
    # m "Да, это довольно непросто..."
    # m "Грамотное планирование своего рабочего времени..."
    # m "А также помощь моих личных ассистентов..."
    # m "Позволяют мне иногда провести вечер не на работе, а в свое удовольствие."
    # client_friend_1 "Удивительно, что при столь плотном графике работы..." 
    # client_friend_1 "Вы, [monica_hotel_name], находите время на..."
    # client_friend_1 "Работу в эскорте!" 
    # m "Ч-что?"
    # m "???"
    # mt "Мне это послышалось?"
    # mt "Или он и правда сказал что-то про... Эскорт?!"
    # m "..."
    # m "Простите, Мистер, я не свосем понимаю, о чем Вы..."
    # client_friend_2 "Не надо путать ее с эскортницей."
    # client_friend_2 "Это обычная проститутка."
    # client_friend_2 "Ты же сам говорил, что купил ее на вечер за 900 баксов."
    # client "Аха-ха!!! Ты понял кто она такая, да?!"
    # client_friend_2 "Да, тебе снова не удалось провести нас."
    # client "Я думал, у меня получится в этот раз."
    # client "Эта проститутка очень похожа на приличную девушку."
    # client_friend_1 "Да ладно! Совсем не похожа!"
    # client_friend_1 "Она врет о больших делах, в которых видно, что ничего не понимает!"
    # mt "Что?! Это Я ничего не понимаю?!"
    # mt "!!!"
    # mt "Я не могу поверить!!!"
    # mt "Он это все... Он все подстроил что-ли?!"
    # mt "Они с начала вечера знали, что я... С эскорта..."
    # mt "!!!"
    # mt "Сволочь! Мерзавец!"
    # mt "АААААА!!!"
    # mt "НЕНАВИЖУ!!!"
    # client "А теперь тебе нужно отработать деньги."
    # client "Я плачу 900 баксов."
    # client "Залезь под стол и отсоси мне!"
    # mt "Долбанный неудачник!"
    # mt "!!!"
    mt "И где мне искать этого неудачника?"
    mt "???"
    mt "Долго мне тут стоять?!"
    client "Вы [monica_hotel_name]?"
    client "Рад, что Вы приехали."
    mt "Ну наконец-то..."
    client "Я вперве обращаюсь к услугам девушки с эскорта." 
    client "И немного переживаю, как все пройдет."
    client "Дело в том, что у меня тут мероприятие в компании знакомых и коллег."
    client "И мне нужна девушка, которая будет просто меня сопровождать..."
    client "Сделаете вид, что мы с Вами давно знакомы..."
    client "Пообщаетесь с ними, поужинаете и, в принципе, все."
    m "Das ist alles?" 
    m "На этом моя работа будет закончена?"
    client "Да-да. Мне другие услуги, которые оказывает ваша компания, не нужны."
    client "Только эскорт-сопровождение на ужине. И все."
    mt "Моника, тебе сегодня повезло с клиентом..."
    mt "Вкусный ужин, светские разговоры в компании приличных людей..."
    mt "И никаких извращенцев."
    mt "Все, к чему ты так привыкла... И чего так не хватает теперь."
    mt "Похоже, что эта работа не так уж и плоха..."
    mt "Нет, конечно это отвратительно, но..."
    mt "Учитывая другие варианты заработать деньги, эскорт не так уж плох..."
    client "[monica_hotel_name], если Вы не против, то я могу пригласить Вас к столу..."
    m "Да, пройдемте к столу."
    client "Ребята, минуту внимания."
    client "Хочу представить вам [monica_hotel_name]."
    client "Это моя девушка и мы с ней встречаемся."
    client_friend_1 "Привет, [monica_hotel_name]!"
    client_friend_2 "Рад знакомству, [monica_hotel_name]!"
    m "Danke." 
    client_friend_1 "[monica_hotel_name], расскажите, чем Вы занимаетесь?" 
    m "Я являюсь руководителем в одной крупной компании."
    m "Помимо основной деятельности, связанной с индустрией моды..."
    m "У меня еще бизнес в сфере недвижимости."
    client_friend_2 "О, серьезно?!" 
    m "Да, у меня в подчинении много сотрудников."
    client_friend_1 "Ваша деятельность связана с модной индустрией?"
    client_friend_1 "Расскажите нам. Это очень интересно." 
    m "По работе мне часто приходится посещать публичные мероприятия..."
    m "Выступать перед публикой, давать интервью."
    m "И, конечно же, я всегда в курсе всех событий в мире моды."
    client "Да, [monica_hotel_name] очень занятая девушка." 
    client "Редко такое бывает, что мы можем провести вечер вместе..."
    client "Она по вечерам обычно ведет какие-нибудь переговоры..."
    client "И важные бизнес-встречи."
    client "Ведь такие встречи очень важны для развития ее бизнеса."
    client "Да, [monica_hotel_name]?"
    m "Все верно."
    m "У меня широкий круг знакомых, которые имеют немалый вес в этом городе..."
    m "И даже в стране."
    m "И не в моих интересах отказывать им, когда меня приглашают на встречу."
    client_friend_2 "И как у Вас хватает времени на что-то, кроме работы?" 
    mt "Никчемные людишки..."
    mt "Млеют при виде меня и от мысли том, кто я такая!!!"
    m "Да, это довольно непросто..."
    m "Грамотное планирование своего рабочего времени..."
    m "А также помощь моих личных ассистентов..."
    m "Позволяют мне иногда провести вечер не на работе, а в свое удовольствие."
    client_friend_1 "Удивительно, что при столь плотном графике работы..." 
    client_friend_1 "Вы, [monica_hotel_name], находите время на..."
    client_friend_1 "Работу в эскорте!" 
    m "Ч-что?"
    m "???"
    mt "Мне это послышалось?"
    mt "Или он и правда сказал что-то про... Эскорт?!"
    m "..."
    m "Простите, Мистер, я не свосем понимаю, о чем Вы..."
    client_friend_2 "Не надо путать ее с эскортницей."
    client_friend_2 "Это обычная проститутка."
    client_friend_2 "Ты же сам говорил, что купил ее на вечер за 900 баксов."
    client "Аха-ха!!! Ты понял кто она такая, да?!"
    client_friend_2 "Да, тебе снова не удалось провести нас."
    client "Я думал, у меня получится в этот раз."
    client "Эта проститутка очень похожа на приличную девушку."
    client_friend_1 "Да ладно! Совсем не похожа!"
    client_friend_1 "Она врет о больших делах, в которых видно, что ничего не понимает!"
    mt "Что?! Это Я ничего не понимаю?!"
    mt "!!!"
    mt "Я не могу поверить!!!"
    mt "Он это все... Он все подстроил что-ли?!"
    mt "Они с начала вечера знали, что я... С эскорта..."
    mt "!!!"
    mt "Сволочь! Мерзавец!"
    mt "АААААА!!!"
    mt "ICH HASSE SIE!!!"
    client "А теперь тебе нужно отработать деньги."
    client "Я плачу 900 баксов."
    client "Залезь под стол и отсоси мне!"
    mt "Долбанный неудачник!"
    mt "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1258
translate german ep210_dialogues7_escort_hotel_12_0120cd0f:

    # m "Да пошел ты к черту!"
    # m "Вместе со своими друзьями!"
    # m "Сборище жалких неудачников!!!"
    # client "Воу... Полегче!"
    # client "А то останешься без денег!"
    # m "Засунь их себе в задницу!"
    # m "!!!"
    m "Да пошел ты к черту!"
    m "Вместе со своими друзьями!"
    m "Сборище жалких неудачников!!!"
    client "Воу... Полегче!"
    client "А то останешься без денег!"
    m "Засунь их себе в задницу!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1268
translate german ep210_dialogues7_escort_hotel_12_110c33c2:

    # m "Да пошел ты к черту!"
    # m "Вместе со своими друзьями!"
    # m "Сборище жалких неудачников!!!"
    # client "Воу... Полегче!"
    # client "А то останешься без денег!"
    # m "Засунь их себе в задницу!"
    # m "!!!"
    # m "Сволочь!"
    # m "!!!"
    # client "Сучка!!!"
    # m "Да пошел ты, придурок!"
    # m "!!!"
    m "Да пошел ты к черту!"
    m "Вместе со своими друзьями!"
    m "Сборище жалких неудачников!!!"
    client "Воу... Полегче!"
    client "А то останешься без денег!"
    m "Засунь их себе в задницу!"
    m "!!!"
    m "Сволочь!"
    m "!!!"
    client "Schlampe!"
    m "Да пошел ты, придурок!"
    m "!!!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1286
translate german ep210_dialogues7_escort_hotel_12_59c0695e:

    # mt "Сволочь!!!"
    # mt "Они унизили меня! Посмеялись надо мной!"
    # mt "Компания придурков! Жалкие неудачники!"
    # mt "!!!"
    # mt "Если я сейчас откажу ему..."
    # mt "Я не заработаю 450 долларов. Он говорил, что заплатит 900."
    # mt "А 450 долларов для меня сейчас будут совсем не лишними..."
    # mt "..."
    # mt "Да и потом, если я сейчас откажусь..."
    # mt "Я потеряю не только деньги, но и работу."
    # mt "А столько зарабатывать я больше нигде пока не смогу."
    # mt "Черт!!!"
    # client "Давай, отрабатывай свои деньги, шлюха!"
    # mt "Ненавижу!!!" 
    # client "Хорошо сосешь. Чувствуется опыт. Аха-ха!"
    # mt "Интересно, что будет, если я сейчас резко сожму зубы?"
    # mt "И откушу ему член."
    # mt "..."
    # client "Мммммм..."
    # mt "Фууу!"
    # client_friend_2 "Эй, я тоже хочу ей засунуть!"
    # client_friend_2 "Только не в рот..."
    # client "Без проблем, чувак. Можешь забирать ее."
    # client "Мне она больше не нужна."
    # mt "!!!"
    # client "Ты слышала?"
    # client "Выбирайся оттуда!"
    # m "Меня никто не предупреждал, что будет не один клиент!"
    # client "Вообще-то, в вашей организации есть дополнительный прайс."
    # client "Я плачу сверху еще 300 долларов..."
    # client "И ты не можешь отказаться."
    # client "Так мне сказала ваша начальница."
    # client "Поняла?!"
    # mt "!!!"
    # mt "Мерзавец!!!"
    # client "Сейчас пойдешь с ним."
    # mt "Жалкие неудачники!"
    # mt "Мне администратор ничего не говорила про добавление денег по прайсу!"
    # mt "Какого черта я узнаю об этом от этого мудака?!"
    # mt "Эта никчемная сучка не могла меня предупредить заранее?!"
    # mt "!!!"
    # mt "С другой стороны... Дополнительно я заработаю еще 300 долларов."
    # mt "..."
    # client_friend_2 "Говоришь, ты большой босс? Аха-ха!"
    # client_friend_2 "Никогда не трахал большого босса. Аха-ха!"
    # client_friend_2 "Давай, пососи мне сначала. Шлюшка."
    # mt "Никчемный неудачник!!!"
    # mt "!!!"
    # client_friend_2 "Настало время трахнуть большого босса!"
    # mt "!!!"
    # mt "Боже! Моника! Неужели ЭТО стоит терпеть из-за 900 долларов?!"
    # mt "Каких-то 900 долларов!!!"
    # client "О чувак! Давай, так ее!"
    # mt "!!!"
    # client_friend_2 "Эй, я трахаю большого босса. Аха-ха!"
    # client "Аха-ха!!!"
    # client "Так ее, эту шлюху!"
    # client_friend_2 "ААААААА!!!"
    # mt "Теперь я понимаю, каких клиентов имели ввиду те сучки с отеля!"
    # mt "!!!"
    # client_friend_2 "Шлюха в этот раз, конечно, не самый шик..."
    # client "Сойдет... Под пиво. Аха-ха!!!"
    # mt "!!!"
    # mt "!!!!!!!"
    # mt "АААААА!!!"
    # client "Свободна!"
    mt "Schwein!!!"
    mt "Они унизили меня! Посмеялись надо мной!"
    mt "Компания придурков! Жалкие неудачники!"
    mt "!!!"
    mt "Если я сейчас откажу ему..."
    mt "Я не заработаю 450 долларов. Он говорил, что заплатит 900."
    mt "А 450 долларов для меня сейчас будут совсем не лишними..."
    mt "..."
    mt "Да и потом, если я сейчас откажусь..."
    mt "Я потеряю не только деньги, но и работу."
    mt "А столько зарабатывать я больше нигде пока не смогу."
    mt "Verdammt!!!"
    client "Давай, отрабатывай свои деньги, шлюха!"
    mt "Ich hasse das!!!" 
    client "Хорошо сосешь. Чувствуется опыт. Аха-ха!"
    mt "Интересно, что будет, если я сейчас резко сожму зубы?"
    mt "И откушу ему член."
    mt "..."
    client "Мммммм..."
    mt "Puh!"
    client_friend_2 "Эй, я тоже хочу ей засунуть!"
    client_friend_2 "Только не в рот..."
    client "Без проблем, чувак. Можешь забирать ее."
    client "Мне она больше не нужна."
    mt "!!!"
    client "Ты слышала?"
    client "Выбирайся оттуда!"
    m "Меня никто не предупреждал, что будет не один клиент!"
    client "Вообще-то, в вашей организации есть дополнительный прайс."
    client "Я плачу сверху еще 300 долларов..."
    client "И ты не можешь отказаться."
    client "Так мне сказала ваша начальница."
    client "Поняла?!"
    mt "!!!"
    mt "Dieser Bastard!!!"
    client "Сейчас пойдешь с ним."
    mt "Жалкие неудачники!"
    mt "Мне администратор ничего не говорила про добавление денег по прайсу!"
    mt "Какого черта я узнаю об этом от этого мудака?!"
    mt "Эта никчемная сучка не могла меня предупредить заранее?!"
    mt "!!!"
    mt "С другой стороны... Дополнительно я заработаю еще 300 долларов."
    mt "..."
    client_friend_2 "Говоришь, ты большой босс? Аха-ха!"
    client_friend_2 "Никогда не трахал большого босса. Аха-ха!"
    client_friend_2 "Давай, пососи мне сначала. Шлюшка."
    mt "Никчемный неудачник!!!"
    mt "!!!"
    client_friend_2 "Настало время трахнуть большого босса!"
    mt "!!!"
    mt "Боже! Моника! Неужели ЭТО стоит терпеть из-за 900 долларов?!"
    mt "Каких-то 900 долларов!!!"
    client "О чувак! Давай, так ее!"
    mt "!!!"
    client_friend_2 "Эй, я трахаю большого босса. Аха-ха!"
    client "Hahaha!"
    client "Так ее, эту шлюху!"
    client_friend_2 "ААААААА!!!"
    mt "Теперь я понимаю, каких клиентов имели ввиду те сучки с отеля!"
    mt "!!!"
    client_friend_2 "Шлюха в этот раз, конечно, не самый шик..."
    client "Сойдет... Под пиво. Аха-ха!!!"
    mt "!!!"
    mt "!!!!!!!"
    mt "АААААА!!!"
    client "Свободна!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1376
translate german ep210_dialogues7_escort_hotel_11c_f916e7af:

    # m "Клиент заплатил 900 долларов. Плюс сверху 300 долларов."
    # m "За дополнительную услугу..."
    # m "Он изменил условия!"
    # m "Я не знала о том, что клиентам позволено такое!"
    # reception "Зато клиент остался доволен!"
    # reception "Есть желание клиента и есть прейскурант..."
    # reception "Если тебя что-то не устраивает, можешь отправляться на улицу."
    # mt "!!!"
    # mt "Может, к черту эти дурацки прейскуранты?"
    # mt "Попробую заработать на улице... Хоть и меньше, зато без таких унижений."
    # mt "..."
    # mt "Нет. Мне нужны деньги. А здесь неплохой заработок."
    # mt "Я сегодня за один вечер заработала 600 долларов."
    # mt "Пожалуй, мне стоит еще немного поработать здесь."
    # mt "..."
    # reception "Вот твои деньги."
    # reception "Завтра можешь приходить снова."
    # reception "На сегодня все. Клиенты не любят использованный товар."
    # mt "Вот сучка!"
    # mt "!!!"
    m "Клиент заплатил 900 долларов. Плюс сверху 300 долларов."
    m "За дополнительную услугу..."
    m "Он изменил условия!"
    m "Я не знала о том, что клиентам позволено такое!"
    reception "Зато клиент остался доволен!"
    reception "Есть желание клиента и есть прейскурант..."
    reception "Если тебя что-то не устраивает, можешь отправляться на улицу."
    mt "!!!"
    mt "Может, к черту эти дурацки прейскуранты?"
    mt "Попробую заработать на улице... Хоть и меньше, зато без таких унижений."
    mt "..."
    mt "Нет. Мне нужны деньги. А здесь неплохой заработок."
    mt "Я сегодня за один вечер заработала 600 долларов."
    mt "Пожалуй, мне стоит еще немного поработать здесь."
    mt "..."
    reception "Hier ist dein Geld."
    reception "Завтра можешь приходить снова."
    reception "На сегодня все. Клиенты не любят использованный товар."
    mt "Вот сучка!"
    mt "!!!"

translate german strings:

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:63
    old "50 долларов?!"
    new "50 долларов?!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:128
    old "Моника работает посудомойщицей в Shiny Hole"
    new "Моника работает посудомойщицей в Shiny Hole"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:213
    old "Моника по приказу Филиппа делала служащему отеля минет в туалете"
    new "Моника по приказу Филиппа делала служащему отеля минет в туалете"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:222
    old "Моника была вежлива со служащим отеля и он не остался в туалете, как предлагал ему Филипп."
    new "Моника была вежлива со служащим отеля и он не остался в туалете, как предлагал ему Филипп."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:240
    old "Сделать ему минет."
    new "Сделать ему минет."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:362
    old "Пошла она к черту!"
    new "Пошла она к черту!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:362
    old "Сказать то, что требует администратор."
    new "Сказать то, что требует администратор."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:582
    old "Что за бред?! Я не собираюсь раздеваться!"
    new "Что за бред?! Я не собираюсь раздеваться!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:582
    old "Раздеться."
    new "Раздеться."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:689
    old "Нет. Я не смогу."
    new "Нет. Я не смогу."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:752
    old "Прийти в отель ЛеГранд завтра."
    new "Прийти в отель ЛеГранд завтра."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:753
    old "Мне предложили работу в эскорте! Неужели я решусь на такое?! Хотя... Там можно неплохо заработать."
    new "Мне предложили работу в эскорте! Неужели я решусь на такое?! Хотя... Там можно неплохо заработать."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:820
    old "Переодеться и идти в ресторан."
    new "Переодеться и идти в ресторан."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:954
    old "Уйти отсюда!"
    new "Уйти отсюда!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:954
    old "Пойти в номер с клиентом."
    new "Пойти в номер с клиентом."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1026
    old "Нет, я не смогу!"
    new "Нет, я не смогу!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1026
    old "Сделать, как требует клиент."
    new "Сделать, как требует клиент."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1071
    old "Пойти на ресепшн и отдать 50 процентов от заработка администратору."
    new "Пойти на ресепшн и отдать 50 процентов от заработка администратору."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1145
    old "Вызов к клиенту. Он ждет в кафе."
    new "Вызов к клиенту. Он ждет в кафе."

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1255
    old "Ударить мерзавца и уйти!"
    new "Ударить мерзавца и уйти!"

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1255
    old "Залезть под стол и сделать клиенту минет."
    new "Залезть под стол и сделать клиенту минет."

# TODO: Translation updated at 2020-01-16 22:18

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:36
translate german ep210_dialogues7_escort_hotel_1_5a3d0d61:

    # mt "Я помню этого неудачника!"
    mt "Я помню этого неудачника!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:40
translate german ep210_dialogues7_escort_hotel_1_5f2846a7:

    # mt "Это он тогда был в туалете, когда я..."
    # mt "Когда мерзавец Филипп заставил меня..."
    # mt "Делать всякие его извращенские гадости!"
    mt "Это он тогда был в туалете, когда я..."
    mt "Когда мерзавец Филипп заставил меня..."
    mt "Делать всякие его извращенские гадости!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:57
translate german ep210_dialogues7_escort_hotel_1_be1047d2:

    # m "???" 
    # m "Ужин за счет ресторана?"
    m "???" 
    m "Ужин за счет ресторана?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:68
translate german ep210_dialogues7_escort_hotel_1_2ceac500:

    # hotel_staff "Так Вы позволите, Мэм?"
    # m "Что позволю?"
    hotel_staff "Так Вы позволите, Мэм?"
    m "Что позволю?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:177
translate german ep210_dialogues7_escort_hotel_1_03090b9c:

    # m "Ты пожалеешь, если расскажешь об этом хоть одной живой душе!"
    # m "Я сделаю так, что тебя выкинут с работы!"
    m "Ты пожалеешь, если расскажешь об этом хоть одной живой душе!"
    m "Я сделаю так, что тебя выкинут с работы!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:180
translate german ep210_dialogues7_escort_hotel_1_f22e0a4d:

    # m "И ты после этого не сможешь устроиться на работу даже..."
    m "И ты после этого не сможешь устроиться на работу даже..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:285
translate german ep210_dialogues7_escort_hotel_2_7f1b2644:

    # hotel_staff "Если бы я сразу сказал, что это обычный коридор..."
    # hotel_staff "То Вы не согласились бы, Мэм."
    # hotel_staff "Однако, прошу Вас, поверьте! Здесь никто никогда не ходит!"
    hotel_staff "Если бы я сразу сказал, что это обычный коридор..."
    hotel_staff "То Вы не согласились бы, Мэм."
    hotel_staff "Однако, прошу Вас, поверьте! Здесь никто никогда не ходит!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:677
translate german ep210_dialogues7_escort_hotel_6_b1f79569:

    # m "!!!"
    # m "Я делала это не за деньги! Это мои личные отношения! И это тебя не должно касаться!"
    m "!!!"
    m "Я делала это не за деньги! Это мои личные отношения! И это тебя не должно касаться!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:767
translate german ep210_dialogues7_escort_hotel_6_57a264e4:

    # m "..."
    # m "Да... Я пришла, чтобы заработать денег..."
    m "..."
    m "Да... Я пришла, чтобы заработать денег..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:770
translate german ep210_dialogues7_escort_hotel_6_1c88d382:

    # reception "И? Ты леди?"
    reception "И? Ты леди?"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:890
translate german ep210_dialogues7_escort_hotel_6_62ca7f99:

    # mt "Какой кошмар!!!"
    # mt "Мне предложили работать шлюхой в отеле!"
    # mt "Моника, до чего ты докатилась?!"
    # mt "..."
    # mt "С другой стороны..."
    # mt "Это хорошая возможность быстро заработать неплохие деньги."
    # mt "Но черт! Неужели я смогу работать здесь?!"
    mt "Какой кошмар!!!"
    mt "Мне предложили работать шлюхой в отеле!"
    mt "Моника, до чего ты докатилась?!"
    mt "..."
    mt "С другой стороны..."
    mt "Это хорошая возможность быстро заработать неплохие деньги."
    mt "Но черт! Неужели я смогу работать здесь?!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1204
translate german ep210_dialogues7_escort_hotel_7_2acdca2e:

    # girl_4 "Добро пожаловать в наш дружный коллектив." 
    girl_4 "Добро пожаловать в наш дружный коллектив." 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1206
translate german ep210_dialogues7_escort_hotel_7_16bd7868:

    # mt "!!!" 
    mt "!!!" 

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1217
translate german ep210_dialogues7_escort_hotel_7_6d543bf5:

    # m "Меня зовут..."
    m "Меня зовут..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1368
translate german ep210_dialogues7_escort_hotel_8_enter_restaurant_937f7632:

    # reception "Куда собралась? Тебе надо переодеться!"
    reception "Куда собралась? Тебе надо переодеться!"

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1371
translate german ep210_dialogues7_escort_hotel_8_enter_restaurant_910799f0:

    # m "Я не на работу. Я просто поесть..."
    m "Я не на работу. Я просто поесть..."

# game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1374
translate german ep210_dialogues7_escort_hotel_8_enter_restaurant_86424ace:

    # reception "Хорошо, иди."
    # reception "Но учти, что здесь надо работать и ублажать клиентов, а не отдыхать!"
    reception "Хорошо, иди."
    reception "Но учти, что здесь надо работать и ублажать клиентов, а не отдыхать!"

translate german strings:

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:38
    old "Моника делала минет Филиппу в туалете"
    new ""

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1218
    old "Снежанна"
    new ""

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1357
    old "Работать в эскорте (в следующем апдейте). (disabled)"
    new ""

    # game/Quests/EP2_10/ep210_dialogues7_escort_hotel.rpy:1357
    old "Идти в ресторан."
    new ""

