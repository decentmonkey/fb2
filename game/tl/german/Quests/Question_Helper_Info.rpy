# TODO: Translation updated at 2019-09-07 14:01

# game/Quests/Question_Helper_Info.rpy:11
translate german question_helper_hairdye_99c30ae8:

    # help "Подойти к зеркалам и взять краску для волос."
    help "Geh zum Spiegel und nimm die Haarfarbe."

# game/Quests/Question_Helper_Info.rpy:14
translate german question_helper_hairdye_446cd0a1:

    # help "Разлить краску на ковер."
    help "Schütte die Farbe auf den Teppich."

# game/Quests/Question_Helper_Info.rpy:25
translate german question_helper_hairdye_find_julia_87501fac:

    # help "Юлия в подвале. Вход в подвал у лестницы на нижнем этаже."
    help "Julia ist im Keller. Der Eingang zum Keller ist bei der Treppe."

# game/Quests/Question_Helper_Info.rpy:38
translate german question_helper_laundry_e020c3e0:

    # help "Взять платье в нижнем ящике."
    help "Das Kleid ist in der unteren Schublade."

# game/Quests/Question_Helper_Info.rpy:43
translate german question_helper_laundry_42cf0175:

    # help "Взять утюг из нижней коробки."
    help "Nimm das Bügeleisen aus der unteren Kiste."

# game/Quests/Question_Helper_Info.rpy:47
translate german question_helper_laundry_71fa45e7:

    # help "Убрать мусор. Затем применить поочереди платье и утюг, чтобы погладить."
    help "Entferne das Zeug, dann lege das Kleid und das Bügeleisen auf das Bügelbrett."

# game/Quests/Question_Helper_Info.rpy:58
translate german question_gas_trader1_e4178b4b:

    # help "Кассир за дверью."
    help "Der Kassierer ist hinter der Tür."

# game/Quests/Question_Helper_Info.rpy:69
translate german question_gas_trader2_fdca3557:

    # help "Идти сюда."
    help "Hier her."

# game/Quests/Question_Helper_Info.rpy:72
translate german question_gas_trader2_5443ee53:

    # help "Затем идти сюда."
    help "Geh dort hin."

# game/Quests/Question_Helper_Info.rpy:75
translate german question_gas_trader2_bac6dbd0:

    # help "Разбить дорогую бутылку вина."
    help "Zerbrich eine teuere Flasche Wein."

# game/Quests/Question_Helper_Info.rpy:86
translate german question_cloth_shop_dick_buy_dress_0453fdd4:

    # help "Проверить места."
    help "Schau dich um."

# game/Quests/Question_Helper_Info.rpy:93
translate german question_cloth_shop_dick_buy_dress_cea8ca3e:

    # help "После каждого места идти примерять платье в примерочную."
    help "Nach jedem neuen Ort, probiere das Kleid in der Umkleide an."

# game/Quests/Question_Helper_Info.rpy:96
translate german question_cloth_shop_dick_buy_dress_78ebcbbd:

    # help "Повесить свое платье на вешалку."
    help "Hänge Dein Kleid über den Bügel."

# game/Quests/Question_Helper_Info.rpy:98
translate german question_cloth_shop_dick_buy_dress_bfc68901:

    # help "Нажать сюда чтобы примерить новое платье."
    help "Hier klicken, um ein neues Kleid anzuprobieren."

# game/Quests/Question_Helper_Info.rpy:113
translate german question_office_tea_lost_phone_50c3cbe8:

    # help "Телефон на полу при входе в офис..."
    help "Das Handy liegt auf dem Boden in der Nähe des Büroeingangs."

# game/Quests/Question_Helper_Info.rpy:120
translate german question_clothing_shop_sell_dress_c37d1348:

    # help "Подходить к клиентам и пытаться продать платье."
    # help "Этот клиент, в итоге, должен купить его."
    help "Geh zu den Kunden und versuche, das Kleid zu verkaufen."
    help "Dieser Kunde wird es schlussendlich kaufen."

translate german strings:

    # game/Quests/Question_Helper_Info.rpy:7
    old "Посмотреть подсказку"
    new "Suche einen Hinweis."

    # game/Quests/Question_Helper_Info.rpy:7
    old "Продолжить игру."
    new "Weiter mit dem Spiel."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди постоянно пытается заглянуть мне под юбку."
    new "Bardie versucht mir ständig, unter den Rock zu schauen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди шантажирует Бетти. Может быть я могу использовать это?"
    new "Bardie erpresst Betty. Vielleicht kann ich das irgendwie nutzen?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я могу питаться в доме. Если буду соблюдать условие..."
    new "Ich kann hier essen, wenn ich mich an die Regeln halte..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди в при любом удобном случае заглядывает мне под юбку. И я ничего не могу с этим поделать..."
    new "Bardie schaut mir bei jeder Gelegenheit unter den Rock und ich kann nichts dagegen tun..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди хочет чтобы я одевала трусики Бетти, которые она носила в прошлый раз. Интересно, как я узнаю какие именно трусики она носит сегодня?"
    new "Bardie will, dass ich Betty's Höschen trage, welche sie als letzte getragen hat. Ich frage mich wie ich rausfinden kann, welche sie heute trägt."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди хочет чтобы я одевала трусики Бетти, которые она носила в прошлый раз. Бетти валяется в них своей толстой задницей на моей кровати по вечерам."
    new "Bardie will, dass ich Betty's Höschen trage, welche sie gestern an hatte. Betty wälzt sich darin mit ihrem fetten Arsch in meinem Bett."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди снова требует от меня каких-то извращений! Похоже, удалось переключить его на Бетти. Давно стоит проучить ее за то как она обращается со мной! Итак, надо сообщать Барди когда мы собираемся идти на фитнесс..."
    new "Bardie erwartet schon wieder etwas schmutziges von mir. Sieht so aus, als konnte ich seine Aufmerksamkeit nun auf Betty lenken. Geschieht ihr auch ganz Recht. Wie kann ich Bardie informieren, wann ich mit Betty wieder in's Fitnessstudio gehe?..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди удалось найти компромат на Бетти. Надеюсь теперь он оставит меня в покое!"
    new "Bardie konnte Beweise sammeln, um Betty zu erpressen. Hoffentlich lässt er mich nun in Ruhe."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Барди хочет чтобы я одевала трусики Бетти, которые она носила в прошлый раз. Бетти обязана показать мне их, если я спрошу."
    new "Bardie will, dass ich Betty's getragene Höschen benutze. Sie zeigt sie mir, wenn ich danach frage."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я теперь должна ходить без трусиков в доме. Иначе Барди накажет меня. Мерзавец!"
    new "Ich kann jetzt zu Hause keine Höschen mehr tragen, sonst wird mich Bardie bestrafen. Was für ein Arschloch!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти ждет от меня, что я буду регулярно убираться в доме."
    new "Betty erwartet, dass ich da Haus regelmäßig putze."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти"
    new "Betty"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти ждет от меня, что я буду регулярно убираться в доме и тереть это проклятое пятно."
    new "Betty erwartet, dass ich regelmäßig das Haus und diesen verfluchten Fleck putze."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти хочет чтобы я носила ее сумку с вещами, пока она занимается фитнесом. Фитнес у Бетти по вторникам и четвергам."
    new "Betty will, dass ich ihre Fitnesstasche trage. Sie trainiert immer am Dienstag und Donnerstag."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Кажется, Бетти не так строго следит за тем, чтобы я убиралась все время."
    new "Sieht so aus, als ob Betty nicht sehr gut darauf achtet, ob ich die ganze Zeit putze."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти знает что я ношу ее трусики. Более того, она должна показать трусики, которые носит сегодня, если я спрошу ее об этом."
    new "Betty weiß, dass ich ihre Höschen trage und zeigt mir ihre, wenn ich danach frage."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Бетти теперь должна ходить без трусиков в доме. Она сказала что я могу проверять это. Это все проделки Барди!"
    new "Betty darf ebenfalls keine Höschen mehr zu Hause tragen. Sie sagte mir, dass ich nachschauen kann. Und alles nur wegen Bardie, dem Pisser!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне надо проверить что происходит в моем офисе."
    new "Ich muss nachschauen, was in meinem Büro los ist."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Офис"
    new "Büro"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Теперь я могу работать в офисе. Денег это не добавило, но я могу чувствовать себя Боссом снова..."
    new "Ich kann wieder im Büro arbeiten. Es bringt mir zwar kein Geld, aber ich kann mich wieder als Chef fühlen..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Биф требует, чтобы я собирала отчеты у своих сотрудников и приносила ему. За кого он меня держит?!"
    new "Biff will, dass ich die Berichte von den Angestellten einsammle. Wer denkt er, dass ich bin?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Биф хочет чтобы я ходила к нему на кастинги. Если я хочу убедить его взять меня в офис, мне надо завоевать его доверие. Но смогу-ли я на это пойти?"
    new "Biff will, dass ich an seinen Castings teilnehme. Wenn ich ihn dazu überreden will, mich im Büro anzustellen, muss ich sein Vertrauen gewinnen. Aber bin ich dazu in der Lage?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Похоже я нашла возможность зарабатывать деньги, делая фотосессии у Бифа. Но эти фотосессии все более развратные. Долго-ли я смогу себе это позволять?"
    new "Sieht so aus, als habe ich einen Weg gefunden, Geld zu verdienen, nämlich durch Fotoshoots bei Biff. Aber diese Shootings werden von Mal zu Mal schmutziger. Kann ich das ertragen?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я отказалась от того что хочет Биф. Но найду-ли я другой способ достать деньги? Возможно, мне придется, все-таки, передумать?"
    new "Ich mache das nicht, was Biff verlangt. Aber gibt es einen anderen Weg, an das Geld zu kommen? Vielleicht muss ich meine Einstellung nochmal überdenken."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Фред просит говорить Юлии какие-то непонятные слова и делать странные вещи. Не понимаю зачем это, но мне придется выполнять его просьбы, потому что он шантажирует меня. Но, возможно, я могу потянуть время..."
    new "Fred will, dass ich Julia Unsinn erzähle und komische Dinge mache. Ich weiß nicht, warum er das will, aber ich muss es tun, da er mich erpresst. Aber vielleicht kann ich ihn etwas hinhalten."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне надо как-то найти $ 5000 до пятницы. Где мне их взять? Может быть спросить Бифа о работе?"
    new "Ich muss bis Freitag 5.000 Dollar auftreiben. Wo könnte ich die her bekommen. Ich könnte Biff wegen dem Job fragen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Дик"
    new "Dick"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне надо найти еще $ 1000, иначе конец!"
    new "Ich muss noch 1.000 Dollar auftreiben, sonst ist es um mich geschehen!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Теперь Дик имеет свой галстук! Мой кошмар закончился. Надо проведать его."
    new "Jetzt hat Dick seine Krawatte! Mein Alptraum ist vorüber. Ich sollte ihn treffen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Теперь я должна приносить Виктории раз в неделю $ 5000 до пятницы. Я даже не представляла насколько она опасна."
    new "Ich muss Victoria wöchentlich jeden Freitag 5.000 Dollar bringen. Ich hatte keine Ahnung, wie gefährlich sie ist."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Так продолжаться не может. Виктория не даст мне нормально жить. Надо как-то воздействовать на Дика. Его единственное слабое место - это женщины. У меня не получается, как я ни пыталась. Может быть попробовать попросить помощи у Мелани?"
    new "So kann ich nicht weiter machen. Victoria lässt mir keine Luft zum atmen. Ich muss etwas wegen Dick unternehmen, Frauen sind seine einzige Schwäche. Aber ich komme nicht an ihn ran, egal, wie sehr ich es versuche. Vielleicht sollte ich Melanie um Hilfe bitten."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мелани обещала узнать всю правду про Дика. За это я согласилась поучаствовать с ней в какой-то фотосессии. Надеюсь я не пожалею об этом..."
    new "Melanie hat versprochen, die ganze Wahrheit über Dick heraus zu finden. Dafür habe ich einem Fotoshoot zugestimmt. Ich hoffe, ich werde das nicht bereuen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Это был жуткий кошмар, но я надеюсь оно того стоило! Ну держись, Виктория! Я нашла кое-кого кто тебе не по зубам!"
    new "Das war ein furchtbarer Alptraum, aber ich hoffe, das war es wert! Warte nur Victoria, jetzt habe ich jemanden gefunden, der ist sogar für dich zu tough."

    # game/Quests/Question_Helper_Info.rpy:125
    old "У меня появился шанс вернуть прежнюю жизнь! ШАНС!!! Неужели Мелани сможет мне помочь? Но что она захочет попросить взамен?.."
    new "Ich habe die Möglichkeit, mein altes Leben zurück zu erhalten! DIE MÖGLICHKEIT!!! Kann Melanie mir wirklich helfen? Aber was wird sie als Gegenleistung verlagen?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "От Мелани нет новостей... Я даже боюсь представить что с ней..."
    new "Es gibt keine Neuigkeiten von Melanie... Ich wage mir nicht vorzustellen, was mit ihr passiert sein könnte..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я жду новостей от Мелани..."
    new "Ich warte auf Neuigkeiten von Melanie..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мелани решила унизить меня в обмен на свою помощь... Пойду-ли я на то что она предложила мне?"
    new "Melanie hat beschlossen, mich als Gegenleistung zu erniedrigen... Werde ich es trotzdem tun?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "В фитнес-зале я встретила Стефани и Ребекку. Я, кажется, убедила их что все это игра. Но долго-ли они будут в это верить?"
    new "Ich habe Stephanie und Rebecca im Fitnessstudio getroffen. Ich hoffe, ich konnte sie überzeugen, dass das alles nur ein Spiel ist. Aber wie lange werden sie mir glauben?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Стив приходил в гости к Ральфу. Мне надо избегать встречи с ним!"
    new "Steve besuchte Ralph. Ich muss ihm aus dem Weg gehen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Жизнь"
    new "Leben"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Не могу поверить что я согласилась на секс со Стивом! Мне надо проверить перевел-ли он деньги Виктории."
    new "Ich kann es nicht glauben, dass ich mit Steve Sex hatte. Ich muss überprüfen, ob er sein Versprechen eingehalten hat."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Стив обманул меня! Он что, думает что можно иметь со мной секс за бесплатно?!"
    new "Er hat mich schon wieder belogen! Denkt er, er kann mit mir Sex umsonst haben?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне надо поймать Стива, этого слизняка! Он не уйдет от меня!"
    new "Ich muss diesen Dreckssack Steve erwischen! Er wird mir nicht entkommen!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Стив хочет трахнуть меня снова, чтобы закрыть эту чертову сделку! Мне нужны деньги, но цена..."
    new "Steve will wieder mit mir schlafen um den Deal abzuschließen! Ich brauche das Geld, aber ist es mir das wert?..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Стив слизняк и негодяй! Но, может быть, через него можно найти какую-то работу или просто деньги?"
    new "Steve ist ein Bastard, aber vielleicht kann er mir helfen einen Job zu finden oder mir wenigstens mehr Geld geben."

    # game/Quests/Question_Helper_Info.rpy:125
    old "В доме для меня еды нет. По крайней мере пока. Мне надо где-то питаться!"
    new "Es gibt kein Essen für ich im Haus. Zumindest noch nicht. Ich muss irgendwo essen!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "В доме для меня еды нет. По крайней мере пока. Мне надо где-то питаться. Есть эта дурацкая разноска флаеров. Но, может быть, проще воровать еду где-нибудь? Но готова-ли я на это пойти? Или, может быть, проще занять у кого-то деньги?"
    new "Es ist kein Essen im Haus, zumindest noch nicht. Ich muss irgendwo anders essen. Ich kann mit Flyer austragen Geld verdienen, aber vielleicht ist es einfach, das Essen einfach zu stehlen? Bin ich dafür bereit? Oder vielleicht ist es besser, mir das Geld von irgend jemandem zu leihen?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я разношу гребаные флаеры за жалкий кебаб. Может быть есть способы заработать на еду проще?"
    new "Ich habe die beknackten Flyer verteilt um einen reudigen Kebab zu bekommen. Vielleicht gibt es Wege, einfacher an Essen zu kommen."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Вместо разноски флаеров я нашла более быстрый способ заработать. Он немного... неудобный, но ведь это ненадолго..."
    new "Ich habe einen schnelleren Weg gefunden, Geld zu verdienen, anstatt Flyer zu verteilen. Er ist etwas... unbequem, aber ich mache das ja nicht lange."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я нашла способ зарабатывать на еду, моя посуду в Баре. Я не в восторге, но ведь это временно?"
    new "Ich habe einen weiteren Weg gefunden, um Geld zu verdienen. Nämlich als Tellerwäscherin in der Kneipe. Ich bin zwar nicht glücklich damit, aber es ist ja nur für eine kurze Zeit."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне надо как-то вернуть мою прошлую жизнь. Я не смогу долго прожить в этом кошмаре!"
    new "Ich muss mein altes Leben zurück bekommen. Ich kann in diesem Alptraum nicht mehr länger leben!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я устроилась работать официанткой в Shiny Hole. У меня нет зарплаты и почти все чаевые надо отдавать хозяевам, но это шанс заработать хоть какие-то деньги. Я не могу выносить этих жутких клиентов, но у меня нет выбора. Тем более, это ненадолго..."
    new "Ich habe arbeit im Shiny Hole als Bedienung gefunden. Allerdings bekomme ich keinen Lohn und muss fast all mein Trinkgeld den Besitzern geben. Ich komme auch kaum mit den widerlichen Kunden klar, aber es ist ja nur übergangsweise. Ich brauch diesen Job..."

    # game/Quests/Question_Helper_Info.rpy:125
    old "Кажется, Джо пытается меня лапать, когда я мою посуду... Ненавижу эту работу!"
    new "Anscheinend wollte Joe mich anfassen, während ich abwusch... Ich hasse diesen Job!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Кажется, Эшли пытается меня лапать. Я не понимаю с какой целью. Ведь у нее есть муж!"
    new "Anscheinend wollte Ashley mich anfassen, während ich abwusch... Hat sie nicht einen Ehemann?!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне удалось договориться с Джо, чтобы снова работать официанткой. Для этого мне пришлось позволить ему лапать меня, пока его жена не видит. Моника, ты уверена что это правильный путь?"
    new "Ich konnte mit Joe vereinbaren, dass ich wieder als Bedienung arbeiten kann, aber dafür hat er mich angefasst. Monica, ist das wirklich das, was du willst?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Я танцевала голой перед Джо, пока он дрочил на меня. А потом пряталась за диваном от его жены! Моника, ты точно помнишь кто ты?! Ты не [monica_pub_name]! Одумайся!"
    new "Ich habe nackt for Joe getanzt, während er sich einen runter geholt hat. Dann habe ich mich nackt hinter dem Sofa vor seiner Frau versteckt. Monica, komm schon, wach auf, du bist nicht [monica_pub_name]!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мне удалось договориться с Эшли, чтобы снова работать официанткой. Эта грязная Эшли - гребаная лесбиянка! Неужели я способна на такое?!"
    new "Ich konnte Ashly überreden, wieder als Bedienung zu arbeiten. Sie ist eine verdammte Lesbe! Will ich das wirklich?"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Любой ценой избегать встречи с Маркусом!"
    new "Vermeide um jeden Preis, Marcus zu treffen!"

    # game/Quests/Question_Helper_Info.rpy:125
    old "Мелани советует мне самой придти к Маркусу. Якобы это может помочь... Готова-ли Я на это пойти и что меня ждет?"
    new "Melanie hat mir geraten, zu Marcus zu gehen. Sie sagte, es könnte helfen... Will ich das wirklich?"

# TODO: Translation updated at 2019-10-18 04:32

translate german strings:

    # game/Quests/Question_Helper_Info.rpy:125
    old "Маркус ждет меня снова... Смогу-ли я решиться снова навестить его?"
    new "Marcus wartet wieder auf mich... Werde ich in der Lage sein, ihn nochmal zu treffen?"

# TODO: Translation updated at 2020-01-07 23:40

# game/Quests/Question_Helper_Info.rpy:125
translate german question_steve_ask_ralph_03a21e4f:

    # help "Спросить у Ральфа по поводу Стива."
    help "Frage Ralph wegen Steve."

