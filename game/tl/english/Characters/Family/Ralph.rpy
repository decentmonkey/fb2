# TODO: Translation updated at 2018-12-05 18:00

# game/Characters/Family/Ralph.rpy:7
translate english ralphInteract1_026d6ed0:

    # mt "Это Ральф."
    # "Подкаблучник."
    # "Бетти неплохо выдрессировала его, надо отдать должное ей."
    # "Но мужчины такими и должны быть."
    # "Мы, женщины, выше мужчин и они должны это понимать!"
    mt "It's Ralph."
    "A henpecked man"
    "Betty trained him well, I'll give her that."
    "Men should be like that."
    "Women are above them, and they need to understand that!"

# TODO: Translation updated at 2018-12-06 21:42

# game/Characters/Family/Ralph.rpy:15
translate english ralphInteract1_5daac008:

    # mt "Мне надо переодеться сначала."
    # "Не стоит портить впечатление у Ральфа."
    mt "I need to change clothes first."
    "It's better not to spoil the impression to Ralph."

# game/Characters/Family/Ralph.rpy:18
translate english ralphInteract1_975c2efd:

    # "Я не хочу оставаться один на один с этой сучкой Бетти."
    "I don't want to be alone with this bitch Betty."

# game/Characters/Family/Ralph.rpy:20
translate english ralphInteract1_5693bbc1:

    # "Я не хочу оставаться один на один с Бетти."
    "I don't want to be alone with Betty."

