# TODO: Translation updated at 2018-12-05 18:00

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:55
translate english rich_hotel_event_hall_environment_e53dfcf1:

    # mt "Черт! Что же делать???"
    # "Где мне достать деньги???"
    mt "Fuck! What should I do???"
    "Where can I get money???"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:58
translate english rich_hotel_event_hall_environment_e3973153:

    # mt "Приятное место..."
    # "Я расслабляюсь..."
    mt "Pleasant place..."
    "I'm relaxing..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:63
translate english rich_hotel_event_hall_environment_23ced526:

    # mt "Какой-то кавалер..."
    # "Сотрит на меня весь вечер."
    # "Мило..."
    mt "Some admirer..."
    "Staring at me all night."
    "Nice..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:71
translate english rich_hotel_event_hall_environment_c595b2e9:

    # mt "Этот мальчик из персонала отеля."
    # "Не стоит внимания..."
    mt "This boy is from the hotel staff."
    "Not worth attention..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:78
translate english rich_hotel_event_hall_environment_a127c17c:

    # mt "Эта одежда закрывает собой мой постер!"
    mt "This apparel covers my poster!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:82
translate english rich_hotel_event_hall_environment_3775ea5b:

    # mt "В этих стульях явно не спрятаны деньги, которые мне так нужны!"
    mt "It is clear that there is no money that I need in these chairs."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:84
translate english rich_hotel_event_hall_environment_32e9aad3:

    # mt "В этом месте дорогая мебель..."
    # "Мило..."
    mt "This place has expensive furniture..."
    "Nice..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:89
translate english rich_hotel_event_hall_environment_9efc8dbb:

    # mt "Мне не до этого сейчас!"
    # "Мне надо найти деньги, срочно!"
    mt "I'm not up to that right now!"
    "I need to find money, now!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:92
translate english rich_hotel_event_hall_environment_65b86e51:

    # mt "Милый светильник."
    # "Приятный свет."
    mt "Nice lamp."
    "Nice light."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:97
translate english rich_hotel_event_hall_environment_9efc8dbb_1:

    # mt "Мне не до этого сейчас!"
    # "Мне надо найти деньги, срочно!"
    mt "I'm not up to that right now!"
    "I need to find money, now!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:100
translate english rich_hotel_event_hall_environment_5ed498fe:

    # mt "Пальма..."
    # "Вспоминаю свой отдых на островах..."
    mt "Palm tree..."
    "I remember my vacation on the Islands..."

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:105
translate english rich_hotel_event_hall_environment_1bfda7ac:

    # mt "Может быть попросить помощи у кого-нибудь???"
    # "Но вряд-ли это поможет. Судя по виду, здесь все такие же мерзавцы как этот Филип!"
    mt "Maybe I should ask for help from someone???"
    "But they're unlikely to help. Apparently, they're all scoundrels like that Philip!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:108
translate english rich_hotel_event_hall_environment_d05e7948:

    # mt "Наверное это журналист..."
    # "Надеюсь мои слова не слишком растиражируют..."
    # "Мне надо будет потом как-то отказаться от них, объяснить что это было не так понято."
    # "Я придумаю что-нибудь когда верну все назад!"
    mt "Probably a journalist..."
    "I hope my words aren't too public..."
    "I will have to take them back later, and explain that everything was not understood correctly."
    "I'll figure something out when I get my life back!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:114
translate english rich_hotel_event_hall_environment_0a9c18c7:

    # mt "Постер с обложкой моего журнала."
    # mt "Эта одежда закрывает собой мой постер!"
    mt "A poster with the cover of my magazine."
    mt "This apparel covers my poster!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:118
translate english rich_hotel_event_hall_environment_c8ebb783:

    # mt "Мелани?! Здесь?!"
    # "А почему не Я?!"
    mt "Melanie?! Here?!"
    "Why not me?!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:122
translate english rich_hotel_event_hall_environment_5a2e00d7:

    # mt "Я наговорила на этой сцене всякой ерунды..."
    # mt "Надеюсь мои слова не слишком растиражируют..."
    # "Мне надо будет потом как-то отказаться от них, объяснить что это было не так понято."
    # "Я придумаю что-нибудь когда верну все назад!"
    mt "I said some bullshit on that stage..."
    mt "I hope my words aren't too public..."
    "I will have to take them back later, and explain that everything was not understood correctly."
    "I'll figure something out when I get my life back!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:129
translate english rich_hotel_event_hall_environment_9efc8dbb_2:

    # mt "Мне не до этого сейчас!"
    # "Мне надо найти деньги, срочно!"
    mt "I'm not up to that right now!"
    "I need to find money, now!"

# game/Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:132
translate english rich_hotel_event_hall_environment_146855af:

    # mt "Может быть мне присесть за один из столиков?"
    mt "Maybe I should sit at one of the tables."

translate english strings:

    # Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:33
    old "СТОЛИКИ"
    new "TABLES"

    # Rooms/Rich_Hotel/Event/rich_hotel_event_hall.rpy:34
    old "ВИП ЗОНА"
    new "VIP AREA"

