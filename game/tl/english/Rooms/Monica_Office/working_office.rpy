# TODO: Translation updated at 2019-07-17 02:25

# game/Rooms/Monica_Office/working_office.rpy:50
translate english working_office_teleport_563f5c25:

    # mt "Это дверь в мой временный кабинет."
    # mt "Пока я не верну себе мой кабинет наверху!"
    mt "This is a door to my temporary office."
    mt "Until I get back everything upstairs!"

# game/Rooms/Monica_Office/working_office.rpy:60
translate english working_office_environment_873ae103:

    # mt "Это какая-то дыра, наполненная бездельниками."
    # mt "Но ничего, скоро я все верну назад и этот офис обязательно закрою."
    mt "Hole, filled with useless slackers."
    mt "But that’s okay, I'll get everything back soon and close this deparment!"

# game/Rooms/Monica_Office/working_office.rpy:63
translate english working_office_environment_4d8eb37a:

    # mt "Наконец-то у меня есть хоть какой-то офис!"
    mt "Finally I have an office again!"

# game/Rooms/Monica_Office/working_office.rpy:65
translate english working_office_environment_0a2a66c0:

    # mt "На этих компьютерах нет ничего хоть сколько-то стоящего."
    # mt "Только бесполезные отчеты"
    mt "This computers dont have anything valuable."
    mt "Just useless reports."

# game/Rooms/Monica_Office/working_office.rpy:68
translate english working_office_environment_19e1549d:

    # mt "Может украсть один из них?"
    # mt "Его бы можно было продать..."
    # mt "Хотя нет... Слишком большой риск..."
    mt "Should I steal one of them?"
    mt "I might be able to sell one..."
    mt "On the other hand... risk is too high..."

# game/Rooms/Monica_Office/working_office.rpy:72
translate english working_office_environment_be3c971d:

    # mt "Надо же какой роскошный цветок."
    # mt "Ему явно не место здесь."
    # mt "Его надо будет переселить ко мне в кабинет!"
    mt "What a beautifull flower."
    mt "It doesn’t fit here."
    mt "I'll have to move it to my office!"

# game/Rooms/Monica_Office/working_office.rpy:76
translate english working_office_environment_3397adbd:

    # mt "Папки с бесполезными отчетами!"
    mt "Folders with useless reports!"

