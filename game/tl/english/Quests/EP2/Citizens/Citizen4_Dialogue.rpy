# TODO: Translation updated at 2019-08-29 19:50

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:11
translate english citizen4_dialogue_b8f1bd99:

    # m "Мистер... Можно к Вам обратиться?"
    m "Мистер... Можно к Вам обратиться?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:16
translate english citizen4_dialogue_2df7ab1d:

    # citizen4 "Нельзя!"
    citizen4 "Нельзя!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:18
translate english citizen4_dialogue_67ab3514:

    # citizen4 "Да?"
    citizen4 "Да?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:23
translate english citizen4_dialogue_05979ea1:

    # m "Возьмите, пожалуйста, этот флаер..."
    m "Возьмите, пожалуйста, этот флаер..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:27
translate english citizen4_dialogue_5e125193:

    # citizen4 "Хорошо."
    # citizen4 "А что на нем?"
    # m "Это реклама вкусного кебаба... Там же написано!"
    citizen4 "Хорошо."
    citizen4 "А что на нем?"
    m "Это реклама вкусного кебаба... Там же написано!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:31
translate english citizen4_dialogue_94968960:

    # citizen4 "А я подумал Вы решили познакомиться со мной!"
    citizen4 "А я подумал Вы решили познакомиться со мной!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:37
translate english citizen4_dialogue_da9bd1ba:

    # m "У меня нет желания знакомиться с Вами!"
    m "У меня нет желания знакомиться с Вами!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:39
translate english citizen4_dialogue_5d080525:

    # m "Что вы имеете ввиду?"
    # citizen4 "Я бы хотел узнать по ближе такую красотку. Думаю, мы бы договорились."
    # m "Договорились о чем?"
    # citizen4 "Что за вопросы? Обычно девушки вашей профессии это понимают."
    # mt "Да что за люди здесь живут?! Всем надо одно и то же..."
    m "Что вы имеете ввиду?"
    citizen4 "Я бы хотел узнать по ближе такую красотку. Думаю, мы бы договорились."
    m "Договорились о чем?"
    citizen4 "Что за вопросы? Обычно девушки вашей профессии это понимают."
    mt "Да что за люди здесь живут?! Всем надо одно и то же..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:45
translate english citizen4_dialogue_d443b848:

    # mt "В любом случае об этом лучше говорить без этой дурацкой рекламы кебаба..."
    mt "В любом случае об этом лучше говорить без этой дурацкой рекламы кебаба..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:48
translate english citizen4_dialogue_a3e67ba0:

    # citizen4 "Давайте в другой раз!"
    citizen4 "Давайте в другой раз!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:58
translate english citizen4_dialogue_pilon_34556769:

    # m "Привет! Помню, ты хотел познакомиться поближе?"
    m "Привет! Помню, ты хотел познакомиться поближе?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:60
translate english citizen4_dialogue_pilon_d52af87d:

    # citizen4 "Конечно, давай познакомимся."
    # m "Ну я не о близком знакомстве...Немного о другом."
    # citizen4 "Да не вопрос, я понимаю о чем речь. Любой из нашего района тебя поймет с полуслова."
    citizen4 "Конечно, давай познакомимся."
    m "Ну я не о близком знакомстве...Немного о другом."
    citizen4 "Да не вопрос, я понимаю о чем речь. Любой из нашего района тебя поймет с полуслова."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:67
translate english citizen4_dialogue_pilon_0bf21327:

    # citizen4 "Ну что, давай начнем."
    citizen4 "Ну что, давай начнем."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:77
translate english citizen4_dialogue_pilon_loop4_a4d6d47c:

    # citizen4 "Покажи мне свои сиськи!"
    citizen4 "Покажи мне свои сиськи!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:79
translate english citizen4_dialogue_pilon_loop4_e7678610:

    # mt "Я не могу себе этого позволить!"
    # "Я еще не настолько опустилась!"
    # "И, надеюсь, этого не произойдет НИКОГДА!"
    # help "Требуется [monicaWhoringClothBoobsCorruptionRequired] corruption"
    mt "Я не могу себе этого позволить!"
    "Я еще не настолько опустилась!"
    "И, надеюсь, этого не произойдет НИКОГДА!"
    help "Требуется [monicaWhoringClothBoobsCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:86
translate english citizen4_dialogue_pilon_loop4_48b1ace1:

    # m "Я не собираюсь раздеваться, только так."
    m "Я не собираюсь раздеваться, только так."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:90
translate english citizen4_dialogue_pilon_loop4_810a1fb4:

    # citizen4 "Да, очень хорошо, но я видел сиськи и получше!"
    citizen4 "Да, очень хорошо, но я видел сиськи и получше!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:92
translate english citizen4_dialogue_pilon_loop4_dd728403:

    # mt "Козел!"
    mt "Козел!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:94
translate english citizen4_dialogue_pilon_loop4_f52852df:

    # citizen4 "Хотя весьма не плохо."
    citizen4 "Хотя весьма не плохо."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:112
translate english citizen4_dialogue_pilon_loop4_7c030a6c:

    # citizen4 "Покажи свой задницу."
    citizen4 "Покажи свой задницу."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:115
translate english citizen4_dialogue_pilon_loop4_299ae0b0:

    # mt "Я не могу себе этого позволить!"
    # "Я еще не настолько опустилась!"
    # "И, надеюсь, этого не произойдет НИКОГДА!"
    # help "Требуется [monicaWhoringClothAssCorruptionRequired] corruption"
    mt "Я не могу себе этого позволить!"
    "Я еще не настолько опустилась!"
    "И, надеюсь, этого не произойдет НИКОГДА!"
    help "Требуется [monicaWhoringClothAssCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:122
translate english citizen4_dialogue_pilon_loop4_48b1ace1_1:

    # m "Я не собираюсь раздеваться, только так."
    m "Я не собираюсь раздеваться, только так."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:126
translate english citizen4_dialogue_pilon_loop4_74152a4d:

    # citizen4 "Красивая задница, но я люблю побольше!"
    citizen4 "Красивая задница, но я люблю побольше!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:128
translate english citizen4_dialogue_pilon_loop4_a834c593:

    # citizen4 "Но ее можно увеличить, подумай об этом."
    citizen4 "Но ее можно увеличить, подумай об этом."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:137
translate english citizen4_dialogue_pilon_loop4_4df3fe38:

    # citizen4 "Покрутись на шесте немного. Надеюсь, ты хорошо двигаешься."
    citizen4 "Покрутись на шесте немного. Надеюсь, ты хорошо двигаешься."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:140
translate english citizen4_dialogue_pilon_loop4_d15b9b6b:

    # mt "Я не могу себе этого позволить!"
    # "Я еще не настолько опустилась!"
    # "И, надеюсь, этого не произойдет НИКОГДА!"
    # help "Требуется [monicaWhoringClothPylonDanceCorruptionRequired] corruption"
    mt "Я не могу себе этого позволить!"
    "Я еще не настолько опустилась!"
    "И, надеюсь, этого не произойдет НИКОГДА!"
    help "Требуется [monicaWhoringClothPylonDanceCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:149
translate english citizen4_dialogue_pilon_loop4_d3108dd3:

    # m "Хорошо, только не долго."
    m "Хорошо, только не долго."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:152
translate english citizen4_dialogue_pilon_loop4_d65a7faa:

    # citizen4 "Сойдет. У меня есть знакомая стриптизерша. Если хочешь, могу вас познакомить."
    # "Уж она то научит тебя всему."
    citizen4 "Сойдет. У меня есть знакомая стриптизерша. Если хочешь, могу вас познакомить."
    "Уж она то научит тебя всему."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:157
translate english citizen4_dialogue_pilon_loop4_2c5e1ce2:

    # mt "И что ты за козел?!"
    mt "И что ты за козел?!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:174
translate english citizen4_dialogue_pilon_loop4_4009d169:

    # citizen4 "А теперь покажи своих малышек, похоже им там тесно."
    # mt "Как он может так говорить о моей прекрасной груди?! Извращенец..."
    citizen4 "А теперь покажи своих малышек, похоже им там тесно."
    mt "Как он может так говорить о моей прекрасной груди?! Извращенец..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:178
translate english citizen4_dialogue_pilon_loop4_49aafc4d:

    # mt "Я не могу себе этого позволить!"
    # "Я еще не настолько опустилась!"
    # "И, надеюсь, этого не произойдет НИКОГДА!"
    # help "Требуется [monicaWhoringClothNakedBoobsCorruptionRequired] corruption"
    mt "Я не могу себе этого позволить!"
    "Я еще не настолько опустилась!"
    "И, надеюсь, этого не произойдет НИКОГДА!"
    help "Требуется [monicaWhoringClothNakedBoobsCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:185
translate english citizen4_dialogue_pilon_loop4_b71b7bd7:

    # m "Так и быть, только руками не трогать."
    # mt "Только попробуй к ним прикоснуться и я сломаю тебе пальцы."
    m "Так и быть, только руками не трогать."
    mt "Только попробуй к ним прикоснуться и я сломаю тебе пальцы."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:189
translate english citizen4_dialogue_pilon_loop4_f42f201c:

    # citizen4 "А ничего такие сиськи, но я видел и лучше."
    citizen4 "А ничего такие сиськи, но я видел и лучше."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:191
translate english citizen4_dialogue_pilon_loop4_93f3cbe1:

    # mt "Очень сомневаюсь..."
    mt "Очень сомневаюсь..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:198
translate english citizen4_dialogue_pilon_loop4_b60d294d:

    # citizen4 "Показывай сиськи, только не забудь все снять."
    # mt "Урод..."
    citizen4 "Показывай сиськи, только не забудь все снять."
    mt "Урод..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:202
translate english citizen4_dialogue_pilon_loop4_49aafc4d_1:

    # mt "Я не могу себе этого позволить!"
    # "Я еще не настолько опустилась!"
    # "И, надеюсь, этого не произойдет НИКОГДА!"
    # help "Требуется [monicaWhoringClothNakedBoobsCorruptionRequired] corruption"
    mt "Я не могу себе этого позволить!"
    "Я еще не настолько опустилась!"
    "И, надеюсь, этого не произойдет НИКОГДА!"
    help "Требуется [monicaWhoringClothNakedBoobsCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:209
translate english citizen4_dialogue_pilon_loop4_7dec49ad:

    # m "Так и быть, только руками не трогать."
    m "Так и быть, только руками не трогать."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:212
translate english citizen4_dialogue_pilon_loop4_7c87a2f3:

    # citizen4 "Должен признать, твои сиськи хороши."
    # mt "Ну еще бы!"
    # citizen4 "Хотя страшно подумать, что с ними станет лет через 10..."
    # mt "Что?"
    # citizen4 "Знаешь, мой знакоый может тебе помочь кое в чем... За каких нибудь 500$ твои подруги наберут вес."
    # citizen4 "Тебе дать его номер?"
    citizen4 "Должен признать, твои сиськи хороши."
    mt "Ну еще бы!"
    citizen4 "Хотя страшно подумать, что с ними станет лет через 10..."
    mt "Что?"
    citizen4 "Знаешь, мой знакоый может тебе помочь кое в чем... За каких нибудь 500$ твои подруги наберут вес."
    citizen4 "Тебе дать его номер?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:219
translate english citizen4_dialogue_pilon_loop4_1_90a5df7d:

    # m "Меня это не интересует! С тебя хватит!"
    # citizen4 "Погоди минутку. Потряси ка своими сочными сиськами!"
    m "Меня это не интересует! С тебя хватит!"
    citizen4 "Погоди минутку. Потряси ка своими сочными сиськами!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:226
translate english citizen4_dialogue_pilon_loop4_1_408e0125:

    # mt "Уже достаточно, что он вот так глазеет на меня"
    # "Хватит с него и того, что он видит."
    # help "Требуется [monicaWhoringClothNakedBoobsShakeCorruptionRequired] corruption"
    mt "Уже достаточно, что он вот так глазеет на меня"
    "Хватит с него и того, что он видит."
    help "Требуется [monicaWhoringClothNakedBoobsShakeCorruptionRequired] corruption"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:230
translate english citizen4_dialogue_pilon_loop4_1_745b11c5:

    # m "Ладно."
    m "Ладно."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:236
translate english citizen4_dialogue_pilon_loop4_1_4258b7e6:

    # citizen4 "О да! Теперь мне еще больше хочется их потрогать!"
    # m "Даже не думай!"
    citizen4 "О да! Теперь мне еще больше хочется их потрогать!"
    m "Даже не думай!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:240
translate english citizen4_dialogue_pilon_loop4_1_e63fe1be:

    # m "Не собираюсь, и так достаточно."
    # citizen4 "Как хочешь. Похоже, мне придется найти другую шлюху."
    m "Не собираюсь, и так достаточно."
    citizen4 "Как хочешь. Похоже, мне придется найти другую шлюху."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:258
translate english citizen4_dialogue_pilon_loop4_1_b29e8d1c:

    # citizen4 "А ты ничего такая, надо будет вернуться к нашему знакомству. Держи, заслужила."
    citizen4 "А ты ничего такая, надо будет вернуться к нашему знакомству. Держи, заслужила."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:261
translate english citizen4_dialogue_pilon_loop4_1_ea17d522:

    # m "Что?! Так мало? Мог бы дать и больше!"
    # mt "Ну ничего, скоро я стану богатой и верну свою жизнь..."
    m "Что?! Так мало? Мог бы дать и больше!"
    mt "Ну ничего, скоро я стану богатой и верну свою жизнь..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:266
translate english citizen4_dialogue_pilon_loop4_1_48a0c8d9:

    # citizen4 "Какое то не продуктивное вышло знакомство. Надеюсь, в другой раз будет лучше."
    citizen4 "Какое то не продуктивное вышло знакомство. Надеюсь, в другой раз будет лучше."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:279
translate english citizen4_show_boobs_first_time_e55cd06e:

    # citizen4 "Хотя... Ты красивая девочка. Хочу оценить их без одежды."
    # "Я недавно провернул одно дельце и у меня есть лишние 50$. Что скажешь?"
    citizen4 "Хотя... Ты красивая девочка. Хочу оценить их без одежды."
    "Я недавно провернул одно дельце и у меня есть лишние 50$. Что скажешь?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:292
translate english citizen4_show_boobs_first_time_4d8fb1d0:

    # m "Да за кого ты меня принимаешь?!"
    # m "Этого не будет никогда!"
    m "Да за кого ты меня принимаешь?!"
    m "Этого не будет никогда!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:295
translate english citizen4_show_boobs_first_time_e07e6d72:

    # citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."
    citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:300
translate english citizen4_show_boobs_first_time_9914d493:

    # mt "50$ были бы не лишними... Хорошо, что здесь никого нет."
    mt "50$ были бы не лишними... Хорошо, что здесь никого нет."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:303
translate english citizen4_show_boobs_first_time_feded4a2:

    # m "50$ ?"
    m "50$ ?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:305
translate english citizen4_show_boobs_first_time_c253e880:

    # citizen4 "Все верно, девочка. Покажешь своих подружек и они твои."
    citizen4 "Все верно, девочка. Покажешь своих подружек и они твои."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:308
translate english citizen4_show_boobs_first_time_742c6916:

    # mt "Черт! Моника, ты уверена что станешь делать это???"
    # mt "Станешь показывать свою грудь какому-то нищему за жалкие 50$?"
    mt "Черт! Моника, ты уверена что станешь делать это???"
    mt "Станешь показывать свою грудь какому-то нищему за жалкие 50$?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:312
translate english citizen4_show_boobs_first_time_43678127:

    # mt "Но, с другой стороны, это же не я показываю грудь, а какая-то шлюха в трущобах."
    # mt "Ведь никто даже представить себе не может что это делает Моника Бакфетт."
    # mt "Это как какая-то виртуальная игра, в которой все не по настоящему..."
    # mt "Но вот 50$, которые я получу, вполне реальны!"
    mt "Но, с другой стороны, это же не я показываю грудь, а какая-то шлюха в трущобах."
    mt "Ведь никто даже представить себе не может что это делает Моника Бакфетт."
    mt "Это как какая-то виртуальная игра, в которой все не по настоящему..."
    mt "Но вот 50$, которые я получу, вполне реальны!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:319
translate english citizen4_show_boobs_first_time_1ee1646f:

    # m "Ладно, только не трогать!"
    m "Ладно, только не трогать!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:321
translate english citizen4_show_boobs_first_time_64cc2ce5:

    # citizen4 "Об этом не волнуйся, детка. Я не хочу проблем с твоим сутенером."
    citizen4 "Об этом не волнуйся, детка. Я не хочу проблем с твоим сутенером."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:324
translate english citizen4_show_boobs_first_time_afbf5733:

    # mt "Да за кого он меня принимает?"
    mt "Да за кого он меня принимает?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:327
translate english citizen4_show_boobs_first_time_cea02102:

    # m "Хорошо, давай деньги."
    m "Хорошо, давай деньги."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:331
translate english citizen4_show_boobs_first_time_83dd791e:

    # citizen4 "Только после того, как покажешь."
    # citizen4 "Деньги вперед я могу дать только твоему сутенеру!"
    # citizen4 "Извини, но таким девочкам как ты я на слово не верю..."
    citizen4 "Только после того, как покажешь."
    citizen4 "Деньги вперед я могу дать только твоему сутенеру!"
    citizen4 "Извини, но таким девочкам как ты я на слово не верю..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:340
translate english citizen4_show_boobs_first_time_d6109ac7:

    # m "!!!"
    # m "Отвернись!"
    m "!!!"
    m "Отвернись!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:361
translate english citizen4_show_boobs_first_time_e6a24dbc:

    # citizen4 "Эй! Куда ты их спрятала?!"
    citizen4 "Эй! Куда ты их спрятала?!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:364
translate english citizen4_show_boobs_first_time_c906aaf6:

    # m "Я показала тебе свою грудь! Давай деньги!"
    # citizen4 "Ты что, пошутила?! Я ничего не успел рассмотреть!"
    # m "Ты увидел достаточно! Давай деньги, скорее!"
    m "Я показала тебе свою грудь! Давай деньги!"
    citizen4 "Ты что, пошутила?! Я ничего не успел рассмотреть!"
    m "Ты увидел достаточно! Давай деньги, скорее!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:369
translate english citizen4_show_boobs_first_time_defc2759:

    # citizen4 "Я не дам тебе ничего, пока ты не покажешь мне грудь нормально!"
    # citizen4 "Я хочу рассмотреть ее как следует! Иначе никаких денег!"
    citizen4 "Я не дам тебе ничего, пока ты не покажешь мне грудь нормально!"
    citizen4 "Я хочу рассмотреть ее как следует! Иначе никаких денег!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:373
translate english citizen4_show_boobs_first_time_29a4f6ab:

    # m "!!!"
    m "!!!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:382
translate english citizen4_show_boobs_first_time_50ea7d20:

    # m "Да за кого ты меня принимаешь?!"
    # citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."
    m "Да за кого ты меня принимаешь?!"
    citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:395
translate english citizen4_show_boobs_first_time_5eaa9c3c:

    # citizen4 "Эй! Хватит кривляться! Покажи грудь нормально!"
    citizen4 "Эй! Хватит кривляться! Покажи грудь нормально!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:398
translate english citizen4_show_boobs_first_time_6e80a8a3:

    # citizen4 "Покажи грудь, а я досчитаю до пяти!"
    # citizen4 "Если ты уберешь грудь раньше, то не получишь никаких денег!"
    citizen4 "Покажи грудь, а я досчитаю до пяти!"
    citizen4 "Если ты уберешь грудь раньше, то не получишь никаких денег!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:402
translate english citizen4_show_boobs_first_time_097e8b2d:

    # m "..."
    # citizen4 "..."
    # m "..."
    # citizen4 "50$!"
    m "..."
    citizen4 "..."
    m "..."
    citizen4 "50$!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:414
translate english citizen4_show_boobs_first_time_50ea7d20_1:

    # m "Да за кого ты меня принимаешь?!"
    # citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."
    m "Да за кого ты меня принимаешь?!"
    citizen4 "Тогда мои 50$ достанутся более сговорчивой девочке."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:425
translate english citizen4_show_boobs_first_time_bb4d3628:

    # citizen4 "Один..."
    citizen4 "Один..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:428
translate english citizen4_show_boobs_first_time_f3d2aa59:

    # citizen4 "Два..."
    citizen4 "Два..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:431
translate english citizen4_show_boobs_first_time_58c84d59:

    # citizen4 "Три..."
    citizen4 "Три..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:443
translate english citizen4_show_boobs_first_time_168aba37:

    # citizen4 "Четыре..."
    citizen4 "Четыре..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:450
translate english citizen4_show_boobs_first_time_460141bf:

    # citizen4 "Пять..."
    citizen4 "Пять..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:455
translate english citizen4_show_boobs_first_time_3b75f582:

    # m "Доволен?"
    # citizen4 "Более чем..."
    m "Доволен?"
    citizen4 "Более чем..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:473
translate english citizen4_show_boobs_regular_time_5521c66a:

    # citizen4 "А теперь покажи своих малышек, похоже им там тесно."
    citizen4 "А теперь покажи своих малышек, похоже им там тесно."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:476
translate english citizen4_show_boobs_regular_time_9825bf93:

    # mt "Мне надо как-то получить больше денег с этого бродяги..."
    # mt "Может быть показать ему снова мою грудь?"
    # mt "Он ведь уже видел ее..."
    mt "Мне надо как-то получить больше денег с этого бродяги..."
    mt "Может быть показать ему снова мою грудь?"
    mt "Он ведь уже видел ее..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:481
translate english citizen4_show_boobs_regular_time_1ddb90ac:

    # m "Если ты хочешь, я могу показать тебе свою грудь..."
    # m "Снова..."
    m "Если ты хочешь, я могу показать тебе свою грудь..."
    m "Снова..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:484
translate english citizen4_show_boobs_regular_time_c9325300:

    # citizen4 "Конечно! Я соскучился по твоим малышкам!"
    citizen4 "Конечно! Я соскучился по твоим малышкам!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:486
translate english citizen4_show_boobs_regular_time_fdfa89c4:

    # m "50$!"
    m "50$!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:488
translate english citizen4_show_boobs_regular_time_45ccfdf5:

    # citizen4 "Что? Какие еще 50$?"
    # citizen4 "Я уже видел твоих малышек!"
    # citizen4 "Хочешь снова 50$ - покажи мне другие!"
    # citizen4 "Ты можешь привести кого-нибудь и поделить с ней деньги."
    # citizen4 "Но учти, я уже видел почти всех малышек в этом районе!"
    # citizen4 "За малышек, которые я уже видел, я готов заплатить [monicaWhoringNakedBoobsMoney]$!"
    citizen4 "Что? Какие еще 50$?"
    citizen4 "Я уже видел твоих малышек!"
    citizen4 "Хочешь снова 50$ - покажи мне другие!"
    citizen4 "Ты можешь привести кого-нибудь и поделить с ней деньги."
    citizen4 "Но учти, я уже видел почти всех малышек в этом районе!"
    citizen4 "За малышек, которые я уже видел, я готов заплатить [monicaWhoringNakedBoobsMoney]$!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:498
translate english citizen4_show_boobs_regular_time_1cb5edc2:

    # m "Мне некого приводить!"
    # m "И моя грудь самая лучшая!"
    # mt "Черт! Все мировые издания об этом писали!"
    m "Мне некого приводить!"
    m "И моя грудь самая лучшая!"
    mt "Черт! Все мировые издания об этом писали!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:503
translate english citizen4_show_boobs_regular_time_4147e877:

    # citizen4 "Я готов заплатить тебе [monicaWhoringNakedBoobsMoney]$ и не более..."
    citizen4 "Я готов заплатить тебе [monicaWhoringNakedBoobsMoney]$ и не более..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:509
translate english citizen4_show_boobs_regular_time_ef3db6ad:

    # m "Да за кого ты меня принимаешь?!"
    m "Да за кого ты меня принимаешь?!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:521
translate english citizen4_show_boobs_regular_time_7dec49ad:

    # m "Так и быть, только руками не трогать."
    m "Так и быть, только руками не трогать."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:524
translate english citizen4_show_boobs_regular_time_9dde3e16:

    # mt "Только попробуй к ним прикоснуться и я сломаю тебе пальцы."
    mt "Только попробуй к ним прикоснуться и я сломаю тебе пальцы."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:527
translate english citizen4_show_boobs_regular_time_f42f201c:

    # citizen4 "А ничего такие сиськи, но я видел и лучше."
    citizen4 "А ничего такие сиськи, но я видел и лучше."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:530
translate english citizen4_show_boobs_regular_time_93f3cbe1:

    # mt "Очень сомневаюсь..."
    mt "Очень сомневаюсь..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:540
translate english citizen4_show_boobs_regular_time_9825bf93_1:

    # mt "Мне надо как-то получить больше денег с этого бродяги..."
    # mt "Может быть показать ему снова мою грудь?"
    # mt "Он ведь уже видел ее..."
    mt "Мне надо как-то получить больше денег с этого бродяги..."
    mt "Может быть показать ему снова мою грудь?"
    mt "Он ведь уже видел ее..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:545
translate english citizen4_show_boobs_regular_time_1ddb90ac_1:

    # m "Если ты хочешь, я могу показать тебе свою грудь..."
    # m "Снова..."
    m "Если ты хочешь, я могу показать тебе свою грудь..."
    m "Снова..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:548
translate english citizen4_show_boobs_regular_time_70a19bf3:

    # citizen4 "Сегодня я не хочу смотреть твоих малышек!"
    # citizen4 "Покажи их кому-нибудь другому!"
    # citizen4 "Уверен, ты сможешь заработать [monicaWhoringNakedBoobsMoney]$."
    citizen4 "Сегодня я не хочу смотреть твоих малышек!"
    citizen4 "Покажи их кому-нибудь другому!"
    citizen4 "Уверен, ты сможешь заработать [monicaWhoringNakedBoobsMoney]$."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:560
translate english citizen4_dialogue_after_boobs_first_time_8574f688:

    # mt "Дьявол!"
    # mt "Я не уверена что мне стоило делать это!"
    # mt "Я думала это будет проще!"
    # mt "Этот урод заставил меня пол дня стоять на улице, сверкая своей обнаженной бесценной, очаровательной грудью!"
    # mt "Но за эти же деньги мне пришлось бы два месяца носить рекламу кебаба..."
    # mt "Главное - это то что меня никто не видел."
    # mt "И никто не знает кто я на самом деле, так что я могу делать все что потребуется."
    # mt "Мне пришлось показать грудь - пускай!"
    # mt "Это поможет мне сэкономить время на поиск еды, чтобы потратить его на решение моих главных проблем."
    # mt "Моя цель - это вернуть назад мою роскошную жизнь."
    # mt "И я не остановлюсь ни перед чем!"
    mt "Дьявол!"
    mt "Я не уверена что мне стоило делать это!"
    mt "Я думала это будет проще!"
    mt "Этот урод заставил меня пол дня стоять на улице, сверкая своей обнаженной бесценной, очаровательной грудью!"
    mt "Но за эти же деньги мне пришлось бы два месяца носить рекламу кебаба..."
    mt "Главное - это то что меня никто не видел."
    mt "И никто не знает кто я на самом деле, так что я могу делать все что потребуется."
    mt "Мне пришлось показать грудь - пускай!"
    mt "Это поможет мне сэкономить время на поиск еды, чтобы потратить его на решение моих главных проблем."
    mt "Моя цель - это вернуть назад мою роскошную жизнь."
    mt "И я не остановлюсь ни перед чем!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:576
translate english citizen4_dialogue_after_boobs_second_time_234f2bb7:

    # mt "Вот мерзавец!"
    # mt "Он отказался смотреть на мою грудь даже за [monicaWhoringNakedBoobsMoney]$ !"
    # mt "Может быть показать мою грудь кому-нибудь еще?"
    # mt "Это звучит дико, но мне нужны деньги..."
    # mt "Похоже, в этом районе все относятся совершенно нормально к таким вещам..."
    # mt "А я нахожусь здесь анонимно..."
    # mt "То что здесь происходит никак не связано с жизнью Моники Бакфетт."
    # mt "Так что мне нечего стесняться."
    # mt "Мне это глубоко противно, но я отношусь к этому с хладнокровием."
    # mt "В конце концов, это ненадолго."
    mt "Вот мерзавец!"
    mt "Он отказался смотреть на мою грудь даже за [monicaWhoringNakedBoobsMoney]$ !"
    mt "Может быть показать мою грудь кому-нибудь еще?"
    mt "Это звучит дико, но мне нужны деньги..."
    mt "Похоже, в этом районе все относятся совершенно нормально к таким вещам..."
    mt "А я нахожусь здесь анонимно..."
    mt "То что здесь происходит никак не связано с жизнью Моники Бакфетт."
    mt "Так что мне нечего стесняться."
    mt "Мне это глубоко противно, но я отношусь к этому с хладнокровием."
    mt "В конце концов, это ненадолго."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:593
translate english cit4_naked_boobs_dance_1st_65421f38:

    # citizen4 "Я бы посмотрел на твои сиськи еще разок, но в танце. Что скажешь?"
    citizen4 "Я бы посмотрел на твои сиськи еще разок, но в танце. Что скажешь?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:595
translate english cit4_naked_boobs_dance_1st_7920ed99:

    # m "Не слишком ли многого ты хочешь?!"
    m "Не слишком ли многого ты хочешь?!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:597
translate english cit4_naked_boobs_dance_1st_70c6c360:

    # citizen4 "Девочка, ты забыла где ты? В этом районе каждая вторая готова таким образом заработать пару лишних долларов."
    # citizen4 "Решайся быстрее, у меня нет времени. Ну дак что?"
    citizen4 "Девочка, ты забыла где ты? В этом районе каждая вторая готова таким образом заработать пару лишних долларов."
    citizen4 "Решайся быстрее, у меня нет времени. Ну дак что?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:605
translate english cit4_naked_boobs_dance_1st_a885f5b2:

    # m "Хватит с тебя того, что ты уже видел!"
    m "Хватит с тебя того, что ты уже видел!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:608
translate english cit4_naked_boobs_dance_1st_f4965ca3:

    # mt "Я уже танцевала, мою прекрасную грудь он видел... Не так страшно, если все это будет вместе."
    # mt "Хуже уже не будет..."
    mt "Я уже танцевала, мою прекрасную грудь он видел... Не так страшно, если все это будет вместе."
    mt "Хуже уже не будет..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:611
translate english cit4_naked_boobs_dance_1st_b6b83365:

    # m "Хорошо. Только отвернись!"
    m "Хорошо. Только отвернись!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:614
translate english cit4_naked_boobs_dance_1st_d0978175:

    # citizen4 "Ладно, только в этот раз..."
    citizen4 "Ладно, только в этот раз..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:620
translate english cit4_naked_boobs_dance_1st_c1f95314:

    # citizen4 "Залезай уже на пилон и начинай!"
    citizen4 "Залезай уже на пилон и начинай!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:622
translate english cit4_naked_boobs_dance_1st_61ceb40b:

    # m "Почему Вы такой грубый?"
    m "Почему Вы такой грубый?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:624
translate english cit4_naked_boobs_dance_1st_995b8014:

    # citizen4 "Я нормальный, да и не твое это дело! Твое дело крутить жопой на пилоне! Начинай!"
    citizen4 "Я нормальный, да и не твое это дело! Твое дело крутить жопой на пилоне! Начинай!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:631
translate english cit4_naked_boobs_dance_1st_6766db94:

    # m "Знаете, я передумала!"
    m "Знаете, я передумала!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:634
translate english cit4_naked_boobs_dance_1st_f62dbd3e:

    # m "Хорошо."
    # mt "Мне нужны эти деньги... Черт..."
    m "Хорошо."
    mt "Мне нужны эти деньги... Черт..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:640
translate english cit4_naked_boobs_dance_1st_4c18ce6d:

    # citizen4 "А знаешь, не плохо!"
    citizen4 "А знаешь, не плохо!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:647
translate english cit4_naked_boobs_dance_1st_784cce77:

    # citizen4 "Жопу бы тебе побольше!"
    citizen4 "Жопу бы тебе побольше!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:652
translate english cit4_naked_boobs_dance_1st_5fc686fe:

    # citizen4 "Да, очень не дурно."
    citizen4 "Да, очень не дурно."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:657
translate english cit4_naked_boobs_dance_1st_b04819e1:

    # citizen4 "Знаешь, у тебя есть будущее, но надо больше практики и накачай жопу!"
    citizen4 "Знаешь, у тебя есть будущее, но надо больше практики и накачай жопу!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:660
translate english cit4_naked_boobs_dance_1st_bf617ce0:

    # citizen4 "Вот держи, заработала!"
    # mt "!!!"
    citizen4 "Вот держи, заработала!"
    mt "!!!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:668
translate english cit4_naked_boobs_dance_variant1_35710c7e:

    # citizen4 "Думаю, стоит закрепить начатое. Полезай на пилон. И блузку сними!"
    citizen4 "Думаю, стоит закрепить начатое. Полезай на пилон. И блузку сними!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:670
translate english cit4_naked_boobs_dance_variant1_9af81142:

    # m "Зачем мне снимать блузку, если ты не хочешь смотреть на мою грудь?"
    m "Зачем мне снимать блузку, если ты не хочешь смотреть на мою грудь?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:672
translate english cit4_naked_boobs_dance_variant1_e5d71a7a:

    # citizen4 "Это не твое дело! Я говорю, ты делаешь."
    # citizen4 "Ну дак что?"
    citizen4 "Это не твое дело! Я говорю, ты делаешь."
    citizen4 "Ну дак что?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:680
translate english cit4_naked_boobs_dance_variant1_a885f5b2:

    # m "Хватит с тебя того, что ты уже видел!"
    m "Хватит с тебя того, что ты уже видел!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:683
translate english cit4_naked_boobs_dance_variant1_b6b83365:

    # m "Хорошо. Только отвернись!"
    m "Хорошо. Только отвернись!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:686
translate english cit4_naked_boobs_dance_variant1_a4066f0b:

    # citizen4 "Все равно мне не интересны твои сиськи."
    citizen4 "Все равно мне не интересны твои сиськи."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:688
translate english cit4_naked_boobs_dance_variant1_7279e034:

    # mt "Как он может такое говорить? Моя прекрасная грудь много кого интересует!"
    mt "Как он может такое говорить? Моя прекрасная грудь много кого интересует!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:691
translate english cit4_naked_boobs_dance_variant1_c1f95314:

    # citizen4 "Залезай уже на пилон и начинай!"
    citizen4 "Залезай уже на пилон и начинай!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:693
translate english cit4_naked_boobs_dance_variant1_94420ba9:

    # m "Хорошо."
    m "Хорошо."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:698
translate english cit4_naked_boobs_dance_variant1_1836d0b0:

    # citizen4 "Мне кажется, я начинаю любить твои сиськи!"
    citizen4 "Мне кажется, я начинаю любить твои сиськи!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:703
translate english cit4_naked_boobs_dance_variant1_0d98331f:

    # citizen4 "Да, вот так, двигайся!"
    citizen4 "Да, вот так, двигайся!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:708
translate english cit4_naked_boobs_dance_variant1_3513b231:

    # citizen4 "Знаешь, все равно что-то не то. Кое чего не хватает."
    citizen4 "Знаешь, все равно что-то не то. Кое чего не хватает."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:722
translate english cit4_naked_boobs_dance_variant1_6fab9fb3:

    # citizen4 "Черт! Ты меня ударила!"
    citizen4 "Черт! Ты меня ударила!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:725
translate english cit4_naked_boobs_dance_variant1_23653138:

    # m "Вообще то я не хотела, ты подошел слишком близко!"
    m "Вообще то я не хотела, ты подошел слишком близко!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:727
translate english cit4_naked_boobs_dance_variant1_cc50c89e:

    # citizen4 "Ты мне сломала нос!"
    citizen4 "Ты мне сломала нос!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:729
translate english cit4_naked_boobs_dance_variant1_a32a8dcc:

    # m "Не правда, у тебя даже крови нет. В следующий раз не подходи близко."
    # mt "Иначе я сделаю это специально!"
    m "Не правда, у тебя даже крови нет. В следующий раз не подходи близко."
    mt "Иначе я сделаю это специально!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:732
translate english cit4_naked_boobs_dance_variant1_871671e8:

    # citizen4 "Ха-ха-ха! Девочка начинает входить во вкус?"
    # citizen4 "Следующий раз..."
    citizen4 "Ха-ха-ха! Девочка начинает входить во вкус?"
    citizen4 "Следующий раз..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:735
translate english cit4_naked_boobs_dance_variant1_7b787145:

    # m "Плати!"
    m "Плати!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:737
translate english cit4_naked_boobs_dance_variant1_48a6b115:

    # citizen4 "Да не вопрос!"
    citizen4 "Да не вопрос!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:739
translate english cit4_naked_boobs_dance_variant1_08347811:

    # mt "Все это толко для того, чтобы заработать немного денег. Ведь так, Моника?"
    mt "Все это толко для того, чтобы заработать немного денег. Ведь так, Моника?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:745
translate english cit4_naked_boobs_dance_variant2_35710c7e:

    # citizen4 "Думаю, стоит закрепить начатое. Полезай на пилон. И блузку сними!"
    citizen4 "Думаю, стоит закрепить начатое. Полезай на пилон. И блузку сними!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:747
translate english cit4_naked_boobs_dance_variant2_e2446fe8:

    # m "Зачем мне снимать кофту, если ты не хочешь смотреть на мою грудь?"
    m "Зачем мне снимать кофту, если ты не хочешь смотреть на мою грудь?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:749
translate english cit4_naked_boobs_dance_variant2_e5d71a7a:

    # citizen4 "Это не твое дело! Я говорю, ты делаешь."
    # citizen4 "Ну дак что?"
    citizen4 "Это не твое дело! Я говорю, ты делаешь."
    citizen4 "Ну дак что?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:757
translate english cit4_naked_boobs_dance_variant2_a885f5b2:

    # m "Хватит с тебя того, что ты уже видел!"
    m "Хватит с тебя того, что ты уже видел!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:760
translate english cit4_naked_boobs_dance_variant2_b6b83365:

    # m "Хорошо. Только отвернись!"
    m "Хорошо. Только отвернись!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:763
translate english cit4_naked_boobs_dance_variant2_a4066f0b:

    # citizen4 "Все равно мне не интересны твои сиськи."
    citizen4 "Все равно мне не интересны твои сиськи."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:765
translate english cit4_naked_boobs_dance_variant2_7279e034:

    # mt "Как он может такое говорить? Моя прекрасная грудь много кого интересует!"
    mt "Как он может такое говорить? Моя прекрасная грудь много кого интересует!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:768
translate english cit4_naked_boobs_dance_variant2_c1f95314:

    # citizen4 "Залезай уже на пилон и начинай!"
    citizen4 "Залезай уже на пилон и начинай!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:770
translate english cit4_naked_boobs_dance_variant2_94420ba9:

    # m "Хорошо."
    m "Хорошо."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:775
translate english cit4_naked_boobs_dance_variant2_1836d0b0:

    # citizen4 "Мне кажется, я начинаю любить твои сиськи!"
    citizen4 "Мне кажется, я начинаю любить твои сиськи!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:780
translate english cit4_naked_boobs_dance_variant2_0d98331f:

    # citizen4 "Да, вот так, двигайся!"
    citizen4 "Да, вот так, двигайся!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:787
translate english cit4_naked_boobs_dance_variant2_3513b231:

    # citizen4 "Знаешь, все равно что-то не то. Кое чего не хватает."
    citizen4 "Знаешь, все равно что-то не то. Кое чего не хватает."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:797
translate english cit4_naked_boobs_dance_variant2_e502a163:

    # citizen4 "Вот чего не хватает! Моей руки на твоей сочной жопе!"
    citizen4 "Вот чего не хватает! Моей руки на твоей сочной жопе!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:799
translate english cit4_naked_boobs_dance_variant2_c051171b:

    # m "Какого черта? Отпути меня!"
    m "Какого черта? Отпути меня!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:801
translate english cit4_naked_boobs_dance_variant2_36243c5c:

    # citizen4 "Не, мне нравится твой зад!"
    citizen4 "Не, мне нравится твой зад!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:803
translate english cit4_naked_boobs_dance_variant2_93e0dd07:

    # mt "Черт, если я отпущу пилон, он может меня отпустить и я упаду... Что делать?"
    mt "Черт, если я отпущу пилон, он может меня отпустить и я упаду... Что делать?"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:817
translate english cit4_naked_boobs_dance_variant2_7594db58:

    # citizen4 "Ха-ха-ха! Глупая девочка!"
    citizen4 "Ха-ха-ха! Глупая девочка!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:821
translate english cit4_naked_boobs_dance_variant2_a9b7b8e1:

    # m "Знаешь что! Если ты меня не отпустишь, у тебя будут проблемы!"
    m "Знаешь что! Если ты меня не отпустишь, у тебя будут проблемы!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:823
translate english cit4_naked_boobs_dance_variant2_e0406624:

    # citizen4 "Девочка, ты не в тех условиях, чтобы угрожать!"
    # citizen4 "Погоди минутку..."
    citizen4 "Девочка, ты не в тех условиях, чтобы угрожать!"
    citizen4 "Погоди минутку..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:827
translate english cit4_naked_boobs_dance_variant2_a29dc817:

    # citizen4 "О да, отличный зад!"
    citizen4 "О да, отличный зад!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:830
translate english cit4_naked_boobs_dance_variant2_6d654a59:

    # citizen4 "Эх, маловат, но вполне годный!"
    citizen4 "Эх, маловат, но вполне годный!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:835
translate english cit4_naked_boobs_dance_variant2_9e8e68c1:

    # mt "Похоже, у меня нет выбора..."
    mt "Похоже, у меня нет выбора..."

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:838
translate english cit4_naked_boobs_dance_variant2_a29dc817_1:

    # citizen4 "О да, отличный зад!"
    citizen4 "О да, отличный зад!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:841
translate english cit4_naked_boobs_dance_variant2_6d654a59_1:

    # citizen4 "Эх, маловат, но вполне годный!"
    citizen4 "Эх, маловат, но вполне годный!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:845
translate english cit4_naked_boobs_dance_variant2_8fb8bc5e:

    # m "Я тебя убью!"
    m "Я тебя убью!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:847
translate english cit4_naked_boobs_dance_variant2_03c481b9:

    # citizen4 "Началось... Вот твои два бакса, заработала."
    # citizen4 "И вот еще один, за твой красивый зад!"
    citizen4 "Началось... Вот твои два бакса, заработала."
    citizen4 "И вот еще один, за твой красивый зад!"

# game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:851
translate english cit4_naked_boobs_dance_variant2_cccfe7b4:

    # mt "Когда нибудь ты за это ответишь, я обещаю!"
    mt "Когда нибудь ты за это ответишь, я обещаю!"

translate english strings:

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:32
    old "У меня нет желания знакомиться с Вами!"
    new "У меня нет желания знакомиться с Вами!"

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:32
    old "Что Вы подразумеваете под знакомством?"
    new "Что Вы подразумеваете под знакомством?"

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:281
    old "Согласиться..."
    new "Согласиться..."

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:281
    old "Согласиться... (low corruption, required: [monicaWhoringClothNakedBoobsCorruptionRequired], 20 customers served) (disabled)"
    new "Согласиться... (low corruption, required: [monicaWhoringClothNakedBoobsCorruptionRequired], 20 customers served) (disabled)"

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:281
    old "Отказаться."
    new "Отказаться."

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:805
    old "Отпустить руки."
    new "Отпустить руки."

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:805
    old "Угрожать."
    new "Угрожать."

    # game/Quests/EP2/Citizens/Citizen4_Dialogue.rpy:805
    old "Терпеть."
    new "Терпеть."

