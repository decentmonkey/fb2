...
label ep217_dialogues6_office_6:
    ...
    # добавить что наряд Моники - от Кэмбелла
    # Бифф заставляет Монику стоять спиной и проводить презентацию
    # Далее, заставляет ее сказать слова о новом курсе журнала и чтобы у инвесторов не было сомнений в ее решимости проведения нового курса, (поворачивается спиной) она одела этот чудесный наряд от мистера Кэмбелла.
    # он ей очень нравится и она хотела бы почаще одевать что-то подобное
    ...
    # и скажешь те слова, которые я заставил тебя запомнить перед приездом сюда
    ...
    m "..."
    mt "Дьявол, хорошо что на этой очередной дурацкой презентации обычные графики и нет никаких отвратительных кадров из фотосессий!"
    ...
    biff "В благодарность за ваше потраченное время, а также за перспективу спонсорства, Миссис Бакфетт решила устроить вам небольшой презент!"
    ...
    # девочки заходят, встают в ряд.
    # админша видит Стива и очень удивляется
    # Стив в ответ улыбается своей улыбочкой (лыбится)
    # админ подходит к девочкам и говорит им: приступайте к работе!
    # девочки быстро разбредаются по инвесторам.
    # Линда к своему, начинают что-то говорить. Ну привет милый, я уже скучала
    # Красавчик, давай с тобой сегодня развлечемся
    # Линда садится на колени
    # Кто-то садится на ручку кресла
    # Кто-то еще как-то
    mt "Это провал!!!"
    mt "!!!"
    ...
    philip "А другая половина не знает что вы работаете в этом отеле в эскорте и должны обслуживать гостей в этот вечер."
    m "!!!"
    philip "Но, если вы отказываетесь, я готов выбрать другую девушку..."
    philip "Так что вы решили?"
    menu:
        "Уехать с Филиппом.":
            pass
    mt "Это провал! Похоже, единственный путь выбраться отсюда и сохранить репутацию, это согласиться поехать с этим мерзавцем!"
    mt "Если я попытаюсь уйти сама, эта чертова администратор поднимет шум и тогда всем станет известно про то, что я имею отношение к этому месту!"
    m "..."
    m "Филипп, я... Я поеду с тобой..."
    m "Только забери меня скорее из этого кошмара, я не хочу иметь к происходящему здесь никакого отношения!"
    # затемнение, лифт
