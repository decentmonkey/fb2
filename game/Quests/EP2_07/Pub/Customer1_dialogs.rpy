...
label customer1_afterbattle:
    ...
    menu:
        "Быть вежливой.":
            ...
            # Мон улыбается своим мыслям
            customer1 "Вот, держи немного чаевых за твою прекрасную улыбку."
            return
        "Ответить грубо":
            # Мон возмущенно
...
# Показать грудь или потрогать ее (не обнажая)
# Все время оглядывается что никто не смотрит
# Задирает юбку сзади
# Приспускает трусики
# Спереди покзывает киску, приспуская трусики (быстро)
# Моника стоит и принмает заказ, а рука клиента под юбкой
# потрогать ножку
# поцеловать (поцелуй от королевы)

# Эшли говорит что ты теперь королева и могла бы обслуживать клиентов получше
