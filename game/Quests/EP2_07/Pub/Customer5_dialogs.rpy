...
label customer5_afterbattle:
    customer5 "О! Это ты!"
    customer5 " Повернись ко мне своей сладкой покой, хочу с ней поздороваться!"
    mt "Мерзкой извращенец!"
    m  "Желаете сделать заказ?"
    customer5 "Я желаю посмотреть на твою попку!"
    ...
    menu:
        "Принести заказ, как обычно.":
            # на лице мон отвращение и раздраженность, она оскорблена
            m "Я принесу ваш заказ, но без виляний и подмигиваний."
            return
        "$ 50? Хммм...":
            mt "50 $ за то что бы пройтись и подмигнуть, не самая плохая сделка.":
            mt "Боже, Моника ты готова выслуживаться за 50 $ перед каким-то грязным пьяньчугой, да что с тобой?"
            mt "Но если на это согласится не Моника, а Королева Shiny Hole...?"
            m "Только деньги вперед..."
            # Мон удаляется за заказом виляя попой,
    ...
    menu:
        "Отшить мерзавца":
            # Мон оденет ему на голову пасту
            m "Грязный свин!"
            m "Tеперь ты выглядишь соответствуще!"
            # увольнение или он скажет что простит её за приват
            return
        "Мне нужны деньги":
            mt "100 $ звучит неплохо, я могу слегка прислониться к его члену пока ни кто не видит."
            m "Только если слегка... коснусь... не больше..."
            #customer5 "Сойдет для первого раза."
            customer5 "Давай уже, не томи..."
            # Мон слегка касается члена
            customer5 "Оооооо! Королевская жопа!!!"
            # хватает Мон за попу и та отпрыгивает
            m "Достаточно! А теперь плати!"
            customer5 "Ух, горячая! Дааа, хорошего понемножку, согласен."
            # платит Мон
            customer5 "А если я докину еще сотню, продолжим в привате?"
            # Мон раздражительно
            m "Я не танцую приваты и ты и так уже получил достаточно внимания от королевы Shiny Hole."
            customer5 "Раньше ты говорила что и на сцене не танцуешь!"
            customer5 "А теперь - Королева Shiny Hole...хы-хы-хы."
            ...
            mt "Или этот хитрый мерзавец хочет заказать приват через Джо!? Хммммм..."
            customer5 "А знаешь что, я сам к нему подойду! Так что топай..."
            m "Плати!"
            customer5 "Ой, точно! Ну, ты слегка коснулась члена...а теперь денежка слегка коснется твоей ручки."
            # проводит деньгами по руке Мон и подмигивает ей
            customer5 "А это, за сервис."
            # Дает ей пару баксов
            # Мон злиться и уходит
            # След кадр: Шлепает по попе уходящую Мон, она отпрыгивает, куксит злую моську и уходит

            return

    mt "Грязный извращенец!"
    mt "Ты!!!Нет!!!...ВЫ!!! Вы все поплатитесь!!!"
