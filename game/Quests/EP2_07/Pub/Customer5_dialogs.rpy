...
label customer5_afterbattle:
    # О! Это ты!
    # Повернись к мне своей сладкой покой, хочу с ней поздороваться!
    # mt Мерзкой извращенец!
    # m Желаете сделать заказ?
    # Я желаю посмотреть на твою попку! Неси пасту, а когда будешь идти повиляй своей раскошной задницей, я хорошо приплачу тебе за это. И подмигни мне.

    # 1) m Я принесу ваш заказ, но без виляний и подмигиваний.
    # 2) mt 50 баксов за то что бы пройтись, и подмигнуть, не самая плохая сделка. Боже, Моника ты готова выслуживаться за 50 баксов перед каким-то грязным пьяньчугой, что с тобой?
    # 2) mt Это же не Моника, а Королева SH готова пройтись за 50 $.
    # 2) m Ладно, я согласна. (Мон удаляется за заказом виляет попой , поворачивается и подмигивает.)
    # О, детка, смотри, мой "дружок" оценил твою походку. Может потрёшься о него, ...за сотню. Что скажешь?
    # 1) m Грязный свин (Мон оденет ему на голову пасту) теперь ты выглядишь соответствуще. (увольнение или он скажет что простит её за приват)
    # 2) mt 100 дол звучит неплохо, и если я незаметно прислонюсь к его члену ни кто не увидит.
    # 2) Мон трётся
    # Оооооо! королевская жопа!!!
    # хватает Мон за попу и та отпрыгивает
    # m достаточно! А теперь плати!
    # Ух, горячая! Дааа, хорошего понемножку, согласен. А если я докину ещё сотню, продолжим в привате?
    # m Я не танцую приваты и ты и так уже получил достаточно внимания от королевы SH.
    # Раньше ты говорила что и на сцене не танцуешь!
    # А знаешь что, позови Джо!
    # m Зачем вам Джо?
    # mt о боже он хочет рассказать о том что я только что сделала
    # mt или этот мерзавец хочет заказать приват через Джо
    # Ладно, сам к нему подойду, оставлю отзыв о твоей работе, скажу что ты не исполняешь просьбы гостей заведения.
    # m я позову его.
    # Шлёпает по попе уходящую Мон, она отпрыгивает, куксит злую моську и уходит за джо
    #
    return
