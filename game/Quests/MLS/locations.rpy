    ## Локации ## www.cgmodelx - купить

# ШКОЛА (три этажа):
    #COLLEGE
    # 1-й этаж:
        # школьный двор:
            # двор и фасад здания школы
            # + https://www.cgmodelx.com/model/details/39293.html
            # https://www.cgmodelx.com/model/details/40287.html
        # вход в школу
        # коридор
        # кабинет биологии
        # кабинет информатики
        # кабинет химии
        # кабинет географии
        # мед. пункт
        # туалет для мальчиков
        # туалет для девочек
        # бассейн:
            # https://www.cgmodelx.com/model/details/38465.html
        # спорт. зал
        # раздевалка для девочек
        # раздевалка для мальчиков
        # столовая:
            # - https://www.cgmodelx.com/model/details/31684.html
        # актовый зал
        # задний двор школы со спорт. площадкой
    # 2-й этаж:
        # коридор
        # кабинет английского языка и литературы
        # кабинет изо:
            # https://evermotion.org/shop/show_product/scene-9-ai21-archinteriors/5938
            # + https://www.cgmodelx.com/model/details/32345.html
            # https://www.cgmodelx.com/model/details/20208.html (мольберты)
        # кабинет алгебры и геометрии
        # кабинет физики
        # кабинет иностранных языков
        # кабинет музыки (может = актовый зал?)
    # 3-й этаж:
        # кабинет директора школы
        # приемная (секретарь директора)
        # учительская
        # школьная библиотека:
            # https://www.cgmodelx.com/model/details/38535.html
            # ? https://www.cgmodelx.com/model/details/50359.html
            # + https://www.cgmodelx.com/model/details/11032.html (убрать зеленые стены, убрать панорамные окна, убрать диваны, поставить вместо них столы со стульями)
        # хоз. помещения (н-р, спорт. инвентарь)
    # крыша школы:
        # - https://evermotion.org/shop/show_product/rubble-55-am165-archmodels/12830


# ДОМ БАРДИ (два этажа): (все локации день/ночь)
    # дом Барди:
        # + https://evermotion.org/shop/show_product/archexteriors-vol-31/14243
            # кухня:
                # + https://evermotion.org/shop/show_product/ai48-scene-6/14313
                # или https://evermotion.org/shop/show_product/archinteriors-vol-43/12555
            # гостиная:
                # - https://evermotion.org/shop/show_product/scene-9-ai30-archinteriors/10478
            # спальня родителей:
                # + https://evermotion.org/files/model_images/Cam_013_PP.jpg
                # https://evermotion.org/shop/show_product/scene-2-ai35-archinteriors/11615 =>
                # или https://evermotion.org/shop/show_product/ai48-scene-9/14316
            # комната старшей сестры:
                # + https://www.cgmodelx.com/model/details/35833.html (мебель сделать цветную, постельное тоже цветным + вид за окном)
            # комната младшей сестры:
                # взять за основу комнату старшей сестры и перенести в нее предметы из:
                # + https://evermotion.org/shop/show_product/scene-4-ai3-archinteriors/239
                # цвет изменить на детский
            # комната Барди:
                # https://www.cgmodelx.com/model/details/52649.html =>
                # + https://evermotion.org/shop/show_product/scene-9-ai35-archinteriors/11622 (дерево крыши чуть потемнее, стены светло-серые/светло-корчневые, одеяло инвертировать(полоски,чтобы было темнее), мартрац сделать серым под цвет темных полосок одеяла)
                # + https://evermotion.org/shop/show_product/baby-bedroom-props-9-am189-archmodels/14192 (старые вещи в комнату Барди)
                # + https://www.cgmodelx.com/model/details/32177.html (пробки на стену)
                # + https://www.cgmodelx.com/model/details/52227.html (на стену что-то можно взять отсюда)
            # туалет, ванная:
                # + https://evermotion.org/files/model_images/Cam_014_PP.jpg (добавить душ)
                # https://www.cgmodelx.com/model/details/36070.html
                # или https://www.cgmodelx.com/model/details/53247.html
            # лестница:
                # взять из общей локации
                # - https://evermotion.org/shop/show_product/scene-9-ai45-archinteriors/13276
            # чердак:
                # - https://evermotion.org/shop/show_product/scene-12-furniture-ai49-archinteriors/14486
                    # хлам на чердаке:
                        # https://evermotion.org/shop/show_product/set-25-am188-archmodels/14170
                        # https://evermotion.org/shop/show_product/bookshelf-50-am179-archmodels/13733
                        # https://evermotion.org/shop/show_product/baby-bedroom-props-9-am189-archmodels/14192
                        # https://evermotion.org/shop/show_product/lab-props-12-am228-archmodels/16106 - хлам на чердак или в гараж
            # гараж:
                # + https://evermotion.org/shop/show_product/archmodels-vol-222/15820 (сделать старым и темные стены)

# ДОМ ТЕТИ (два этажа) (день/ночь):
    # дом тети:
        # ???? https://evermotion.org/shop/show_product/scene-9-ae6-archexteriors/494
        # + https://evermotion.org/shop/show_product/-scene-8-ae34-archexteriors/15715
            # кухня:
                # + https://evermotion.org/shop/show_product/archinteriors-vol-8/53
            # гостиная:
                # + https://evermotion.org/shop/show_product/scene-5-ai46-archinteriors/13579
                # или https://evermotion.org/shop/show_product/scene-4-ai51-archinteriors/14956
            # спальня:
                # https://www.cgmodelx.com/model/details/51856.html
                # + https://www.cgmodelx.com/model/details/39066.html
            # комната двоюродной сестры Барди:
                # https://www.cgmodelx.com/model/details/53428.html
                # + https://www.cgmodelx.com/model/details/36436.html
                # https://evermotion.org/shop/show_product/scene-7-ai6-archinteriors/272
            # туалет, ванная комната:
                # https://evermotion.org/shop/show_product/scene-3-ai4-archinteriors/248
                # + https://www.cgmodelx.com/model/details/53247.html

# ДОМ ДРУГА БАРДИ (два этажа) (день/ночь):
    # дом друга:
        # + https://evermotion.org/shop/show_product/-scene-3-ae34-archexteriors/15710 (пальмы заменить на деревья)
            # кухня:
                # + https://evermotion.org/shop/show_product/scene-7-ai43-archinteriors/12562 (стены темные)
            # гостиная:
                # + https://evermotion.org/shop/show_product/scene-4-ai50-archinteriors/14540
            # комната друга:
                # https://evermotion.org/shop/show_product/scene-9-ai37-archinteriors/11710
                # + https://www.cgmodelx.com/model/details/54159.html
            # спальня родителей:
                # + https://www.cgmodelx.com/model/details/30232.html
            # туалет, ванная комната:
                # + https://evermotion.org/shop/show_product/scene-4-ai39-archinteriors/11936

# ДОМ ОДНОКЛАССНИЦЫ:
    # дом одноклассницы (день/ночь улица, внутри ночь):
        # ? https://evermotion.org/shop/show_product/-scene-8-ae34-archexteriors/15715
            # кухня и гостиная:
                # + https://evermotion.org/shop/show_product/scene-4-ai54-archinteriors/15922
            # комната одноклассницы:
                # + https://www.cgmodelx.com/model/details/29324.html
            # спальня родителей:
                # + https://www.cgmodelx.com/model/details/36028.html
            # туалет, ванная комната:
                # + https://evermotion.org/shop/show_product/scene-8-ai39-archinteriors/11940

# ДОМ УЧИЛКИ ПО ИЗО:
    # дом училки (день/ночь улица, внутри ночь):
        # + https://evermotion.org/shop/show_product/scene-7-ae11-archexteriors/2895
            # кухня:
                # + https://evermotion.org/shop/show_product/scene-10-ai1-archinteriors/225
            # гостиная:
                # + https://www.cgmodelx.com/model/details/34124.html
            # спальня:
                # + https://www.cgmodelx.com/model/details/27846.html
            # туалет, ванная комната:
                # + https://www.cgmodelx.com/model/details/36466.html

# ДОМ ДИРЕКТОРА ШКОЛЫ:
    # дом директора (день/ночь улица, внутри ночь):
        # https://evermotion.org/shop/show_product/-scene-6-ae34-archexteriors/15713
            # кухня:
                # + https://evermotion.org/shop/show_product/scene-10-ai22-archinteriors/6348
            # гостиная:
                # + https://www.cgmodelx.com/model/details/10606.html
            # спальня:
                # + https://evermotion.org/shop/show_product/scene-4-ai35-archinteriors/11617
            # туалет, ванная комната:
                # + https://evermotion.org/shop/show_product/scene-9-ai39-archinteriors/11941

# КВАРТИРА УЧИЛКИ ПО ФИЗРЕ:
    # квартира училки (день/ночь улица, внутри ночь):
        # https://evermotion.org/shop/show_product/scene-2-ai17-archinteriors/2442 - (только гостиная)
        # или https://evermotion.org/shop/show_product/ai48-scene-2/14309 - (с кухней)
            # кухня:
                # + https://evermotion.org/shop/show_product/scene-1-ai1-for-mental-ray-archinteriors/2574
            # спальня:
                # + https://www.cgmodelx.com/model/details/54040.html
            # туалет, ванная комната:
                # + https://evermotion.org/shop/show_product/scene-10-ai7-archinteriors/285

# КВАРТИРА УЧИЛКИ ПО ИН-ЯЗУ:
    # квартира училки (день/ночь улица, внутри ночь):
        # + https://evermotion.org/shop/show_product/scene-3-ai7-archinteriors/278
            # спальня:
                # https://evermotion.org/shop/show_product/scene-5-ai30-archinteriors/10474 (
                # + https://evermotion.org/shop/show_product/scene-2-ai10-archinteriors/307
            # туалет, ванная комната:
                # + https://www.cgmodelx.com/model/details/52622.html



# ВИД НА ГОРОД СВЕРХУ:
    # https://evermotion.org/shop/show_product/city-1-set-1-am-131/10356
    # https://evermotion.org/shop/show_product/city-2-set-10-am-133/10454
    # https://evermotion.org/shop/show_product/city-1-am-131/10353
    # https://evermotion.org/shop/show_product/city-2-set-7-am-133/10451
    # + https://www.turbosquid.com/ru/3d-models/3d-archmodels-vol-131-city/762477 (сделать город сверху с домами училок и Барди и тд + пляж внизу) (день/ночь)
    # https://www.turbosquid.com/ru/3d-models/city-beach-3d-model-1166663
    # https://www.turbosquid.com/ru/3d-models/3d-town-houses-buildings-1572467



# УЛИЦЫ, ДОМА:
    # https://evermotion.org/shop/show_product/scene-9-ae30-archexteriors/13345
    # https://evermotion.org/shop/show_product/scene-1-ae24-archexteriors/11488
    # + https://evermotion.org/shop/show_product/scene-3-ae24-archexteriors/11490
    # https://evermotion.org/shop/show_product/scene-4-ae12-archexteriors/3825
    # https://evermotion.org/shop/show_product/archinteriors-vol-16/80
    # + https://www.cgmodelx.com/model/details/41391.html
    # https://evermotion.org/shop/show_product/scene-9-ae11-archexteriors/2897
    # https://evermotion.org/shop/show_product/scene-9-ae19-archexteriors/10573
    # https://evermotion.org/shop/show_product/scene-8-ae33-archexteriors/15440
    # https://evermotion.org/shop/show_product/scene-2-ae35-archexteriors/16084



# ТОРГОВЫЙ ЦЕНТР (:
    # + https://evermotion.org/shop/show_product/scene-1-ae28-archexteriors/12885 (убрать городские постройки вокруг, нужен 2-этажный торговый центр по центру)
    # + https://www.cgmodelx.com/model/details/39303.html
        # продукты (здесь работает парень ст. сестры Барди):
            # https://www.cgmodelx.com/model/details/52012.html
            # + https://www.cgmodelx.com/model/details/54117.html
            # https://www.cgmodelx.com/model/details/51507.html
            # https://www.cgmodelx.com/model/details/30855.html
            # https://www.cgmodelx.com/model/details/29983.html
            # https://www.cgmodelx.com/model/details/20989.html
            # https://www.cgmodelx.com/model/details/11472.html
        # вещи (итемсы):
            # + https://www.cgmodelx.com/model/details/51097.html
            # https://www.cgmodelx.com/model/details/30548.html
        # тетин интернет-магазин, пункт выдачи заказов:
            # https://www.cgmodelx.com/model/details/30074.html
            # https://www.cgmodelx.com/model/details/34866.html
            # + https://www.cgmodelx.com/model/details/21170.html
        # одежда:
            # + https://www.cgmodelx.com/model/details/52514.html
            # https://www.cgmodelx.com/model/details/16575.html
            # https://www.cgmodelx.com/model/details/29176.html
            # https://www.cgmodelx.com/model/details/17051.html
            # https://www.cgmodelx.com/model/details/34726.html
        # салон красоты в ТЦ:
            # + https://www.cgmodelx.com/model/details/14119.html
        # туалет:
            # + https://www.cgmodelx.com/model/details/34193.html
        # кинозал:
            # -https://www.cgmodelx.com/model/details/19503.html
            # -https://www.cgmodelx.com/model/details/11246.html
            # -https://www.cgmodelx.com/model/details/34668.html
        # парковка:
            # + https://www.cgmodelx.com/model/details/10738.html
            # https://www.cgmodelx.com/model/details/52031.html



# ПЛЯЖ:
    # + https://evermotion.org/shop/show_product/scene-9-ae23-archexteriors/11302 (убрать пальмы) (день)
    # https://evermotion.org/shop/show_product/scene-2-ae16-archexteriors/8886
    # https://evermotion.org/shop/show_product/scene-7-ae4-archexteriors/452
        # кафе на пляже:
            # + https://evermotion.org/shop/show_product/scene-3-ai10-archinteriors/308
            # https://www.cgmodelx.com/model/details/31905.html
            # https://www.cgmodelx.com/model/details/54671.html

# ВОКЗАЛ:
    # +https://evermotion.org/shop/show_product/railway-station-34-am223-archmodels/15910


# ОТЕЛЬ:
    # https://evermotion.org/shop/show_product/scene-7-ae23-archexteriors/11300
    # + https://evermotion.org/shop/show_product/scene-2-ae23-archexteriors/11295
        # холл, ресепшн:
            # https://evermotion.org/shop/show_product/scene-4-ai44-archinteriors/12610
            # https://www.cgmodelx.com/model/details/43269.html
            # + https://www.cgmodelx.com/model/details/11103.html
        # лестница наверх:
            # -https://evermotion.org/shop/show_product/scene-6-ai9-archinteriors/301
            # -https://www.cgmodelx.com/model/details/33157.html
            # -https://www.cgmodelx.com/model/details/50327.html
            # -https://www.cgmodelx.com/model/details/11459.html
        # лифт:
            # https://www.cgmodelx.com/model/details/35572.html
            # + https://www.cgmodelx.com/model/details/42523.html
            # https://www.cgmodelx.com/model/details/17333.html
            # + https://www.cgmodelx.com/model/details/11096.html
        # коридор на этажах с номерами:
            # https://www.cgmodelx.com/model/details/32782.html
            # + https://www.cgmodelx.com/model/details/50332.html
            # https://www.cgmodelx.com/model/details/50309.html
            # https://www.cgmodelx.com/model/details/19128.html
        # номер как минимум пока один:
            # https://www.cgmodelx.com/model/details/48929.html
            # +https://www.cgmodelx.com/model/details/51066.html
            # +https://www.cgmodelx.com/model/details/53573.html
            # https://www.cgmodelx.com/model/details/51171.html
            # https://www.cgmodelx.com/model/details/53329.html
            # https://www.cgmodelx.com/model/details/35424.html
            # https://www.cgmodelx.com/model/details/20236.html
        # прачечная:
            # +https://www.cgmodelx.com/model/details/28390.html
        # тележка для разноса вещей по номерам:
            # +https://evermotion.org/shop/show_product/hospital-equipment-39-am70-archmodels/2938 (ассет)



# МЕБЕЛЬ, ЭЛЕМЕНТЫ ИНТЕРЬЕРА + ЛОКАЦИИ:
    # https://evermotion.org/shop/show_product/ai48-scene-4/14311
    # https://www.cgmodelx.com/model/details/52227.html
    # https://www.cgmodelx.com/model/details/52841.html
    # https://www.cgmodelx.com/model/details/53913.html
    # https://www.cgmodelx.com/model/details/51721.html
    # https://www.cgmodelx.com/model/details/54197.html
    # https://www.cgmodelx.com/model/details/32177.html
    # https://evermotion.org/shop/show_product/scene-5-ai27-archinteriors/8714
    # https://evermotion.org/shop/show_product/scene-12-furniture-ai49-archinteriors/14486
    # https://evermotion.org/shop/show_product/scene-8-props-ai49-archinteriors/14482
    # https://evermotion.org/shop/show_product/scene-3-ai8-archinteriors/288
    # https://evermotion.org/shop/show_product/scene-13-furniture-ai49-archinteriors/14487
    # https://evermotion.org/shop/show_product/scene-9-ai36-archinteriors/11644
    # https://evermotion.org/shop/show_product/scene-6-ai33-archinteriors/11230
    # https://www.cgmodelx.com/model/details/20089.html
    # https://www.cgmodelx.com/model/details/38200.html
    # https://www.cgmodelx.com/model/details/54406.html
    # https://www.cgmodelx.com/model/details/47486.html
    # https://www.cgmodelx.com/model/details/47485.html
    # https://www.cgmodelx.com/model/details/42154.html
    # https://evermotion.org/shop/show_product/archmodels-vol-236/17362
    # https://evermotion.org/shop/show_product/scene-8-ai36-archinteriors/11643 - офис (лофт)
    # https://evermotion.org/shop/show_product/archmodels-vol-169/13023 - фитнес



# ВЕЛИК, ГАДЖЕТЫ и т.п.:
    # https://evermotion.org/shop/show_product/archmodels-vol-93/5815 - байк
    # https://evermotion.org/shop/show_product/archmodels-vol-156/12086
    # https://www.cgmodelx.com/model/details/54180.html
    # https://www.cgmodelx.com/model/details/35294.html
    # https://www.cgmodelx.com/model/details/28486.html
    # https://www.cgmodelx.com/model/details/30185.html
    # https://www.cgmodelx.com/model/details/31213.html
    # https://www.cgmodelx.com/model/details/21010.html
    # https://www.cgmodelx.com/model/details/21248.html
    # https://www.cgmodelx.com/model/details/47293.html



# АВТО:
    # https://evermotion.org/shop/show_product/hdmodels-cars-vol-5/7519
    # https://evermotion.org/shop/show_product/hdmodels-cars-vol-4/3422
    # https://evermotion.org/shop/show_product/hdmodels-cars-vol-2/127
    # https://evermotion.org/shop/show_product/-hdmodels-cars-vol-3-/2343
    # https://evermotion.org/shop/show_product/car-6-am132-archmodels/10385
    # https://evermotion.org/shop/show_product/car-22-am132-archmodels/10401
    # https://evermotion.org/shop/show_product/car-38-am132-archmodels/10417
    # https://evermotion.org/shop/show_product/car-3-hdmc6/13063
    # https://evermotion.org/shop/show_product/destroyed-car-7-am165-archmodels/12782
    # https://evermotion.org/shop/show_product/car-4-hdmc6/13064
    # https://evermotion.org/shop/show_product/car-5-hdmc6/13065
    # https://evermotion.org/shop/show_product/car-27-am132-archmodels/10406
