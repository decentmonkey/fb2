...
label ep216_dialogues3_citizens_1:
    ...
    ###################################################################################################################                                ###################################################################################################################
    ################################################################################################################### подходит к ней и шепчет на ухо ###################################################################################################################
    ###################################################################################################################                                ###################################################################################################################
    ...
    menu:
        "Я не собираюсь говорить этого!":
            ...
            cit7 "Где ты вообще нашел эту дуру?!" # указывает на Монику
            m "Это Я хамка?!"
            m "ВОН ОТСЮДА!!!"
            m "БЫСТРО!!!"
            m "!!!"
    ...
    mt "А мне нужны эти сто долларов! Они уже мои!!!"
    mt "Черт!"
    ...
    citizen15 "Эй, чика! Потрогай его!"
    citizen15 "Скажи что на ощупь он огромный! Йо!"
    m "Что?!"
    m "Я?!"
    m "С какой стати?!"
    m "!!!"
    ...
    menu:
        "Я не собираюсь делать этого!":
            ...
            m "Отрабатывать?!"
            m "!!!"
            # да, накину 10 баксов, всего 20!
            # легкие деньги!
            # не буду!
            # чика, никто не узнает, йо!
            # мы у тебя на хате, никто не видит!
            # я знаю он понравился тебе, он нравится таким чикам как ты, йо!
            # иначе ничего не дам
            # моника думает это мерзость, но и правда никто не видит
            # из-за этой старой карги я не могу ничего заработать на улице
            # а мне чем-то надо платить за эту дыру!
    ...
    menu:
        "Я не собираюсь делать этого!":
            ...
            # отсюда вставить предыдущие, уже одетые
            citizen15 "Эй, крошка!"
            citizen15 "Ты чего?! Мы так не договаривались!"
            m "Я передумала!"
            # парень смотрит на свою подругу
    ...
    mt "Она думает, что я просто какая-то никчемная продавщица в магазине одежды." ### проверка на менекен!
    mt "Черт!"
    m "..."
    ...
    # да, держи его так, чтобы он не выскочил!
    # Моника держит во время секса, а он лапает ее грудь
    # Моника говорит какого хрена ты меня лапаешь
    # он говорит, чтобы мой питон был еще больше!
    # секс
    #
    citizen15 "ООООО!!!"
    cit7 "ААААА!!!"
    cit7 "Наконец-то!!! Да!!!"
    # + еще текст, какие вы чики клевые и тд
...
label ep216_dialogues3_citizens_4:
    ...
    # И, вообще, какого хрена! Я - самая лучшая женщина в этом городе!
    # Я что, не возбудила его?! Да как он смеет!
    # Меня должны хотеть все! Даже геи!!!
    mt "!!!"
    return
