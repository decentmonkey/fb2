...
##### отсюда регулярные встречи
    ...
    mt "Ненавижу эту деревенщину Бетти!"
...
label ep214_dialogues5_bardie_ralph_13:
    ...
    menu:
        "Да, мне нужны деньги, Мистер Робертс.":
            ...
            ralph "Но для тебя, Бетти, мне не жалко этих денег."
            mt "ДА!"
            mt "Отлично!!!"
            mt "!!!"
            mt "Черт, надо было озвучить ему большую сумму!"
            ...
            $ log1 = _("Подкаблучник Ральф готов платить мне $ 5 000 за секс с ним по вторникам и четвергам. Теперь я не буду унижаться перед всякими никчемными неудачниками!") #квест-лог
            pass
...
label ep214_dialogues5_bardie_ralph_14:
    ...
    mt "И он будет давать мне $ 5 000."
    mt "Конечно, это мерзко."
    mt "Но это намного лучше, чем терпеть разных грязных извращенцев! Фу!"
    mt "!!!"
    return
...
label ep214_dialogues5_bardie_ralph_16:
    ...
    menu:
        "Предложить Ральфу узаконить наши отношения (в следующем обновлении)":
            return
        "Поцеловать Ральфа.":
...
# если Моника не хочет идти с Бетти на фитнес (пропуск свидания с Ральфом), мысли
label ep214_dialogues5_bardie_ralph_17:
    # не рендерить!!
    mt "Я не хочу встречаться сегодня с этим старикашкой!"
    mt "Мне это все надоело!"
    mt "У меня есть более важные дела!"
    mt "А он никуда от меня не денется."
    mt "Стоит только мне надеть платье этой провинциальной дуры Бетти..."
    mt "Ральф на все готов."
    mt "Если я пропущу встречу с ним один раз, ничего не случится."
    return

# если Моника пропустила свидание с Ральфом, мысли
label ep214_dialogues5_bardie_ralph_18:
    # не рендерить!!
    mt "Мне нужны деньги..."
    mt "А я пропустила встречу с Ральфом..."
    mt "Черт!"
    mt "Надеюсь, он мне не откажет."
    return

# если Моника пропустила свидание с Ральфом
# приходит к нему на очередное свидание в наряде Бетти
label ep214_dialogues5_bardie_ralph_19:
    # заходит в спальню к нему
    mt "Так, Моника..."
    mt "Тебе нужно взять у него деньги."
    mt "Сделай вид, что соскучилась по нему."
    # Ральф недоволен
    ralph "Бетти..."
    ralph "Я думал, ты не придешь..."
    ralph "Как в прошлый раз."
    ralph "Я ждал тебя, Бетти..."
    m "Мистер Робертс, я так переживала, что у меня не получилось прийти к вам!"
    m "У меня были срочные неотложные дела..."
    m "И я не смогла вовремя освободиться!"
    ralph "Надеюсь, Бетти, этого больше не повториться?"
    ralph "И ты будешь более тщательно планировать встречи со мной?"
    ralph "Ты же не хочешь, чтобы я расстраивался?"
    m "Конечно, нет, Мистер Робертс..."
    menu:
        "Попросить у Ральфа денег.":
            m "Мистер Робертс..."
            m "Вы говорили мне, что я могу рассчитывать на вашу помощь..."
            ralph "Да, говорил."
            ralph "Чем я могу тебе помочь, Бетти?"
            m "Мне... Мне нужны деньги, Мистер Робертс..."
            ralph "Нет, Бетти."
            ralph "Я тебе обещал помогать деньгами, если ты будешь регулярно встречаться со мной."
            ralph "Но так как ты не пришла в прошлый раз, то я вынужден отказать тебе сегодня."
            m "Но, Мистер Робертс, я сделала это не по своей воле!"
            ralph "Бетти, сегодня я откажу тебе."
            ralph "Не пропускай больше наших свиданий."
            ralph "Только в таком случае ты можешь рассчитывать на мою помощь."
            mt "Черт!"
            mt "И что мне теперь делать?!"
            mt "?!?!?!"
            mt "Мерзкий старикашка!"
            mt "Он еще мне условия будет ставить!"
            mt "!!!"
            mt "Мне нужно продолжать играть перед ним роль Бетти."
            mt "Тогда в следующий раз он точно не откажет мне."
            pass
        "Не делать этого.":
            mt "Нет!"
            mt "Я не буду просить у него деньги сегодня."
            mt "Я сама могу их заработать!"
            mt "Сейчас я просто сделаю вид, что хочу его."
            pass
    # jump на регулярное меню интима с Ральфом
    return




# продолжение в след. апдейте

# Моника говорит ему, когда решается предложить узаконить отношения с ним
# что будет лушей женой, чем Бетти, у нее никогда не будет болеть голова, она не будет пилить и его и т.п.
# после скандала с Бетти
# она будет жить в своем доме на правах хозяйки, но она должна будет ежедневно готовить Ральфу,
# убираться и заниматься с ним сексом и ходить только в платье Бетти и называться Бетти.
# Ральф ей денег давать не будет.
# Если она что-то не выполняет, то он перестает ей давать деньги.

# у Фреда появляеется новый повод для шантажа, он угрожает, что расскажет Ральфу правду про нее.
