...
label monica_office_cabinet_teleport:
    if obj_name == "Teleport_Monica_Office_Secretary":
        call change_scene("monica_office_secretary") from _call_change_scene_73
        return
    return
label monica_office_cabinet_environment:
    if obj_name == "Biff":
        ...
        if obj_data["action"] == "w":
            call change_scene("monica_office_cabinet_table") from _call_change_scene_74
        return
    if obj_name == "Melanie":
    ...
    if obj_name == "Table":
        if obj_data["action"] == "l":
            mt "Мой... Стол..."
        if obj_data["action"] == "w":
            call change_scene("monica_office_cabinet_table") from _call_change_scene_75
            return
    return
